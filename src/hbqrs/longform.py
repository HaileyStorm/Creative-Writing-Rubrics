...
def validate_route_selection(
    ...
    *,
    ...
    sampled = value["sampling_plan"]["unit_ids"]
    if local_sample_limit is not None and len(sampled) > local_sample_limit:
        raise HBQError(
            f"Sampling plan selected {len(sampled)} units, exceeding the declared limit of {local_sample_limit}"
        )
    unknown_units = set(sampled) - set(unit_ids)
    if unknown_units:
    ...
    for stratum in value["sampling_plan"]["strata"]:
        outside_sample = set(stratum["unit_ids"]) - set(sampled)
        if outside_sample:
            raise HBQError(f"Sampling stratum {stratum['name']!r} contains unselected units")
        stratified.update(stratum["unit_ids"])
    if stratified != set(sampled):
        raise HBQError("Every sampled unit must appear in at least one declared stratum")
...
def build_workflow_report(
    *,
    ...
    sampled = list(route_selection["sampling_plan"]["unit_ids"])
    ...
    if set(local_by_id) != set(sampled):
        raise HBQError("Local results must cover every sampled unit exactly once")
    ...
    report = {
        ...
        "route": {
            "bundle_id": route_selection["selected_bundle_id"],
        ...
        "local_results": [local_by_id[unit_id] for unit_id in sampled],
...
def render_workflow_markdown(report: Mapping[str, Any]) -> str:
    """Render a compact explanatory report without averaging local scores."""
    ...
    lines.extend(
        [
            ...
            "## Route",
            "",
            f"Bundle `{report['route']['bundle_id']}` with {len(report['route']['module_ids'])} selected modules, "
...
def run_longform_workflow(
    ...
    * ``synthesizer(request)`` optionally returns exactly ``findings`` and
    ...
    semantic boundary and never averages local scores.
    """
    ...
    route = validate_route_selection(
        selected, segmentation=segmentation, modules=modules, bundles=bundles
    )
