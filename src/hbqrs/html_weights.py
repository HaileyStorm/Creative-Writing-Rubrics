...
<style>:root{{color-scheme:light dark;font:16px/1.5 system-ui,sans-serif}}body{{max-width:72rem;margin:auto;padding:1.25rem}}header,section,details{{border:1px solid #8886;border-radius:.75rem;padding:.8rem 1rem;margin:.8rem 0}}summary{{cursor:pointer;font-size:1.1rem;font-weight:750}}.count{{font-size:.8rem;opacity:.7}}.muted,small{{opacity:.75}}.weight-row{{display:grid;grid-template-columns:minmax(16rem,1fr) 8rem;gap:1rem;align-items:center;padding:.35rem;border-bottom:1px solid #8884;min-width:0}}.weight-row[hidden]{{display:none}}.weight-row span,.weight-row small{{display:block;min-width:0;overflow-wrap:anywhere}}input,textarea,button{{font:inherit}}input[type=number],input[type=text],input[type=search]{{box-sizing:border-box;width:100%;padding:.35rem}}textarea{{box-sizing:border-box;width:100%;min-height:12rem}}button{{padding:.5rem .7rem;margin:.35rem .35rem .35rem 0}}:focus-visible{{outline:3px solid #e39b18;outline-offset:2px}}.error{{color:#b00020;font-weight:700}}@media(max-width:38rem){{.weight-row{{grid-template-columns:minmax(0,1fr)}}}}@media(prefers-color-scheme:dark){{.error{{color:#ffb4ab}}}}</style></head><body>
...
{section_html}
<section><label for="hbqrs-json">Strict profile JSON</label><textarea id="hbqrs-json" readonly></textarea><button id="hbqrs-download" type="button">Download profile JSON</button><button id="hbqrs-select" type="button">Select JSON for copying</button><p class="muted">Use with <code>--weight-profile</code>; long-form local diagnostics can use a separate <code>--local-weight-profile</code> bound to their local bundle.</p></section>
