...
def render_workflow_status(
    ...
    """Render a no-script status page; optional meta refresh only reloads this file."""
...
<style>:root{{color-scheme:light dark;font:16px/1.5 system-ui,sans-serif}}body{{max-width:58rem;margin:auto;padding:2rem}}header,section{{border:1px solid #8886;border-radius:1rem;padding:1rem 1.25rem;margin:1rem 0}}.phase{{font-size:1.5rem;font-weight:750}}table{{width:100%;border-collapse:collapse}}th,td{{text-align:left;padding:.55rem;border-bottom:1px solid #8885}}small{{opacity:.75}}</style></head><body>
<header><p>HBQ-RS long-form workflow</p><div class="phase">{esc(progress.get('phase', 'unknown')).title()}</div><p>{esc(progress.get('completed_scopes', 0))} of {esc(scope_total)} whole/local scopes complete.</p></header>
