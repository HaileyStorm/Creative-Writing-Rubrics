...
(() => {
  ...
  const downloadJson = (value, filename) => { const blob = new Blob([JSON.stringify(value, null, 2) + '\n'], {type: 'application/json'}); const anchor = document.createElement('a'); anchor.href = URL.createObjectURL(blob); anchor.download = filename; document.body.append(anchor); anchor.click(); anchor.remove(); URL.revokeObjectURL(anchor.href); };
  const downloadProfile = () => { const value = render(); if (!value) return; if (!value.hierarchical_score) { $('hbqrs-status').textContent = 'Enable a custom composite before downloading profile.json.'; $('hbqrs-status').className = 'error'; return; } downloadJson(value.hierarchical_score.profile, 'profile.json'); };
  const download = () => { const value = render(); if (!value) return; if (value.hierarchical_score) downloadJson(value.hierarchical_score.profile, 'profile.json'); downloadJson(value, 'hbqrs-longform-config.json'); };
  const initial = () => {
...
<section aria-labelledby="hbqrs-export-title"><h2 id="hbqrs-export-title">Review and export</h2><p id="hbqrs-status" aria-live="polite" class="muted"></p><label for="hbqrs-config-output">Workflow configuration preview<textarea id="hbqrs-config-output" readonly></textarea></label><div class="actions"><button id="hbqrs-download" type="button">Download configuration JSON</button><button id="hbqrs-download-profile" type="button">Download profile.json</button></div><label for="hbqrs-command-output">Copyable command preview<textarea id="hbqrs-command-output" readonly></textarea></label><div class="actions"><button id="hbqrs-select-command" type="button">Select command for copying</button></div><p class="muted">The command preview is not executed. When a custom composite is enabled, configuration download also writes the exact <code>profile.json</code> named by the command. If a profile uses an unfinished ordinal, bind it during workflow setup and write the resulting strict profile before running the shown profile flag.</p></section>
