...
ROOT=Path(__file__).resolve().parent; STUDY_ID="hbq-figurative-hinge-treatment-successor-v2"; HEAD="6ae9ee0db17dda61bb9adc00a60bcd8072969d5d"; TARGET="penalty.purple_prose.metaphor"; PRIVATE="figurative-hinge-treatment-v2-private"; LEDGER="expected-ledger.json"; REVIEW="independent-sol-review.v1.json"; QUOTE="Include at least one exact_quote copied verbatim from the supplied artifact. Summary-only evidence is invalid."
...
def slots()->list[dict[str,str]]: return [{"slot_id":f"hingev2-{c['case_id']}-r{r}","case_id":c["case_id"],"repeat":str(r),"leaf_id":TARGET} for c in corpus() for r in(1,2)]
...
def validate()->dict[str,Any]:
 ...
 if len(rows)!=4 or len({x["case_id"] for x in rows})!=4 or len(slots())!=8: raise ValueError("balanced geometry drift")
...
def bundle()->list[dict[str,Any]]:
 ...
 return [{"standard":{"id":"HBQ-RS","version":"1.2.0"},"bundle_id":STUDY_ID,"version":1,"title":"Figurative hinge v2 singleton diagnostic","module_ids":[leaf["module_id"]],"task_contract_domain_id":"figurative-hinge-v2","domains":[{"domain_id":"figurative-hinge-v2","title":TARGET,"points":1.0,"components":[{"module_id":leaf["module_id"],"weight":1.0,"include_question_ids":[TARGET]}],"score_mode":"weighted_binary_mean"}],"penalty_modules":[],"hard_gate_policy":{"no_is_invalid":True,"cannot_assess_is_unresolved":True,"not_applicable_requires_condition_or_reason":True,"hard_gates_are_reported_separately":True},"coverage_policy":{"minimum_weighted_coverage":0.0,"below_threshold_status":"PROVISIONAL","score_interval_required_when_unassessed":True,"whole_work_claims_require_whole_work_evidence":True}}]
def task(s:dict[str,str])->dict[str,Any]: return {"contract_version":1,"contract_id":f"{STUDY_ID}-{s['slot_id']}","artifact_id":s["slot_id"],"context":{"artifact_kind":"prose.short_story","declared_scope":"complete supplied passage","completion_status":"complete","background":["Public synthetic figurative hinge v2 diagnostic."],"constraints":["Use only the supplied artifact.",QUOTE,contract()["treatment"]["exact_text"]],"audience":["development-only rubric validation"]},"preferences":[],"priorities":[],"weighted_goals":[],"binding_requirements":[]}
def override(s:dict[str,str],t:dict[str,Any])->dict[str,Any]: return {"format_version":1,"artifact_id":s["slot_id"],"bundle_id":STUDY_ID,"task_contract_sha256":hashlib.sha256(canon(t)).hexdigest(),"contract_id":t["contract_id"],"artifact_kind":t["context"]["artifact_kind"],"declared_scope":t["context"]["declared_scope"],"compatibility_mode":"reviewed_override","decision_id":"figurative-hinge-v2-scope","reviewer":"hbqrs-reviewed-v1","reason":"Reviewed compatibility for a public synthetic figurative hinge v2 diagnostic."}
...
def dry_run(p:Path,runner_call:Any=local)->dict[str,Any]:
 ...
 m={"format_version":1,"study_id":STUDY_ID,"source_head":HEAD,"provider_calls":0,"slots":ss,"max_future_provider_calls":8,"bindings":bindings(p),"promotion":"none"};write(r/"dry-manifest.v1.json",canon(m));return {"provider_calls":0,"slots":8,"manifest_sha256":sha(r/"dry-manifest.v1.json")}
def execute(p:Path,*,allow_remote:bool=False,acknowledged_zero_incremental_charge:bool=False,runner_call:Any=subprocess.run)->dict[str,Any]:
 ...
 write(claim,canon({"study_id":STUDY_ID,"slots":8,"retry_or_resume":"forbidden"}));records=[]
 for s in slots():
  runner_call(command(s,p,True),check=True,text=True,encoding="utf-8",capture_output=True);run=root(p)/"runs"/s["slot_id"];checkpoint=load(run/"responses"/"batch-0001.json")
  ...
  verdicts,n,_=runner._load_checkpoints(run,artifact_text=(root(p)/"inputs"/f"{s['slot_id']}.txt").read_text(encoding="utf-8"),context_texts=[],batch_attempts=1,normalization_policy=runner.EVIDENCE_NORMALIZATION_POLICY)
  if n!=1 or len(verdicts)!=1 or verdicts[0].get("verdict") not in {"YES","NO"}: raise ValueError("strict singleton verdict required")
  runner._validate_exact_quotes(verdicts[0].get("evidence"),artifact_text=(root(p)/"inputs"/f"{s['slot_id']}.txt").read_text(encoding="utf-8"),context_texts=[],question_id=TARGET);records.append({"slot_id":s["slot_id"],"verdict":verdicts[0]["verdict"]})
