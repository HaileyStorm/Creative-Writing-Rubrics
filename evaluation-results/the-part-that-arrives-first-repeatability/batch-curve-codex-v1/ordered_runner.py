...
def _rejected(*, destination: Path, number: int, prompt: str, effective_prompt: str, feedback: Mapping[str, Any] | None, content: str | None, provider: Mapping[str, Any] | None, error: Exception, retryable: bool) -> tuple[Path, dict[str, Any]]:
...
def run(*, output_dir: Path, source: Path, registry: Path, bundles: Path, prefix: Path, binary: Path, response_schema: Path, question_items: Sequence[Mapping[str, Any]], batch_size: int, codex_bin: str, model: str = "gpt-5.6-sol", reasoning: str = "high", artifact_id: str = "the-part-that-arrives-first", invoke: Callable[..., Any] = shared._call_codex) -> dict[str, Any]:
    ...
    if not question_ids or len(set(question_ids)) != len(question_ids) or any(item == "None" for item in question_ids):
    ...
    configuration = {"format_version": 1, "artifact_id": artifact_id, "bundle_id": "prose.short_story", "strict_ai": True, "contexts": [], "question_ids": list(question_ids), "batch_size": batch_size, "batch_attempts": 3, "retry_semantics": "cumulative_batch_attempts_v1", "validation_feedback_policy": shared.VALIDATION_FEEDBACK_POLICY, "checkpoint_format_version": 4, "provider": {"configured": "codex", "reported": "openai", "model": model, "reasoning": reasoning}, "codex_bin": codex_bin}
    ...
    for number, questions in enumerate(chunks, 1):
        ...
        for attempt in range(len(rejected) + 1, 4):
            ...
            try:
                content, provider = invoke(executable=codex_bin, model=model, reasoning=reasoning, prompt=feedback_prompt, output_dir=destination, response_schema=response_schema, batch_number=number, attempt_number=attempt)
                ...
                rejected_path, rejected_record = _rejected(destination=destination, number=number, prompt=prompt, effective_prompt=feedback_prompt, feedback=feedback, content=exc.content, provider=exc.provider_record, error=exc, retryable=exc.retryable)
                ...
                rejected_path, rejected_record = _rejected(destination=destination, number=number, prompt=prompt, effective_prompt=feedback_prompt, feedback=feedback, content=content, provider=provider, error=exc, retryable=True)
                rejected.append((rejected_path, rejected_record))
