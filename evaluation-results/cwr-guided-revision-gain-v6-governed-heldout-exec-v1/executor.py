...
def _current_questions() -> tuple[list[dict[str, Any]], dict[str, Any]]:
    ...
    manifest = {"format_version": 1, "kind": "current_cwr_question_manifest", "python_executable": sys.executable, "registry": {"path": "registry/all_modules.json", "sha256": sha(stable_read(modules))}, "bundles": {"path": "bundles/all_bundles.json", "sha256": sha(stable_read(bundles))}, "question_payload_sha256": sha(canonical(payload)), "question_count": len(payload)}
...
def _validate_runtime(*, route: Mapping[str, Any], result: Mapping[str, Any], runtime: Mapping[str, Any]) -> None:
    ...
    if route["adapter"] == "grok_exec":
        required = {"adapter_version", "requested_model", "reported_model", "requested_reasoning_effort", "reasoning_attested", "reasoning_attestation", "identity_evidence", "cli_version", "session_id_hash", "request_id_hash", "observed_turns", "envelope_hash", "command_identity", "command_identity_hash", "subscription_receipt_hash", "execution_policy", "usage_telemetry"}
        telemetry = runtime.get("usage_telemetry")
        if set(runtime) != required or runtime.get("reported_model") != route["reported_model"] or runtime.get("reasoning_attested") is not False or runtime.get("reasoning_attestation") != "not_reported_by_grok_build_cli" or runtime.get("execution_policy") != "bounded_nonvisual_read_only" or not isinstance(runtime.get("command_identity"), Mapping) or not isinstance(telemetry, Mapping) or telemetry.get("status") not in {"not_reported", "reported"} or not isinstance(runtime.get("session_id_hash"), str) or not isinstance(runtime.get("request_id_hash"), str) or runtime.get("session_id_hash") == runtime.get("request_id_hash") or runtime.get("observed_turns") != 1:
