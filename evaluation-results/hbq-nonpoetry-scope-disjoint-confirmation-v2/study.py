...
BUNDLE_ID = "diagnostic.nonpoetry_scope_disjoint_confirmation_v2"
ATTEMPT_LIFECYCLE_POLICY = "terminal_sidecar_v1"
REPEATS = (1, 2)
SLOTS = 6
TASK_CONTRACT_BOUNDARY = {"completion_status_policy": "schema_required_unknown", "verdict_evidence_sources": ["artifact", "context"], "framing_metadata_is_evidence": False}
PRIVATE_CONTROLLER_NAME = "controller-contract.v2.json"
PRIVATE_CONTROLLER_SHA256 = "85268d62a55a0ab8d00bd935b6f049472163d6b4a6add65808936c108081f075"
PRIVATE_FILES = {
    "fixtures.v2.json": "fe271e819e6f22dcd9e2b6ca58cb804dc3518d35150d134d0884a27fae3cff0a",
    "expected-ledger.v2.json": "0492e38d9566b595ab85d2f7d45f5a418e657448e1302a1eea992d940989ec67",
    "freshness-audit.v1.json": "c707233376e48379efa45fe706584ad165423400018ca1f88d3f2045d21877af",
}
FIXTURE_DIGESTS = (
    "4416960f19302c0f6ac1ce7bbf6f8f6c1a600ce401a35bde7b1bd4c50ffe028e",
    "cc733d3f68282f2f6ac566b050488a8878b010424b777d47e189b56d684262c3",
    "ade11961040ad9e25a180e40b8ea1a5b354d97d9661712acce26f93f0240731c",
)
RUNTIME_BINDINGS = {
...
def execution_root() -> Path:
    return controller_root() / "execution-v4-6ae9ee0"
...
def expected_contract() -> dict[str, Any]:
    ...
    return {
        ...
        "task_contract_boundary": TASK_CONTRACT_BOUNDARY,
...
def validate_package() -> dict[str, Any]:
    ...
    if controller.get("task_contract_boundary") != TASK_CONTRACT_BOUNDARY:
        raise ValueError("Task-contract evidence boundary drifted")
    ...
    private_file("expected-ledger.v2.json", load=False)
    freshness = private_file("freshness-audit.v1.json")
    assert freshness is not None
    if any(value != 0 for value in freshness.get("results", {}).values()):
        raise ValueError("Freshness audit contains a prior-root overlap")
...
def build_schedule() -> list[dict[str, Any]]:
    ...
    for fixture in fixtures["fixtures"]:
        ...
        for repeat in REPEATS:
            ...
            slot = {"slot_id": f"s2dc2-v1-{fixture_id}-r{repeat}", "fixture_id": fixture_id, "fixture_digest": sha256_bytes(canonical_json(fixture)), "repeat": repeat, "leaf_id": LEAF_ID, "artifact_text": artifact_text, "contexts": list(fixture["contexts"]), "artifact_kind": fixture["artifact_kind"], "declared_scope": fixture["declared_scope"], "condition": condition}
...
def task_contract(slot: Mapping[str, Any]) -> dict[str, Any]:
    return {"contract_version": 1, "contract_id": f"s2dc2-{slot['fixture_id']}", "artifact_id": slot["fixture_id"], "context": {"artifact_kind": slot["artifact_kind"], "declared_scope": slot["declared_scope"], "completion_status": "unknown", "background": ["Fresh disjoint passage-status confirmation."], "constraints": ["Use only the supplied artifact and context as verdict evidence; task-contract metadata is framing and must not be cited as evidence."], "audience": ["rubric validation"]}, "preferences": [], "priorities": [], "weighted_goals": [], "binding_requirements": []}


def scope_override(slot: Mapping[str, Any], task: Mapping[str, Any]) -> dict[str, Any]:
    return {"format_version": 1, "artifact_id": slot["fixture_id"], "bundle_id": BUNDLE_ID, "task_contract_sha256": sha256_bytes(canonical_json(task)), "contract_id": task["contract_id"], "artifact_kind": task["context"]["artifact_kind"], "declared_scope": task["context"]["declared_scope"], "compatibility_mode": "reviewed_override", "decision_id": "s2dc2-v1-compatibility", "reviewer": "hbqrs-reviewed-v1", "reason": "Reviewed passage-scope compatibility for this fresh singleton confirmation."}
...
def settle() -> dict[str, Any]:
    ...
    ledger = private_file("expected-ledger.v2.json")
