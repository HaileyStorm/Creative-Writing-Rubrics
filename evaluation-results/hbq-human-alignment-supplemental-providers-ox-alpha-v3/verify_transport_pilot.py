#!/usr/bin/env python3
"""Verify raw Ox Alpha transport evidence without score or label analysis."""
...
def _inside(root: Path, candidate: Path) -> Path:
    try:
        ...
        raise ValueError("Ox v3 raw evidence path escapes its run") from exc
    return candidate
...
def _expected_prompt(frozen: Mapping[str, Any], folder: Path, cell: Mapping[str, Any]) -> bytes:
    ...
    if assets != judge_assets() or not isinstance(assets, Mapping) or assets.get("strict_ai") is not False or assets.get("judge_prefix", {}).get("included") is not False:
        raise ValueError("Ox v3 judge-prefix policy or assets drifted")
    ...
    if [item["question"]["id"] for item in selected] != requested:
        raise ValueError("Ox v3 frozen question sequence cannot render its prompt")
    prompt_names = [item["name"] for item in assets["active_prompts"]]
    if prompt_names != ["BINARY_EVALUATION_PROMPT.md"]:
        raise ValueError("Ox v3 active judge-instruction policy drifted")
    binary = "\n\n".join((prompts_dir() / "judge" / name).read_text(encoding="utf-8").strip() for name in prompt_names)
...
def _raw_transport(run: Path, checkpoint: Mapping[str, Any], prompt: bytes, frozen: Mapping[str, Any]) -> dict[str, Any]:
    ...
    cap1_policy = {**NOUS_TRANSPORT_POLICY, "max_physical_attempts_per_logical_request": 1}
    if not isinstance(provider, Mapping) or provider.get("requested") != {"model": CONTRACT["provider"]["model"], "reasoning_effort": "max"} or provider.get("reported") != {"provider": "nous", "model": CONTRACT["provider"]["model"]} or provider.get("provider_canonical_model") != CONTRACT["provider"]["provider_canonical_model"] or provider.get("transport_policy") != cap1_policy or provider.get("logical_provider_request_count") != 1 or provider.get("physical_http_attempt_count") != 1 or provider.get("recovered_request_count") != 0:
        raise ValueError("Ox v3 requires one unrecovered physical HTTP attempt")
    ...
    if not isinstance(artifacts, Mapping) or set(artifacts) != {"judge_request", "judge_result", "serialization_proof", "evidence_tree"}:
        raise ValueError("Ox v3 provider artifact receipt is incomplete")
    evidence_ref = artifacts["evidence_tree"].get("path") if isinstance(artifacts["evidence_tree"], Mapping) else None
    if not isinstance(evidence_ref, str):
        raise ValueError("Ox v3 evidence root is unbound")
    evidence_root = _inside(run, run / evidence_ref)
    events_path = evidence_root / "events.jsonl"
    provider_receipt = evidence_root / "receipt.json"
    if not events_path.is_file() or not provider_receipt.is_file():
        raise ValueError("Ox v3 raw evidence lacks event or provider receipt")
    try:
        _validate_provider_artifacts(run, checkpoint)
    except Exception as exc:
        raise ValueError("Ox v3 raw provider artifacts are invalid") from exc
    ...
    if len(attempts) != 1 or not isinstance(attempts[0], Mapping):
        raise ValueError("Ox v3 raw evidence has a second physical HTTP attempt")
    attempt = attempts[0]
    started, finished, status = attempt.get("http_started_monotonic_ns"), attempt.get("http_finished_monotonic_ns"), attempt.get("status")
    if isinstance(started, bool) or isinstance(finished, bool) or not isinstance(started, int) or not isinstance(finished, int) or finished <= started or isinstance(status, bool) or not isinstance(status, int) or not 200 <= status < 300 or (finished - started) / 1_000_000_000 >= CONTRACT["transport_pilot"]["maximum_http_seconds_exclusive"]:
        raise ValueError("Ox v3 HTTP attempt is non-2xx or not below 100 seconds")
    ...
    if not isinstance(logical_id, str) or not logical_id or not isinstance(payload_sha, str) or len(payload_sha) != 64:
        raise ValueError("Ox v3 raw HTTP attempt lacks logical request or payload binding")
    bridge_receipt = read_json(provider_receipt)
    run_id = bridge_receipt.get("run_id")
    if bridge_receipt.get("status") != "success" or not isinstance(run_id, str) or not run_id:
        raise ValueError("Ox v3 bridge provider receipt is not successful")
    request_ref = artifacts["judge_request"].get("path") if isinstance(artifacts["judge_request"], Mapping) else None
    result_ref = artifacts["judge_result"].get("path") if isinstance(artifacts["judge_result"], Mapping) else None
    if not isinstance(request_ref, str) or not isinstance(result_ref, str):
        raise ValueError("Ox v3 raw request/result bindings are missing")
    ...
    if not request.is_file() or not result.is_file() or artifacts["judge_request"] != _artifact(run, request) or artifacts["judge_result"] != _artifact(run, result):
        raise ValueError("Ox v3 raw request/result artifact drifted")
    raw_request = read_json(request)
    expected_request = {"schema": "codex-nous-tool-free-judge-request-v2", "model": CONTRACT["provider"]["model"], "reasoning_effort": "max", "max_physical_http_attempts_per_logical_request": 1, "messages": [{"role": "system", "content": "You are a careful HBQ-RS evaluator. Do not use tools or reveal chain-of-thought."}, {"role": "user", "content": prompt.decode("utf-8")}], "response_format": {"type": "json_schema", "json_schema": {"name": "hbqrs_judge", "strict": True, "schema": read_json(schema_dir() / "hbq_judge_response.schema.json")}}}
    if frozen.get("judge_assets") != judge_assets() or raw_request != expected_request:
        raise ValueError("Ox v3 raw request does not bind the frozen judge prompt or response schema")
    return {"receipt_id": f"nous:{provider['evidence_sha256']}:{provider['serialization_proof_sha256']}", "session_id": run_id, "http": {"status": status, "duration_seconds": (finished - started) / 1_000_000_000}, "logical_request_id": logical_id, "payload": {"sha256": payload_sha, "judge_request": _artifact(run, request)}, "raw_evidence": {"root": artifacts["evidence_tree"], "events": _artifact(run, events_path), "judge_result": _artifact(run, result)}, "provider_receipt": _artifact(run, provider_receipt)}
...
def _invocation(work: Path) -> dict[str, Any]:
    ...
    if any(record.get(key) != value for key, value in expected.items()) or not isinstance(record.get("zero_cost_fresh_at_invocation"), str) or set(record) != {*expected, "zero_cost_fresh_at_invocation"}:
        raise ValueError("Ox v3 invocation is unbound")
...
def _claim(work: Path) -> dict[str, Any]:
    ...
    if any(record.get(key) != value for key, value in expected.items()) or not isinstance(record.get("pid"), int) or isinstance(record.get("pid"), bool) or set(record) != {*expected, "pid"}:
        raise ValueError("Ox v3 exclusive claim is unbound")
    return record
...
def verify_cell(work: Path, frozen: Mapping[str, Any], cell: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(configuration, Mapping) or manifest.get("format_version") != 3 or manifest.get("config_sha256") != hashlib.sha256(_json_bytes(configuration)).hexdigest():
        raise ValueError("Ox v3 run manifest is malformed")
    artifact, prompt, task = input_paths(frozen, cell)
    cap1_policy = {**NOUS_TRANSPORT_POLICY, "max_physical_attempts_per_logical_request": 1}
    expected = {"provider": "nous", "model": CONTRACT["provider"]["model"], "reasoning": "max", "batch_size": 16, "retry_policy": {"batch_attempts": 1}, "artifact_id": cell["item_id"], "bundle_id": "prose.short_story", "question_ids": cell["question_ids"], "strict_ai": False, "allow_unattested_reasoning": True, "nous_transport_policy": cap1_policy, "nous_model_policy": {"requested_model": CONTRACT["provider"]["model"], "provider_canonical_model": CONTRACT["provider"]["provider_canonical_model"], "required_reasoning_effort": "max"}}
    assets = frozen.get("judge_assets")
    if any(configuration.get(key) != value for key, value in expected.items()) or _compact(configuration.get("artifact")) != cell["inputs"]["source.md"] or [_compact(item) for item in configuration.get("contexts", [])] != [cell["inputs"]["prompt.md"]] or _compact(configuration.get("task_contract")) != cell["inputs"]["task-contract.json"] or not isinstance(assets, Mapping) or [_compact(item) for item in configuration.get("prompts", [])] != assets.get("active_prompts") or _compact(configuration.get("response_schema")) != assets.get("response_schema"):
        raise ValueError("Ox v3 run settings or inputs drifted")
    ...
    if [path.name for path in checkpoints] != ["batch-0001.json"] or list((run / "responses" / "rejected").rglob("*.json")):
        raise ValueError("Ox v3 requires one clean batch")
    ...
    if checkpoint.get("format_version") != 4 or checkpoint.get("batch") != 1 or checkpoint.get("question_ids") != cell["question_ids"] or checkpoint.get("retry_policy") != {"batch_attempts": 1} or checkpoint.get("accepted_attempt") != 1 or checkpoint.get("recovered_from_rejected") is not None or checkpoint.get("rejected_chain") != {"count": 0, "head_sha256": None} or prompt_bytes != _expected_prompt(frozen, artifact.parent, cell) or checkpoint.get("base_prompt_sha256") != hashlib.sha256(prompt_bytes).hexdigest() or checkpoint.get("prompt_sha256") != hashlib.sha256(prompt_bytes).hexdigest():
        raise ValueError("Ox v3 checkpoint is unbound or recovered")
    try:
        ...
        raise ValueError("Ox v3 completion is not schema-valid") from exc
...
def verify_pilot(work: Path) -> dict[str, Any]:
    ...
    if len(records) != 3 or [record.get("sequence") for record in records] != [1, 2, 3] or any(record.get("status") != "completed" for record in records):
        raise ValueError("Ox v3 did not produce three completed transport cells")
    proofs = [verify_cell(work, frozen, cell) for cell in frozen["cells"]]
    if len({proof["session_id"] for proof in proofs}) != 3 or len({proof["receipt_id"] for proof in proofs}) != 3 or len({proof["logical_request_id"] for proof in proofs}) != 3:
        raise ValueError("Ox v3 reuses a provider session, receipt, or logical request")
    invocation_binding = fingerprint(work / "pilot-invocation.json")
    expected = [{"sequence": number, "cell_id": cell["cell_id"], "item_id": cell["item_id"], "status": "completed", "invocation": invocation_binding, "receipt": fingerprint(work / "pilot-receipts" / f"{cell['cell_id']}.json"), "logical_request_id": proof["logical_request_id"]} for number, (cell, proof) in enumerate(zip(frozen["cells"], proofs), 1)]
    if records != expected or [path.name for path in paths] != [f"{number:04d}-{cell['cell_id']}.json" for number, cell in enumerate(frozen["cells"], 1)]:
        raise ValueError("Ox v3 journal does not bind its immutable receipts")
