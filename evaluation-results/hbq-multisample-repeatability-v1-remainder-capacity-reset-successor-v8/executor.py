#!/usr/bin/env python3
"""Adopt V7's sealed sequence-182 output and execute the untouched 183-330 suffix.
...
def _lexical_package_file() -> Path:
    ...
    while True:
        try:
            ...
            raise ValueError(f"V8 package path is missing: {probe}") from exc
        if stat.S_ISLNK(metadata.st_mode) or bool(getattr(metadata, "st_file_attributes", 0) & 0x400):
            raise ValueError(f"V8 package path contains a symlink/reparse point: {probe}")
...
V7_DEFAULT = Path(r"C:\Users\Haile\Documents\cwr-multisample-capacity-reset-v7-live-1a2d48d")
V6_DEFAULT = Path(r"C:\Users\Haile\Documents\cwr-multisample-capacity-reset-v6-live-unique")

BINDING = "v8-binding.json"
ADMISSION = "admitted-v7-prefix.json"
V7_SETTLEMENT = "v7-sequence-182-settlement.json"
V7_PACKAGE = REPO / "evaluation-results" / "hbq-multisample-repeatability-v1-remainder-capacity-reset-successor-v7"
COHORT_POLICY = V7_PACKAGE / "cohort-compatibility-policy.json"
...
V7_RUNTIME_HEAD = "1a2d48d9c94eee155932ac13cf493a706010fcc4"
EXPECTED_V7_BINDING = "f0f30a0a31500720116e81530d61dfa273804dc36fa949e6e90b92b368e0f824"
EXPECTED_V7_SCHEDULE = "77419f4d84aff2af7c1ac4b8092ad61610d8f8e59c876a51c3afc98109353a73"
EXPECTED_V7_JOURNAL = "ee0bd17bb1c365018d564ac81e20aef25479fcde47595988c14c274d33b88654"
EXPECTED_V7_CLAIM = "ade4239d1927960ef01ddc22351989d3df775ff789f9911e0aafc39be0025ebd"
EXPECTED_V7_DISCLOSURE = "7efc0d024528742f3437d08f97cacb7d16855833621ad89396988e6ffdd8aaa6"
EXPECTED_V7_ACK = "8e78f9b6e72aa0dcc7a7ee80a390ddbd3dffb9e5239afce148572117284a0123"
EXPECTED_V7_CAPACITY_PROOF = "7cc8ff29187e6c1f66216b61a1f3bbeaf3f3766147c10a7629f0ba092c4dbee8"
EXPECTED_V7_OUTPUT_TREE = "d72b5debbd4d80948a60ea477e325d31f92640c76afb554c4b03a8bab8598656"
EXPECTED_V7_SESSION = "01a04569-0c0c-7501-ab85-0f9e2f128231"
EXPECTED_COHORT_POLICY = "399e1ae6f7feccbb4772aa92cd125de9e273c115d6d31586a956ed918c09234b"
EXPECTED_V6 = {
    "binding": "04a7385c9c5a32e718fbc6ec6bc64681fb365edd7fec76a456f0c25c978f63fa",
    "schedule": "fb50b070e1227974f40727f392d2cb4a15c3ec3cdadd0f7804c045f4084f5dad",
    "journal": "cb9215cb97f6b4317352e2f6d957007c96a22c9a7307ab535d11f99d9ec9e1c5",
}
EXPECTED_V7_FAILURE = {
    "executor_location": "executor.py:1935",
    "call": "predecessor_runner._validate_global_sessions(source, work, [*accepted, event])",
    "error": "Successor output lacks unique provider session evidence",
    "cause": "The V1 validator was passed V7 forensic zero-contact sequence 181, whose intentionally absent output has no session evidence.",
    "ordering": [
        "sequence-182 output and one provider message persisted",
        "V1 global-session validator called with forensic 181",
        "validator rejected absent 181 session evidence",
        "V7 journal remained intent-only and claim remained",
    ],
}
...
def _write_immutable(path: Path, value: Mapping[str, Any]) -> None:
    ...
    if path.exists():
        if path.read_text(encoding="utf-8") != payload:
            raise ValueError(f"Immutable V8 artifact drifted: {path.name}")
        return
    path.parent.mkdir(parents=True, exist_ok=True)
    temporary = path.with_name(path.name + ".tmp")
    if temporary.exists():
        raise ValueError(f"Uncertain partial V8 artifact exists: {temporary.name}")
...
def contract() -> dict[str, Any]:
    ...
    if (
        ...
        or value.get("study_id") != "hbq-multisample-repeatability-v1-remainder-capacity-reset-successor-v8"
        or value.get("supersedes", {}).get("v7_runtime_commit") != V7_RUNTIME_HEAD
        or value.get("schedule", {}).get("sha256") != "98fc94c7cd75bbea4f913a144871f31a0ea743695611f797fb86ca7e2e977bd7"
        or value.get("schedule", {}).get("count") != 149
        or value.get("schedule", {}).get("first_sequence") != 182
        or value.get("schedule", {}).get("last_sequence") != 330
        or value.get("admitted_prefix", {}).get("v7_journal_sha256") != EXPECTED_V7_JOURNAL
        or value.get("cohort_compatibility_policy") != {"path": COHORT_POLICY.relative_to(REPO).as_posix(), "sha256": EXPECTED_COHORT_POLICY}
        or value.get("lineage", {}).get("closed_successor_commit") != "9422efffdca1e5e7f82d4bf77588a726af75b4cd"
        or value.get("lineage", {}).get("remainder_study_commit") != REMAINDER_COMMIT
    ):
        raise ValueError("V8 study contract drifted")
    return value
...
def _work_path(work: Path, *parts: str, allow_missing_leaf: bool = True) -> Path:
    ...
    root = _external(work)
    if not root.is_dir():
        raise ValueError("Prepared V8 work root is missing or not a directory")
    candidate = _plain_path(root.joinpath(*parts), allow_missing_leaf=allow_missing_leaf)
    if candidate != root and root not in candidate.parents:
        raise ValueError("V8 work artifact escapes its prepared root")
    return candidate


def _roots(source: Path, closed: Path, v7: Path, work: Path) -> tuple[Path, Path, Path, Path]:
    source_r, closed_r, v7_r = (_external(source), _external(closed), _external(v7))
    work_r = _external(work, allow_missing_leaf=True)
    if not source_r.is_dir() or not closed_r.is_dir() or not v7_r.is_dir():
        raise ValueError("Source, closed, and V7 roots have the wrong type")
    roots = (source_r, closed_r, v7_r, work_r)
    ...
    if work_r.exists() and not work_r.is_dir():
        raise ValueError("Fresh V8 work root must be a directory")
    for root, label in ((source_r, "Source root"), (closed_r, "Closed root"), (v7_r, "V7 evidence root")):
...
def _session_ids_in_output(output: Path) -> list[str]:
    values: list[str] = []
    for path in [output / "response.json", *(output / "responses").glob("batch-*.message.json")]:
        if not path.is_file():
            continue
    ...
    if not values or len(values) != len(set(values)):
        raise ValueError("Output-backed contact cell lacks exactly unique session evidence")
    return values


def _event_output(root: Path, event: Mapping[str, Any]) -> Path:
    return root / "runs" / str(event["item_id"]) / str(event["arm_id"]) / f"run-{int(event['repetition']):02d}"


def _output_sessions(output: Path, event: Mapping[str, Any]) -> list[str]:
    """Read every persisted physical-contact session using the runner's strict topology."""
    values = _physical_output_sessions(output, event)
    if not values or len(values) != len(set(values)):
        raise ValueError("Output session evidence is missing or duplicated")
    return values


def _registry_record(label: str, sessions: list[str]) -> dict[str, Any]:
    if not sessions or len(sessions) != len(set(sessions)):
        raise ValueError(f"Historical {label} session evidence is missing or non-unique")
    ids = sorted(sessions)
    return {"label": label, "count": len(ids), "ids": ids, "sha256": hashlib.sha256(canonical(ids)).hexdigest()}


def _historical_session_registry(source: Path, closed: Path, v7: Path) -> dict[str, Any]:
    source, closed, v7 = _external(source), _external(closed), _external(v7)
    study = _load_successor_study()
    source_sessions = study.predecessor_session_ids(source)
    closed_rows = _jsonl(closed / "successor-schedule-journal.jsonl")
    closed_events = [row for row in closed_rows if row.get("event") == "completed"]
    if [row.get("sequence") for row in closed_events] != list(range(77, 178)):
        raise ValueError("Closed successor does not contain the exact completed 77-177 prefix")
    closed_sessions = [session for event in closed_events for session in _output_sessions(_event_output(closed, event), event)]
    binding = read_json(v7 / "v7-binding.json")
    v6 = _external(Path(str(binding.get("roots", {}).get("v6", ""))))
    if v6 != _external(V6_DEFAULT):
        raise ValueError("V7 does not bind the exact immutable V6 root")
    if any(sha(v6 / name) != EXPECTED_V6[key] for key, name in (("binding", "v6-binding.json"), ("schedule", SCHEDULE), ("journal", JOURNAL))):
        raise ValueError("V6 session-registry provenance drifted")
    v6_schedule = _jsonl(v6 / SCHEDULE)
    v6_rows = _jsonl(v6 / JOURNAL)
    v4_admission = v6_rows[0] if v6_rows else None
    if not isinstance(v4_admission, Mapping) or v4_admission.get("event") != "admitted-prefix" or v4_admission.get("sequence") != 178 or v4_admission.get("v4_run_sha256") != "42b223e0ef9ae6d258c68b35ae1a08f1ecce1d073b64b7f19b77b95e3b584f70":
        raise ValueError("V6 does not bind the immutable completed sequence 178 admission")
    v4 = _external(Path(str(v4_admission.get("v4_root", ""))))
    v4_event = {"sequence": 178, "item_id": "hanna-52", "arm_id": "hbq_short_story_batch32", "repetition": 5}
    v4_sessions = _output_sessions(_event_output(v4, v4_event), v4_event)
    v6_events = [v6_schedule[index] for index in (0, 1)]
    if [event.get("sequence") for event in v6_events] != [179, 180] or [row.get("sequence") for row in v6_rows if row.get("event") == "completed"] != [179, 180]:
        raise ValueError("V6 session-registry completion prefix drifted")
    v6_sessions = [session for event in v6_events for session in _output_sessions(_event_output(v6, event), event)]
    records = [_registry_record("source_1_76", source_sessions), _registry_record("closed_77_177", closed_sessions), _registry_record("v4_178", v4_sessions), _registry_record("v6_179_180", v6_sessions), _registry_record("adopted_v7_182", [EXPECTED_V7_SESSION])]
    all_ids = [session for record in records for session in record["ids"]]
    if len(all_ids) != len(set(all_ids)):
        raise ValueError("Historical session registry contains a cross-root collision")
    return {"records": records, "count": len(all_ids), "ids": sorted(all_ids), "sha256": hashlib.sha256(canonical(sorted(all_ids))).hexdigest(), "excluded_zero_contact_sequence": 181}


def admit_v7_prefix(source_root: Path, closed_root: Path, v7_root: Path = V7_DEFAULT) -> dict[str, Any]:
    """Adopt only the sealed V7 182 result; V7 remains immutable and unresolved."""
    v7 = _external(v7_root)
    if not v7.is_dir():
        raise ValueError("V7 immutable evidence root is missing")
    settlement_path = HERE / V7_SETTLEMENT
    settlement_sha = sha(settlement_path)
    paths = {"binding": v7 / "v7-binding.json", "schedule": v7 / SCHEDULE, "journal": v7 / JOURNAL, "claim": v7 / CLAIM, "disclosure": v7 / DISCLOSURE, "acknowledgement": v7 / DISCLOSURE_ACK, "capacity": v7 / PROOFS / f"{EXPECTED_V7_CAPACITY_PROOF}.json"}
    expected_hashes = {"binding": EXPECTED_V7_BINDING, "schedule": EXPECTED_V7_SCHEDULE, "journal": EXPECTED_V7_JOURNAL, "claim": EXPECTED_V7_CLAIM, "disclosure": EXPECTED_V7_DISCLOSURE, "acknowledgement": EXPECTED_V7_ACK, "capacity": EXPECTED_V7_CAPACITY_PROOF}
    if any(not path.is_file() or sha(path) != expected_hashes[name] for name, path in paths.items()):
        raise ValueError("V7 immutable binding, schedule, journal, claim, disclosure, acknowledgement, or capacity evidence drifted")
    binding = read_json(paths["binding"])
    if binding.get("study_id") != "hbq-multisample-repeatability-v1-remainder-capacity-reset-successor-v7" or binding.get("runtime", {}).get("git") != {"head": V7_RUNTIME_HEAD, "upstream": V7_RUNTIME_HEAD} or binding.get("runtime", {}).get("sha256") != "60e9936163653de3432f3843465016cf310838c20b78e054b9a7da9345877e35":
        raise ValueError("V7 runtime identity drifted")
    schedule = _jsonl(paths["schedule"])
    if [row.get("sequence") for row in schedule] != list(range(181, 331)):
        raise ValueError("V7 schedule order drifted")
    event = schedule[1]
    expected_event = {"sequence": 182, "item_id": "hanna-523", "arm_id": "naplan_narrative_2022", "repetition": 1}
    if {key: event.get(key) for key in expected_event} != expected_event:
        raise ValueError("V7 schedule does not identify exact sequence 182")
    journal = _jsonl(paths["journal"])
    if [row.get("event") for row in journal] != ["admitted-prefix", "forensic-precontact", "attempt-intent"] or journal[-1] != {"event": "attempt-intent", "sequence": 182, "capacity_proof_sha256": EXPECTED_V7_CAPACITY_PROOF, "observed_at": "2026-08-27T22:47:14.0219269Z"}:
        raise ValueError("V7 journal is not the exact unresolved sequence-182 intent")
    claim = read_json(paths["claim"])
    if claim.get("pid") != 17088 or not _pid_is_dead(17088) or any(claim.get(key) != event[key] for key in ("sequence", "item_id", "arm_id")) or claim.get("binding_sha256") != EXPECTED_V7_BINDING or claim.get("study_id") != "hbq-multisample-repeatability-v1-remainder-capacity-reset-successor-v7" or claim.get("source_frozen_contract_sha256") != "5fb06e5a4775ecfe1cee10132e52100733c7e765e8eae9865374bb23f1addddd":
        raise ValueError("V7 sequence-182 dead claim identity drifted")
    capacity = read_json(paths["capacity"])
    if capacity.get("sequence") != 182 or capacity.get("capacity", {}).get("observed_at") != journal[-1]["observed_at"]:
        raise ValueError("V7 sequence-182 capacity proof drifted")
    output = v7 / "runs" / event["item_id"] / event["arm_id"] / "run-01"
    output_snapshot = _manifest(output)
    rows = output_snapshot
    settlement = read_json(settlement_path)
    expected_evidence = {
        "binding_sha256": EXPECTED_V7_BINDING,
        "schedule_sha256": EXPECTED_V7_SCHEDULE,
        "journal_sha256": EXPECTED_V7_JOURNAL,
        "claim_sha256": EXPECTED_V7_CLAIM,
        "disclosure_sha256": EXPECTED_V7_DISCLOSURE,
        "acknowledgement_sha256": EXPECTED_V7_ACK,
        "capacity_proof_sha256": EXPECTED_V7_CAPACITY_PROOF,
    }
    if (
        settlement.get("format_version") != 1
        or settlement.get("kind") != "v7_sequence_182_post_dispatch_validator_settlement"
        or settlement.get("study_id") != contract()["study_id"]
        or settlement.get("not_provider_attestation") is not True
        or settlement.get("evidence_class") != "immutable_local_v7_files_and_local_failure_projection"
        or settlement.get("event") != expected_event
        or settlement.get("v7_evidence") != expected_evidence
        or settlement.get("claim") != {"pid": 17088, "required_dead_pid": True, "binding_sha256": EXPECTED_V7_BINDING}
        or settlement.get("v7_runtime") != {"git_head": V7_RUNTIME_HEAD, "git_upstream": V7_RUNTIME_HEAD, "runtime_sha256": "60e9936163653de3432f3843465016cf310838c20b78e054b9a7da9345877e35", "executor_path": "evaluation-results/hbq-multisample-repeatability-v1-remainder-capacity-reset-successor-v7/executor.py", "executor_sha256": "741245d34a01bbca011355d3981168c9a0e9c8d364225d66f4dda0e5e4e07b7c"}
        or settlement.get("provider_contact") != {"recorded_provider_contacts": 1, "session_id": EXPECTED_V7_SESSION, "provider": "codex", "model": "gpt-5.6-sol", "reasoning": "high"}
        or settlement.get("output", {}).get("root") != "runs/hanna-523/naplan_narrative_2022/run-01"
        or settlement.get("v8_effect") != "V8 adopts only the exact output-backed, one-contact sequence 182; it retains sequence 181 as an explicit zero-contact logical prefix and begins dispatch at 183. V7 is never resumed, mutated, or resent."
    ):
        raise ValueError("V7 settlement identity or immutable evidence commitments drifted")
    output_record = settlement.get("output")
    if not isinstance(output_record, Mapping) or rows != output_record.get("files") or _manifest_sha(rows) != EXPECTED_V7_OUTPUT_TREE or output_record.get("tree_sha256") != EXPECTED_V7_OUTPUT_TREE:
        raise ValueError("V7 sequence-182 output manifest drifted")
    if not (output / "pass.json").is_file() or sha(output / "pass.json") != "b02952fe95e736867d62dfaa780466a367ffbe15612073185b625bab38132067" or _recorded_provider_contacts(v7, event) != 1:
        raise ValueError("V7 sequence-182 output or contact evidence drifted")
    response = read_json(output / "response.json")
    passed = read_json(output / "pass.json")
    reported = response.get("provider", {}).get("reported") if isinstance(response.get("provider"), Mapping) else None
    configuration = passed.get("configuration")
    if (
        not isinstance(reported, Mapping)
        or not isinstance(configuration, Mapping)
        or reported != {"provider": "openai", "model": "gpt-5.6-sol", "reasoning_effort": "high", "session_id": EXPECTED_V7_SESSION}
        or {key: configuration.get(key) for key in ("provider", "model", "reasoning")} != {"provider": "codex", "model": "gpt-5.6-sol", "reasoning": "high"}
        or response.get("config_sha256") != passed.get("config_sha256")
        or response.get("result_sha256") != sha(output / "result.json")
    ):
        raise ValueError("V7 sequence-182 response, configuration, result, model, or reasoning evidence drifted")
    sessions = _session_ids_in_output(output)
    if sessions != [EXPECTED_V7_SESSION] or settlement.get("provider_contact", {}).get("session_id") != EXPECTED_V7_SESSION or settlement.get("provider_contact", {}).get("recorded_provider_contacts") != 1:
        raise ValueError("V7 sequence-182 provider session evidence drifted")
    failure = settlement.get("failure_projection")
    if failure != EXPECTED_V7_FAILURE:
        raise ValueError("V7 post-dispatch failure projection drifted")
    if any(row.get("event") in {"provider-contacts", "completed"} and row.get("sequence") == 182 for row in journal):
        raise ValueError("V7 sequence-182 is no longer the sealed nonresendable intent")
    registry = _historical_session_registry(source_root, closed_root, v7)
    if _manifest(output) != output_snapshot or sha(settlement_path) != settlement_sha:
        raise ValueError("V7 evidence or V8 settlement changed during adoption validation")
    return {"sequence": 182, "v7_root": str(v7), "logical_completed_sequences": [179, 180, 182], "zero_contact_sequence": 181, "session_bearing": [{"sequence": 182, "session_id": EXPECTED_V7_SESSION, "recorded_provider_contacts": 1, "output_tree_sha256": EXPECTED_V7_OUTPUT_TREE}], "session_registry": registry, "v7_binding_sha256": EXPECTED_V7_BINDING, "v7_journal_sha256": EXPECTED_V7_JOURNAL, "settlement_sha256": settlement_sha}
...
def _load_remainder_study() -> Any:
    return _load_module(REMAINDER_STUDY, "v8_remainder_study")


def _load_successor_study() -> Any:
    return _load_module(SUCCESSOR_STUDY, "v8_successor_study")
...
def _runtime_projection(frozen: Mapping[str, Any]) -> dict[str, Any]:
    ...
    files = [_runtime_file(path, require_tracked=False) for path in [CONTRACT_PATH, HERE / V7_SETTLEMENT, COHORT_POLICY, HERE / "executor.py"]]
...
def _fresh_schedule(closed_root: Path) -> list[dict[str, Any]]:
    ...
    if len(schedule) != c["count"] or [row.get("sequence") for row in schedule] != list(range(c["first_sequence"], c["last_sequence"] + 1)) or hashlib.sha256(canonical(schedule)).hexdigest() != c["sha256"]:
        raise ValueError("V8 fresh schedule commitment drifted")
    return schedule
...
def _cohort_policy() -> dict[str, Any]:
    path = _plain_path(COHORT_POLICY)
    ...
    if (
        ...
        or policy.get("study_id") != "hbq-multisample-repeatability-v1-remainder-capacity-reset-successor-v7"
...
def _validate_scope_overrides(work: Path, source: Path, schedule: list[dict[str, Any]], frozen: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    for record in records:
        ...
        if read_json(path) != record["schema"] or sha(path) != record["sha256"]:
            raise ValueError("V8 scope compatibility override drifted")
    return records
...
def _preflight_disclosure(source: Path, schedule: list[dict[str, Any]], frozen: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "cohort_compatibility_policy": {"path": COHORT_POLICY.relative_to(REPO).as_posix(), "sha256": sha(COHORT_POLICY)},
...
def _prepared_binding(source: Path, closed: Path, v7: Path, work: Path, admission: Mapping[str, Any], schedule: list[dict[str, Any]], disclosure_sha256: str, frozen: Mapping[str, Any]) -> dict[str, Any]:
    c = contract()
    return {
        "format_version": 1,
        "study_id": c["study_id"],
        "roots": {"source": str(source), "closed": str(closed), "v7": str(v7)},
...
def _append(path: Path, value: Mapping[str, Any]) -> None:
    ...
    try:
        ...
        while offset < len(payload):
            ...
            if written < 1:
                raise OSError("V8 journal write was partial")
            offset += written
        os.fsync(descriptor)
    finally:
        os.close(descriptor)
...
def _claim(work: Path, source: Path, event: Mapping[str, Any]) -> Path:
    ...
    try:
        ...
        raise ValueError("Exclusive V8 claim exists; stop without duplicate dispatch") from exc
    try:
        payload = canonical(value) + b"\n"
        if os.write(descriptor, payload) != len(payload):
            raise OSError("V8 claim write was partial")
        os.fsync(descriptor)
...
def _retry_pause_result(work: Path, schedule: list[dict[str, Any]], accepted: list[dict[str, Any]], remaining: list[dict[str, Any]], pause: Mapping[str, Any]) -> dict[str, Any]:
    ...
    return {
        "provider_contacts_recorded": _total_recorded_contacts(work, schedule),
        "admitted_sequence": 182,
...
def _assert_plain_work_tree(work: Path) -> None:
    _assert_plain_tree(work, "V8 work root")
...
def _rejected_attempt_paths(output: Path) -> list[Path]:
    ...
    if not root.is_dir():
        return []
    paths: list[Path] = []
    for batch in sorted(root.iterdir(), key=lambda path: path.name):
        if not batch.is_dir() or not re.fullmatch(r"batch-[0-9]{4}", batch.name):
            raise ValueError("V8 rejected HBQ evidence has an unexpected path")
        entries = sorted(batch.iterdir(), key=lambda path: path.name)
        if not entries or any(not path.is_file() or not re.fullmatch(r"attempt-[0-9]{4}\.json", path.name) for path in entries):
            raise ValueError("V8 rejected HBQ attempt topology is malformed")
        paths.extend(entries)
    return paths


def _one_provider_session(value: Any, label: str) -> str:
    found = _session_ids(value)
    if len(found) != 1 or not isinstance(found[0], str):
        raise ValueError(f"{label} lacks one unambiguous provider session")
    return found[0]


def _native_rejected_sessions(output: Path) -> list[str]:
    root = output / "attempts"
    if not root.exists():
        return []
    if not root.is_dir():
        raise ValueError("Native rejected evidence root is not a directory")
    paths = sorted(root.iterdir(), key=lambda path: path.name)
    if any(not path.is_file() or not re.fullmatch(r"rejected-[0-9]{4}\.json", path.name) for path in paths):
        raise ValueError("Native rejected evidence has an unexpected path")
    if [path.name for path in paths] != [f"rejected-{ordinal:04d}.json" for ordinal in range(1, len(paths) + 1)]:
        raise ValueError("Native rejected evidence does not have contiguous attempt filenames")
    return [_one_provider_session(read_json(path).get("response"), "Native rejected checkpoint") for path in paths]


def _physical_output_sessions(output: Path, event: Mapping[str, Any]) -> list[str]:
    _assert_plain_tree(output, "Provider-session output")
    if event.get("arm_id") == "hbq_short_story_batch32":
        run = read_json(output / "run.json")
        if not isinstance(run.get("configuration"), Mapping):
            raise ValueError("HBQ run manifest lacks configuration")
        accepted = sorted((output / "responses").glob("batch-????.json"))
        if len(accepted) != 6 or [path.name for path in accepted] != [f"batch-{batch:04d}.json" for batch in range(1, 7)]:
            raise ValueError("HBQ output lacks six accepted batch checkpoints")
        accepted_records: dict[tuple[int, int], str] = {}
        for batch, path in enumerate(accepted, 1):
            record = read_json(path)
            attempt = record.get("accepted_attempt")
            if record.get("batch") != batch or not isinstance(attempt, int) or not 1 <= attempt <= 3:
                raise ValueError("HBQ accepted checkpoint lacks a valid batch/accepted_attempt")
            accepted_records[(batch, attempt)] = _one_provider_session(record, "HBQ accepted checkpoint")
        rejected_records: dict[tuple[int, int], str] = {}
        for path in _rejected_attempt_paths(output):
            match = re.fullmatch(r"batch-([0-9]{4})/attempt-([0-9]{4})\.json", path.relative_to(output / "responses" / "rejected").as_posix())
            if match is None:
                raise ValueError("HBQ rejected checkpoint ordinal is malformed")
            ordinal = (int(match.group(1)), int(match.group(2)))
            rejected_records[ordinal] = _one_provider_session(read_json(path), "HBQ rejected checkpoint")
        expected = {(batch, attempt) for (batch, attempt) in accepted_records for attempt in range(1, attempt + 1)}
        if set(accepted_records) | set(rejected_records) != expected or set(accepted_records) & set(rejected_records):
            raise ValueError("HBQ accepted/rejected records do not form the exact physical attempt prefix")
        responses = output / "responses"
        messages: dict[tuple[int, int], Path] = {}
        for path in responses.iterdir():
            if path.name == "rejected":
                continue
            message = re.fullmatch(r"batch-([0-9]{4})\.attempt-([0-9]{4})\.message\.json", path.name)
            if message is not None:
                if not path.is_file() or not isinstance(read_json(path), Mapping):
                    raise ValueError("HBQ provider message is malformed")
                messages[(int(message.group(1)), int(message.group(2)))] = path
                continue
            if not re.fullmatch(r"batch-[0-9]{4}(?:\.json|\.prompt\.txt\.gz|\.accepted-[0-9]{4}\.message\.txt)", path.name):
                raise ValueError("HBQ response topology has an unexpected path")
        if set(messages) != expected:
            raise ValueError("HBQ provider-message ordinals do not match session-bearing contacts")
        sessions = [*(accepted_records.values()), *(rejected_records.values())]
        if len(sessions) != len(messages) or len(sessions) != len(set(sessions)):
            raise ValueError("HBQ physical contacts and sessions are not one-to-one")
        return sessions
    messages = _native_message_evidence(output, allow_missing=False)
    message_ordinals = {ordinal for ordinal, _path in messages}
    rejected = _native_rejected_sessions(output)
    response_records: dict[int, str] = {ordinal: session for ordinal, session in enumerate(rejected, 1)}
    response = output / "response.json"
    if response.is_file():
        ordinal = 1 if not rejected else 2
        response_records[ordinal] = _one_provider_session(read_json(response), "Native accepted response")
    retry_root = output / "retry-attempts"
    if retry_root.exists():
        retry = retry_root / "attempt-0002" / "response.json"
        if retry.is_file():
            if 2 in response_records:
                raise ValueError("Native retry response duplicates an accepted response ordinal")
            response_records[2] = _one_provider_session(read_json(retry), "Native retry response")
    if set(response_records) != message_ordinals or list(sorted(response_records)) != list(range(1, len(response_records) + 1)):
        raise ValueError("Native response-record ordinals do not match physical messages")
    sessions = [response_records[ordinal] for ordinal in sorted(response_records)]
    if len(sessions) != len(messages) or len(sessions) != len(set(sessions)):
        raise ValueError("Native physical contacts and sessions are not one-to-one")
    return sessions
...
def _require_no_orphan_output_cells(work: Path, pending: list[Mapping[str, Any]], *, allow_sequence: int | None = None) -> None:
    for candidate in pending:
        ...
        if _output_path(work, candidate).parent.exists():
            raise ValueError("A future or unjournaled V8 cell already has an output tree; no orphan output adoption or resume is allowed")
...
def _require_paused_cell_resumable(work: Path, event: Mapping[str, Any]) -> None:
    ...
    if event["arm_id"] != "hbq_short_story_batch32":
        if target.exists():
            raise ValueError("Paused V8 cell has an orphan completed target; no successor resume or adoption is allowed")
        rejected = _native_rejected_sessions(output)
        if not rejected:
    ...
    for completed_artifact in ("score.json", "diagnostic.json"):
        if (output / completed_artifact).exists():
            raise ValueError("Paused V8 HBQ cell has completed output evidence; no successor resume or adoption is allowed")
    if target.exists():
        ...
        if not isinstance(manifest.get("configuration"), Mapping) or not isinstance(manifest.get("config_sha256"), str) or not isinstance(manifest.get("run_id"), str):
            raise ValueError("Paused V8 HBQ cell has an orphan run.json rather than a resumable runner manifest")
    disclosure = read_json(_work_path(work, DISCLOSURE, allow_missing_leaf=False))
    cells = [cell for cell in disclosure.get("cells", []) if isinstance(cell, Mapping) and all(cell.get(key) == event.get(key) for key in ("sequence", "item_id", "arm_id", "repetition"))]
    if len(cells) != 1:
        raise ValueError("Paused V8 HBQ cell has no unique preflight disclosure")
    payloads = cells[0].get("payload", {}).get("provider_payloads") if isinstance(cells[0].get("payload"), Mapping) else None
    if not isinstance(payloads, list) or not payloads:
        raise ValueError("Paused V8 HBQ disclosure has no batch plan")
    complete = True
    for payload in payloads:
        if not isinstance(payload, Mapping) or not isinstance(payload.get("batch"), int) or not isinstance(payload.get("question_ids"), list):
            raise ValueError("Paused V8 HBQ disclosure batch plan is malformed")
        ...
        if (
            ...
            break
    if complete:
        raise ValueError("Paused V8 HBQ cell already has all accepted batch checkpoints; no orphan completion adoption is allowed")
...
def _journaled_provider_contacts(work: Path, schedule: list[dict[str, Any]]) -> int:
    ...
    for row in _read_journal(work):
        ...
        if event is None or _recorded_provider_contacts(work, event) != row.get("recorded_provider_contacts"):
            raise ValueError("V8 journaled provider contacts do not exactly match persisted attempt evidence")
        total += int(row["recorded_provider_contacts"])
    return total


def _total_recorded_contacts(work: Path, schedule: list[dict[str, Any]]) -> int:
    """V7's immutable adopted output is one factual contact outside V8's journal."""
    return 1 + _journaled_provider_contacts(work, schedule)


def _accounting(schedule: list[dict[str, Any]], accepted: list[dict[str, Any]]) -> dict[str, Any]:
    provider_eligible = schedule
    accepted_provider_eligible = accepted
    return {
        "logical_cells": len(schedule),
        "accepted_cells": len(accepted),
        "accepted_count_basis": "the immutable adopted V7 sequence-182 output plus validated V8 output files; forensic zero-contact sequence 181 is outside this dispatch schedule",
        ...
        "recorded_provider_contacts": "one immutable adopted V7 contact plus persisted per-settled-cell V8 journal evidence",
    }
...
def _capacity_proof(work: Path, digest: str) -> dict[str, Any]:
    ...
    if sha(path) != digest:
        raise ValueError("V8 capacity proof commitment drifted")
    value = read_json(path)
    if set(value) != {"sequence", "capacity"} or not isinstance(value.get("capacity"), Mapping):
        raise ValueError("V8 capacity proof payload is malformed")
...
def _retry_pause_record(work: Path, row: Mapping[str, Any], prior_intent: Mapping[str, Any], event: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if (
        ...
        raise ValueError("V8 retry-disclosure pause is malformed")
    ...
    if sha(disclosure_path) != row["retry_disclosure_sha256"] or disclosure.get("study_id") != contract()["study_id"]:
        raise ValueError("V8 retry disclosure commitment drifted")
    ...
    if (
        ...
        raise ValueError("V8 retry-disclosure pause does not bind the changed provider payload")
    ...
    if disclosure != expected_disclosure:
        raise ValueError("V8 retry disclosure is not an exact immutable hook-context projection")
    return dict(row)
...
def _accepted(work: Path, schedule: list[dict[str, Any]], admission: Mapping[str, Any]) -> list[dict[str, Any]]:
    ...
    if not rows or rows[0] != expected_prefix:
        raise ValueError("V8 admission journal is missing or drifted")
    expected_adoption = {"event": "adopted-v7-output", "sequence": 182, "settlement_sha256": admission["settlement_sha256"]}
    if len(rows) < 2 or rows[1] != expected_adoption:
        raise ValueError("V8 adopted V7 output must precede every V8 intent")
    active: dict[int, dict[str, Any]] = {}
    ...
    for row in rows[1:]:
        ...
        if kind == "adopted-v7-output":
            if (
                expected_index != 0
                or sequence != 182
                or set(row) != {"event", "sequence", "settlement_sha256"}
                or row.get("settlement_sha256") != admission["settlement_sha256"]
            ):
                raise ValueError("V8 adopted V7 output settlement is malformed")
            expected_index += 1
            ...
            if (
                ...
                raise ValueError("V8 attempt-intent journal is not an ordered prefix")
            ...
            if proof.get("sequence") != sequence or row.get("observed_at") != proof["capacity"].get("observed_at"):
                raise ValueError("V8 attempt intent does not match its capacity proof")
            ...
            if (
                ...
                raise ValueError("V8 retry-disclosure pause is not the next unsettled cell")
            ...
            if (
                ...
                raise ValueError("V8 retry intent is not bound to its paused changed payload")
            ...
            if row.get("observed_at") != retry_proof["capacity"]["observed_at"] or _validate_time(retry_proof["capacity"]["observed_at"]) <= _validate_time(prior_proof["capacity"]["observed_at"]):
                raise ValueError("V8 retry intent lacks fresh capacity evidence")
            ack_path = _retry_ack_path(work, str(row["retry_disclosure_sha256"]))
            if sha(ack_path) != row["retry_ack_sha256"]:
                raise ValueError("V8 retry acknowledgement commitment drifted")
            ...
            if (
                ...
                raise ValueError("V8 provider-contact journal is not an ordered settled attempt")
            ...
            if not target.is_file() or _recorded_provider_contacts(work, schedule[expected_index]) != row["recorded_provider_contacts"]:
                raise ValueError("V8 provider-contact evidence does not match the persisted runner artifacts")
            ...
            if (
                ...
                raise ValueError("V8 completion journal is not an ascending contiguous prefix")
            target = _output_path(work, schedule[expected_index])
            if not target.is_file() or sha(target) != row["output_sha256"]:
                raise ValueError("V8 completed output commitment drifted")
            active.pop(sequence)
            contacts.remove(sequence)
            expected_index += 1
        else:
            raise ValueError("V8 journal contains an unknown event")
    ...
    if active or contacts:
        ...
        raise ValueError(f"V8 has an unresolved attempt intent; stop without resend{suffix}")
    return schedule[:expected_index]
...
def prepare(source_root: Path, closed_root: Path, v7_root: Path, work_root: Path) -> dict[str, Any]:
    source, closed, v7, work = _roots(source_root, closed_root, v7_root, work_root)
    if work.exists() and any(work.iterdir()):
        raise ValueError("Fresh V8 root must be empty; use dry-run validation to reload it")
    admission = admit_v7_prefix(source, closed, v7)
    ...
    binding = _prepared_binding(source, closed, v7, work, admission, schedule, sha(work / DISCLOSURE), frozen)
    ...
    _append(work / JOURNAL, {"event": "adopted-v7-output", "sequence": 182, "settlement_sha256": admission["settlement_sha256"]})
    return {"provider_calls": 0, "admitted_sequence": 182, "adopted_sequence": 182, "cells": len(schedule), "first_sequence": schedule[0]["sequence"], "last_sequence": schedule[-1]["sequence"], "work_root": str(work), "disclosure_path": str(work / DISCLOSURE), "disclosure_sha256": sha(work / DISCLOSURE), "destination": "codex", "profile": contract()["provider"], "accounting": _accounting(schedule, [])}


def _verify_prepared(source_root: Path, closed_root: Path, v7_root: Path, work_root: Path) -> tuple[dict[str, Any], list[dict[str, Any]], dict[str, Any]]:
    source, closed, v7, work = _roots(source_root, closed_root, v7_root, work_root)
    if not work.is_dir():
        raise ValueError("Prepared V8 root is missing")
    _assert_plain_work_tree(work)
    admission = admit_v7_prefix(source, closed, v7)
    schedule = _fresh_schedule(closed)
    frozen = read_json(source / "frozen-run-contract.json")
    ...
    if read_json(work / DISCLOSURE) != expected_disclosure:
        raise ValueError("V8 preflight disclosure drifted")
    expected = _prepared_binding(source, closed, v7, work, admission, schedule, sha(work / DISCLOSURE), frozen)
    if read_json(work / BINDING) != expected or read_json(work / ADMISSION) != admission or _jsonl(work / SCHEDULE) != schedule:
        raise ValueError("V8 prepared provenance drifted")
    if _read_journal(work)[:2] != [{"event": "admitted-prefix", **admission}, {"event": "adopted-v7-output", "sequence": 182, "settlement_sha256": admission["settlement_sha256"]}]:
        raise ValueError("V8 admission journal drifted")
    ...
    if unexpected:
        raise ValueError(f"V8 work root contains unexpected entries: {sorted(unexpected)}")
...
def _dispatch_event(
    ...
    if not isinstance(cells, list) or disclosure.get("profile") != contract()["provider"]:
        raise ValueError("V8 dispatch disclosure is malformed")
    ...
    if len(matches) != 1:
        raise ValueError("V8 dispatch event is not uniquely bound to its full disclosure cell")
    ...
    if _scope_override_value(source, event, frozen) is not None:
        ...
        if read_json(override_path) != _scope_override_value(source, event, frozen):
            raise ValueError("V8 scope compatibility override changed before dispatch")
...
def _validate_contact_sessions(source: Path, work: Path, admission: Mapping[str, Any], events: list[Mapping[str, Any]]) -> None:
    """Validate only output-backed contact cells; zero-contact 181 has no session by design."""
    registry = admission.get("session_registry")
    if not isinstance(registry, Mapping) or registry.get("excluded_zero_contact_sequence") != 181 or not isinstance(registry.get("ids"), list) or not all(isinstance(item, str) for item in registry["ids"]) or registry.get("count") != len(registry["ids"]) or registry.get("sha256") != hashlib.sha256(canonical(registry["ids"])).hexdigest() or len(registry["ids"]) != len(set(registry["ids"])):
        raise ValueError("V8 historical session registry is malformed")
    sessions = set(registry["ids"])
    admitted = admission.get("session_bearing")
    if not isinstance(admitted, list) or len(admitted) != 1:
        raise ValueError("V8 admission lacks its one session-bearing V7 output")
    record = admitted[0]
    if not isinstance(record, Mapping) or record.get("sequence") != 182 or record.get("session_id") != EXPECTED_V7_SESSION or record.get("recorded_provider_contacts") != 1:
        raise ValueError("V8 adopted V7 session record drifted")
    if EXPECTED_V7_SESSION not in sessions:
        raise ValueError("V8 historical session registry omits adopted sequence 182")
    for event in events:
        if event.get("sequence") == 182:
            continue
        output = _output_path(work, event).parent
        for session in _output_sessions(output, event):
            if session in sessions:
                raise ValueError("Provider session collides with predecessor, adopted V7, or prior V8 output")
...
def _settle_one(
    ...
    try:
        ...
        def before_provider_attempt(context: Mapping[str, Any]) -> None:
            ...
            if attempt["number"] == 1:
                if current_pause is not None:
                    raise ValueError("Paused V8 retry unexpectedly changed back to attempt one")
        ...
        _validate_contact_sessions(source, work, admission, [*accepted, event])
        target = _output_path(work, event)
...
def execute(source_root: Path, closed_root: Path, v7_root: Path, work_root: Path, capacity_evidence: Path | None = None, *, timeout: float = 3600.0, allow_remote: bool = False, dry_run: bool = False, disclosure_ack: Path | None = None, retry_disclosure_ack: Path | None = None) -> dict[str, Any]:
    work = _external(work_root, allow_missing_leaf=True)
    if not work.exists():
        prepare(source_root, closed_root, v7_root, work)
    if dry_run and not any(work.iterdir()):
        prepare(source_root, closed_root, v7_root, work)
    binding, schedule, admission = _verify_prepared(source_root, closed_root, v7_root, work)
    accepted = _accepted(work, schedule, admission)
    remaining = schedule[len(accepted):]
    if dry_run:
        accounting = _accounting(schedule, accepted)
        return {"provider_calls": 0, "admitted_sequence": 182, "adopted_sequence": 182, "completed": len(accepted), "cells": len(remaining), "first_sequence": remaining[0]["sequence"] if remaining else None, "last_sequence": remaining[-1]["sequence"] if remaining else None, "prepared": True, "disclosure_path": str(work / DISCLOSURE), "disclosure_sha256": sha(work / DISCLOSURE), "destination": "codex", "profile": contract()["provider"], "accounting": accounting}
    if not allow_remote:
    ...
    while remaining:
        try:
            ...
            if pause.get("event") != "retry-disclosure-pause" or pause.get("sequence") != remaining[0]["sequence"]:
                raise ValueError("Runner reported a retry pause without the required durable V8 pause evidence")
            return _retry_pause_result(work, schedule, accepted, remaining, pause)
        except ValueError as exc:
...
def main() -> None:
    ...
    parser.add_argument("--v7-root", type=Path, default=V7_DEFAULT)
    ...
    result = execute(args.source_root, args.closed_root, args.v7_root, args.work_root, args.capacity_evidence, timeout=args.timeout, allow_remote=args.allow_remote, dry_run=args.dry_run or args.preview_disclosure, disclosure_ack=args.disclosure_ack, retry_disclosure_ack=args.retry_disclosure_ack)
