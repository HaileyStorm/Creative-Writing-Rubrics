"""Prospective v6 continuation with a serialized, source-bound candidate loader.

This replaces the frozen ``baseline_grok_standing_v6_continuation.py`` for
future work only.  The predecessor remains immutable provenance for the two
consumed pre-contact waves.
"""
...
_CANDIDATE_LOAD_LOCK = threading.RLock()
_CANDIDATE_CODE_CACHE: dict[tuple[str, str], ModuleType] = {}
...
def _candidate_manifest(root: Path, expected: str) -> tuple[Path, dict[str, Any], dict[str, str]]:
    root = root.resolve(); manifest_path = root / "candidate-manifest.json"; raw = manifest_path.read_bytes(); value = _json(manifest_path, "candidate manifest")
    ...
    return root, value, expected_files


def _candidate(root: Path, expected: str) -> tuple[Any, dict[str, Any]]:
    """Return only fully initialized candidate code after every-call source checks."""
    with _CANDIDATE_LOAD_LOCK:
        root, manifest, files = _candidate_manifest(root, expected)
        broker_path = root / "model_work_queue" / "broker.py"; broker_sha = files.get("model_work_queue/broker.py")
        _need(isinstance(broker_sha, str), "candidate broker missing")
        key = (str(root), expected)
        # Every cache hit has just verified the complete source inventory.
        module = _CANDIDATE_CODE_CACHE.get(key)
        if module is not None:
            _need(Path(str(getattr(module, "__file__", ""))).resolve() == broker_path and hasattr(module, "Broker"), "candidate cached code differs")
            _need(_sha(broker_path.read_bytes()) == broker_sha, "candidate broker changed")
            return module, manifest
        package = root / "model_work_queue" / "__init__.py"
        package_name = "_standing_v6_candidate_" + _sha(_canon([str(root), expected]))[:24]
        ...
        try:
            ...
            if isinstance(existing, ModuleType) and Path(str(getattr(existing, "__file__", ""))).resolve() == broker_path and hasattr(existing, "Broker"):
                module = existing
            else:
                _need(existing is None, "candidate broker load is incomplete")
                package_existing = sys.modules.get(package_name)
                if package_existing is None:
                    spec = importlib.util.spec_from_file_location(package_name, package, submodule_search_locations=[str(package.parent)])
                    _need(spec is not None and spec.loader is not None, "candidate package load")
                    package_module = importlib.util.module_from_spec(spec); sys.modules[package_name] = package_module
                    spec.loader.exec_module(package_module)
                else:
                    _need(isinstance(package_existing, ModuleType) and Path(str(getattr(package_existing, "__file__", ""))).resolve() == package, "candidate package load is incomplete")
            ...
            _CANDIDATE_CODE_CACHE[key] = module
        except BaseException:
            if package_module is not None and sys.modules.get(package_name) is package_module:
                sys.modules.pop(package_name, None)
            if package_module is not None:
                broker_module = sys.modules.get(broker_name)
                if getattr(broker_module, "__package__", None) == package_name: sys.modules.pop(broker_name, None)
            raise
        finally:
...
def _packet(path: Path, expected: str, standing_path: Path, standing_expected: str) -> dict[str, Any]:
    ...
    actions = value["actions"]; _need(actions.get("activation_authority") is False and actions.get("provider_contact_authority") is False and actions.get("provider_contact_count") == 0, "v6 packet is not inert")
    ...
    for relative, digest in files.items(): _need(_sha((path.parent / relative).read_bytes()) == digest, "renewal packet evidence drift")
    predecessor = value["predecessor_packet"]; _need(predecessor.get("sha256") == "585e9ce443cc9302b606f6c945a13b95b035a389fea89e701c20c15ba7793e2c" and _sha(Path(predecessor["path"]).read_bytes()) == predecessor["sha256"], "renewal predecessor differs")
    renewed = value["renewed_source"]; _need(renewed.get("sha256") == standing_expected and renewed.get("fresh_allowance_observation") is False and renewed.get("new_owner_statement") is False and renewed.get("authority_at") == "2026-09-09T19:00:18.842739+00:00", "standing renewal authority differs")
    return value
...
def verify_standing_v6_continuation(*, continuation_root: Path, expected_manifest_sha256: str, expected_controller_sha256: str) -> dict[str, Any]:
    ...
    _candidate(Path(manifest["candidate"]["root"]), manifest["candidate"]["manifest_sha256"]); _packet(Path(manifest["packet"]["path"]), manifest["packet"]["sha256"], Path(manifest["standing_source"]["path"]), manifest["standing_source"]["sha256"]); standing = _standing(Path(manifest["standing_source"]["path"]), manifest["standing_source"]["sha256"])
    _e33, prefix = _prefix(manifest["prefix"]["descriptor"]); _need(prefix["source"] == manifest["prefix"]["source"] and prefix["protected_paths"] == manifest["prefix"]["protected_paths"], "v5/local prefix changed")
...
def _prefix_context(manifest: Mapping[str, Any]) -> tuple[Any, Any, Path, dict[int, Any], Mapping[str, Any], list[dict[str, str]]]:
    e33, _prefix_value = _prefix(manifest["prefix"]["descriptor"]); prefix_manifest, _raw = e33._manifest(Path(manifest["prefix"]["descriptor"]["root"]), manifest["prefix"]["descriptor"]["manifest_sha256"])
    _successor, parent, _source, _epoch_raw, plan_root, requests, partial = e33._source_guard(prefix_manifest); epoch, _raw_epoch = parent._load_epoch(Path(prefix_manifest["successor_root"]), prefix_manifest["inner_epoch"]["sha256"])
...
def _replayed(root: Path, manifest: Mapping[str, Any], manifest_raw: bytes) -> tuple[set[int], list[dict[str, Any]]]:
    ...
    for path in sorted((root / "replays").glob("wave-*-slots-*.json")) if (root / "replays").exists() else []:
        value = _json(path, "v6 replay"); required = {"schema_version", "evidence_class", "controller_sha256", "manifest_sha256", "candidate_manifest_sha256", "packet_sha256", "standing_source_sha256", "ordinals", "terminals", "native_identities", "provider_calls_made"}; current = value.get("ordinals")
...
def _semantic_replay_terminal(*, candidate: Any, broker: Any, terminal: Mapping[str, Any], terminal_path: Path, parent: Any, runtime: Any, plan_root: Path, passes: Mapping[str, Any], requests: Mapping[int, Any]) -> tuple[dict[str, Any], dict[str, Any], Mapping[str, Any]]:
    result = terminal.get("candidate_result"); _need(terminal.get("state") == "completed" and isinstance(result, Mapping), "v6 terminal differs")
    descriptor = result.get("native_envelope_artifact"); _need(isinstance(descriptor, Mapping), "v6 envelope descriptor differs")
    envelope = broker.read_grok_native_envelope(dict(descriptor)); _need(descriptor.get("sha256") == _sha(envelope) and descriptor.get("byte_length") == len(envelope), "v6 candidate envelope differs")
    ordinal = terminal.get("ordinal"); _need(type(ordinal) is int and ordinal in requests, "v6 terminal ordinal differs")
    row, source = requests[ordinal], parent._source_for_pass(plan_root, passes[requests[ordinal]["pass_id"]]); prompt, schema_path, _ids = parent._request_payload(plan_root, row); schema = json.loads(schema_path.read_bytes())
    ...
    output, identity, _usage = adapter._parse_grok_envelope(envelope, model=route["model"], reported_model=route["reported_model"], session_id=session_id, schema=schema, max_turns=1, exact_turns=True); native_identity = _native_identity(identity, "v6 candidate native identity")
...
def dispatch_standing_v6_wave(*, continuation_root: Path, start_ordinal: int, wave_size: int, arming_review_path: Path, expected_arming_review_sha256: str) -> dict[str, Any]:
    root = Path(continuation_root).resolve(); manifest, manifest_raw = _manifest(root); _need(type(start_ordinal) is int and type(wave_size) is int and 1 <= wave_size <= MAX_WAVE and start_ordinal in PENDING, "v6 dispatch geometry differs")
    ...
    review = _review(Path(arming_review_path), expected_arming_review_sha256, manifest, manifest_raw); _e33, parent, plan_root, requests, epoch, protected_ids = _prefix_context(manifest); runtime = parent._runtime_from_epoch(epoch); records, replayed, old_ids = _attempts(root), *_replayed(root, manifest, manifest_raw)
    ordinals = PENDING[PENDING.index(start_ordinal):PENDING.index(start_ordinal) + wave_size]; _need(len(ordinals) == wave_size and ordinals[0] == _next(records, replayed) and not any(_attempt_path(root, ordinal, "attempt-start.json").exists() for ordinal in ordinals), "v6 wave is not exact next pending")
    plan = parent._plan(plan_root, epoch["plan_sha256"])[0]; passes = parent._pass_index(plan); prepared: list[dict[str, Any]] = []
    for index, ordinal in enumerate(ordinals):
        row = requests[ordinal]; prompt, schema_path, question_ids = parent._request_payload(plan_root, row); source = parent._source_for_pass(plan_root, passes[row["pass_id"]]); schema = json.loads(schema_path.read_bytes())
    ...
    def cell(item: Mapping[str, Any]) -> None:
        ordinal, admitted, session_id, outcome, result, identity, callback_failure = item["ordinal"], False, None, None, None, None, None
        try:
            ...
            if state != "completed" or not isinstance(result, Mapping) or not admitted:
                with lock: stop.set()
                terminal = {"schema_version": 1, "ordinal": ordinal, "state": state if state in {"definitely_not_contacted", "ambiguous"} else "ambiguous", "attempt_start_sha256": _sha(item["start_path"].read_bytes()), "session_id": session_id, "broker_outcome": outcome, "contact_admitted": admitted}
        ...
        except Exception as error:  # noqa: BLE001 - every local cell failure must become terminal no-resend evidence
            with lock: stop.set()
    ...
    completed_provider_outcomes = 0; ambiguous_contact_ordinals: list[int] = []
    for ordinal in ordinals:
        terminal = _json(_attempt_path(root, ordinal, "terminal.json"), "v6 terminal"); outcome = terminal.get("broker_outcome")
        if isinstance(outcome, Mapping) and outcome.get("state") == "completed": completed_provider_outcomes += 1
        elif terminal.get("state") != "definitely_not_contacted": ambiguous_contact_ordinals.append(ordinal)
...
def replay_standing_v6_wave(*, continuation_root: Path, start_ordinal: int, wave_size: int) -> dict[str, Any]:
    ...
    candidate, _candidate_manifest = _candidate(Path(manifest["candidate"]["root"]), manifest["candidate"]["manifest_sha256"]); _packet(Path(manifest["packet"]["path"]), manifest["packet"]["sha256"], Path(manifest["standing_source"]["path"]), manifest["standing_source"]["sha256"]); _standing(Path(manifest["standing_source"]["path"]), manifest["standing_source"]["sha256"])
    _e33, parent, plan_root, requests, epoch, protected_ids = _prefix_context(manifest); runtime = parent._runtime_from_epoch(epoch); plan = parent._plan(plan_root, epoch["plan_sha256"])[0]; passes = parent._pass_index(plan); ordinals = PENDING[PENDING.index(start_ordinal):PENDING.index(start_ordinal) + wave_size]; replayed, old_ids = _replayed(root, manifest, manifest_raw); _need(not (set(ordinals) & replayed), "v6 replay already exists")
    broker, identities, terminals = candidate.Broker(Path(manifest["queue"]["path"])), [*protected_ids, *old_ids], []
    for ordinal in ordinals:
        terminal_path = _attempt_path(root, ordinal, "terminal.json"); terminal = _json(terminal_path, "v6 terminal"); _need(terminal.get("candidate_manifest_sha256") == manifest["candidate"]["manifest_sha256"], "v6 terminal differs")
        identity, _normalized, descriptor = _semantic_replay_terminal(candidate=candidate, broker=broker, terminal=terminal, terminal_path=terminal_path, parent=parent, runtime=runtime, plan_root=plan_root, passes=passes, requests=requests); _need(identity not in identities, "v6 terminal semantic replay differs"); identities.append(identity); terminals.append({"ordinal": ordinal, "terminal_sha256": _sha(terminal_path.read_bytes()), "native_envelope_sha256": descriptor["sha256"], "native_identity": identity})
...
def verify_standing_v6_replay_chain(*, continuation_root: Path, expected_manifest_sha256: str, expected_controller_sha256: str) -> dict[str, Any]:
    ...
    for path in sorted((root / "replays").glob("wave-*-slots-*.json")) if (root / "replays").exists() else []:
        ...
        for ordinal in record["ordinals"]:
            ...
            identity, normalized, _descriptor = _semantic_replay_terminal(candidate=candidate, broker=broker, terminal=terminal, terminal_path=terminal_path, parent=parent, runtime=runtime, plan_root=plan_root, passes=passes, requests=requests); terminal_hash = _sha(terminal_path.read_bytes()); terminals.append({"ordinal": ordinal, "terminal_sha256": terminal_hash, "native_identity": _public_identity(identity)}); verdicts_by_ordinal[ordinal] = {"terminal_sha256": terminal_hash, "question_ids": list(requests[ordinal]["question_ids"]), "verdicts": normalized}
