...
STUDY_ID = "hbq-human-alignment-optimizer-v5-f20-nextwave-sol6-validation-exec-v4-threadsafe-route-load"
...
PREDECESSOR_COMMIT = "b231d29001fef3b4df5771a7298efa21f3860da6"
PREDECESSOR_FILES = {
    "evaluation-results/hbq-human-alignment-optimizer-v5-f20-nextwave-sol6-validation-exec-v3-offset-timestamps/README.md": "b6aac264e1f915ace7435108766d01e8d434dc1d08dceebf39c01bcad5924c7c",
    "evaluation-results/hbq-human-alignment-optimizer-v5-f20-nextwave-sol6-validation-exec-v3-offset-timestamps/executor.py": "1a879aa45e24c993d7beaa4bb0cf2eb62a5e6fc244ce48ba7ca576818a7366d8",
    "evaluation-results/hbq-human-alignment-optimizer-v5-f20-nextwave-sol6-validation-exec-v3-offset-timestamps/study-contract.json": "822f8dd1cdb0d794f6c7af990535e8e63e26ec790a0ddf69945d1adacd60c27c",
    "tests/test_hbq_human_alignment_optimizer_v5_f20_nextwave_sol6_validation_exec_v3_offset_timestamps.py": "6a34c5ae4aad3d64801bab83911447015f8da9395e3ae5dca3c7e705078ca21a",
}
_ROUTE_KEYS = frozenset({"name", "model", "adapter", "provider", "destination", "account_class", "zero_charge", "armed", "health", "reasoning_effort", "identity_evidence", "trusted", "allowed_payload_classes", "codex_command", "codex_command_identity", "cli_version_command", "cli_version_identity", "auth_status_command", "auth_status_identity", "codex_cli_version", "command", "command_identity", "cost_evidence", "auth_receipt_hash", "timeout_seconds", "capabilities", "cwd_policy", "intelligence", "output_schema", "priority"})
...
_CAPABILITIES = ["public_repo", "public_synthetic", "json_object", "identity_requested_only"]
_OUTPUT_SCHEMA = {"$schema_version": 1, "additionalProperties": False, "properties": {"findings": {"items": {"type": "string"}, "type": "array"}, "recommendations": {"items": {"type": "string"}, "type": "array"}, "risks": {"items": {"type": "string"}, "type": "array"}, "summary": {"type": "string"}}, "required": ["summary", "findings", "risks", "recommendations"], "type": "object"}
...
def _verify_predecessor() -> None:
    repo = HERE.parents[1]
    for relative, digest in PREDECESSOR_FILES.items():
        raw = stable(repo / relative)
        blob = subprocess.run(["git", "-C", str(repo), "show", f"{PREDECESSOR_COMMIT}:{relative}"], capture_output=True, check=False)
        if sha256(raw) != digest or blob.returncode != 0 or blob.stdout != raw:
            raise ValueError("pinned V3 predecessor drifted")
...
def _configure(base: ModuleType, rows: tuple[dict[str, Any], ...], source_schedule: Mapping[str, Any], bindings: Mapping[str, str]) -> None:
    with _configuration_lock:
        if getattr(base, "_v4_configured", False):
            return
        ...
        def serialized_route(*args: Any, **kwargs: Any):
            with _route_load_lock:
                return original_route(*args, **kwargs)
        base._route = serialized_route
...
def _timestamp(value: Any) -> datetime:
    if type(value) is not str or not re.fullmatch(r"\d{4}-\d{2}-\d{2}T(?:[01]\d|2[0-3]):[0-5]\d:[0-5]\d(?:\.\d+)?(?:Z|[+-](?:[01]\d|2[0-3]):[0-5]\d)", value):
...
def _identity(value: Any, *, artifacts: int) -> bool:
    ...
    return (isinstance(items, list) and len(items) == artifacts and all(isinstance(item, Mapping) and set(item) == {"index", "path_hash", "sha256"}
            and type(item["index"]) is int and item["index"] == index and _hex(item["path_hash"]) and _hex(item["sha256"])
            for index, item in enumerate(items)))


def _command(value: Any, *, length: int) -> bool:
    return isinstance(value, list) and len(value) == length and all(isinstance(item, str) and item for item in value)


def _exact_json(value: Any, expected: Any) -> bool:
    if type(expected) is dict:
        return type(value) is dict and set(value) == set(expected) and all(_exact_json(value[key], item) for key, item in expected.items())
    if type(expected) is list:
        return type(value) is list and len(value) == len(expected) and all(_exact_json(item, target) for item, target in zip(value, expected))
...
def _frozen_route(route: Any, evidence: Any, v3: ModuleType, *, require_unexpired: bool) -> tuple[dict[str, Any], dict[str, Any]]:
    ...
    required = {"name": "codex-chatgpt-gpt-5.6-sol", "model": "gpt-5.6-sol", "adapter": "codex_exec", "provider": "openai_codex", "destination": "openai_codex_chatgpt_subscription", "account_class": "subscription", "zero_charge": True, "armed": True, "health": "healthy", "reasoning_effort": "high", "identity_evidence": "requested_only", "trusted": True, "allowed_payload_classes": ["public_repo", "public_synthetic"], "capabilities": _CAPABILITIES, "cwd_policy": "broker_root", "intelligence": 100, "output_schema": _OUTPUT_SCHEMA, "priority": 0}
    if any(not _exact_json(route.get(key), expected) for key, expected in required.items()):
        raise ValueError("frozen route semantics drifted")
    if (not _command(route["codex_command"], length=1) or not _command(route["command"], length=2)
            ...
            or route["cli_version_command"] != [route["codex_command"][0], "--version"]
            or route["auth_status_command"] != [route["codex_command"][0], "login", "status"]
            or not _identity(route["command_identity"], artifacts=2)
            or not all(_identity(route[key], artifacts=1) for key in ("codex_command_identity", "cli_version_identity", "auth_status_identity"))):
        raise ValueError("frozen route command identity drifted")
    cost = route["cost_evidence"]
    if (not isinstance(cost, Mapping) or set(cost) != {"allowance_state", "checked_at", "evidence_hash", "expires_at", "kind", "version"}
            or cost["allowance_state"] != "available" or cost["kind"] != "subscription_included" or type(cost["version"]) is not int or cost["version"] != 1
            or not _hex(cost["evidence_hash"])):
...
def execute_wave(*, cell_ids: list[str], **kwargs: Any) -> list[dict[str, Any]]:
    ...
    _configure(_base, rows, _schedule_value, _bindings)
