...
def _freeze_module(expected_sha256: str) -> ModuleType:
    return _load_exact(FREEZE, expected_sha256, "_wpb_grok_selection_freeze")


def _full_freeze(freeze_path: Path, expected_sha256: str, expected_verifier_sha256: str, *, replay_native: bool) -> dict[str, Any]:
    module = _freeze_module(expected_verifier_sha256)
    verify = getattr(module, "verify_freeze", None)
    ...
    required = {"freeze_sha256", "selection_frozen_at", "selected_profile", "schedule_sha256", "source_bindings", "evidence_files", "native_measurement_count"}
    _require(set(value) == required and value["freeze_sha256"] == expected_sha256 and value["native_measurement_count"] == 129
             and isinstance(value["selected_profile"], Mapping) and isinstance(value["source_bindings"], Mapping)
             and isinstance(value["evidence_files"], Mapping), "WPB Grok selection freeze differs")
...
def _campaign(root: Path, resolution: Mapping[str, Any], freeze: Mapping[str, Any], expected_freeze_sha256: str,
              expected_freeze_verifier_sha256: str) -> tuple[dict[str, Any], str]:
...
def _freeze_review(path: Path, expected_sha256: str, freeze_sha256: str, freeze_verifier_sha256: str) -> str:
...
def create_campaign(*, campaign_root: Path, queue_root: Path, freeze_root: Path, freeze_path: Path,
                    ...
                    independent_review_path: Path, expected_independent_review_sha256: str) -> dict[str, Any]:
    ...
    context = _full_freeze(Path(freeze_path), expected_freeze_sha256, expected_freeze_verifier_sha256, replay_native=True)
    ...
    review_sha256 = _freeze_review(Path(independent_review_path), expected_independent_review_sha256,
                                   expected_freeze_sha256, expected_freeze_verifier_sha256)
...
def _review(path: Path, expected_sha256: str, *, campaign_sha256: str, batch_number: int,
            cell_ids: list[str], freeze_sha256: str, route_sha256: str) -> tuple[dict[str, Any], str]:
    ...
    required = {"format_version", "kind", "decision", "campaign_sha256", "batch_number", "cell_ids", "freeze_sha256", "route_sha256", "reviewed_at", "expires_at"}
...
def _review_companion(batch_root: Path, plan: Mapping[str, Any], *, require_unexpired: bool) -> None:
    ...
    expected = {"format_version", "kind", "decision", "campaign_sha256", "batch_number", "cell_ids",
                "freeze_sha256", "route_sha256", "reviewed_at", "expires_at"}
...
def prepare_next_batch(*, campaign_root: Path, queue_root: Path, freeze_root: Path, freeze_path: Path,
                       ...
                       authorization_acknowledgement_sha256: str, broker_factory: Callable[[Path], Any] | None = None) -> dict[str, Any]:
    ...
    context = _full_freeze(Path(freeze_path), expected_freeze_sha256, expected_freeze_verifier_sha256, replay_native=False)
    _require(context["schedule_sha256"] == resolution["schedule_sha256"], "Grok freeze schedule differs")
    _campaign_value, campaign_sha256 = _campaign(root, resolution, context, expected_freeze_sha256, expected_freeze_verifier_sha256)
    ...
    plan = {"format_version": 1, "kind": "wpb_sol_batched_plan", "study_id": STUDY_ID, "campaign_sha256": campaign_sha256,
            ...
            "freeze_verifier_sha256": expected_freeze_verifier_sha256,
            "selection_frozen_at": context["selection_frozen_at"], "schedule_sha256": resolution["schedule_sha256"],
            "freeze_source_bindings_sha256": sha256(context["source_bindings"]), "freeze_evidence_files_sha256": sha256(context["evidence_files"]),
...
def _plan(root: Path, number: int, campaign_sha256: str) -> tuple[dict[str, Any], str]:
    ...
    required = {"format_version", "kind", "study_id", "campaign_sha256", "batch_number", "cell_ids", "freeze_sha256", "freeze_verifier_sha256",
                "selection_frozen_at", "schedule_sha256", "freeze_source_bindings_sha256", "freeze_evidence_files_sha256",
...
def _cheap_freeze(plan: Mapping[str, Any], freeze_path: Path, expected_freeze_sha256: str, expected_freeze_verifier_sha256: str) -> dict[str, Any]:
    context = _full_freeze(freeze_path, expected_freeze_sha256, expected_freeze_verifier_sha256, replay_native=False)
    _require(plan.get("freeze_sha256") == context["freeze_sha256"] and plan.get("selection_frozen_at") == context["selection_frozen_at"]
             ...
             and plan.get("freeze_evidence_files_sha256") == sha256(context["evidence_files"]), "WPB Sol frozen Grok binding drifted")
...
def dispatch_batch(*, campaign_root: Path, queue_root: Path, freeze_root: Path, freeze_path: Path,
                   ...
                   call_codex: Callable[..., tuple[str, dict[str, Any]]] | None = None) -> list[dict[str, Any]]:
    ...
    _campaign_value, campaign_sha256 = _campaign(root, resolution, context, expected_freeze_sha256, expected_freeze_verifier_sha256)
    _require(not (root / "stopped.json").exists(), "WPB Sol campaign is stopped")
    plan, _plan_sha256 = _plan(root, batch_number, campaign_sha256)
...
def settle_batch(*, campaign_root: Path, freeze_root: Path, freeze_path: Path, expected_freeze_sha256: str,
                 expected_freeze_verifier_sha256: str, batch_number: int) -> dict[str, Any]:
    """Re-admit one route epoch before it can authorize any receipt-only renewal."""
    root = Path(campaign_root).resolve(); frozen = _frozen(); resolution = _resolution(frozen, Path(freeze_root))
    context = _full_freeze(Path(freeze_path), expected_freeze_sha256, expected_freeze_verifier_sha256, replay_native=False)
    _campaign(root, resolution, context, expected_freeze_sha256, expected_freeze_verifier_sha256)
...
def report(*, campaign_root: Path, freeze_root: Path, freeze_path: Path, expected_freeze_sha256: str,
           expected_freeze_verifier_sha256: str) -> dict[str, Any]:
    ...
    for number in numbers:
        plan, plan_sha256 = _plan(root, number, campaign_sha256)
        _cheap_freeze(plan, Path(freeze_path), expected_freeze_sha256, expected_freeze_verifier_sha256)
