...
STUDY_ID = "cwr-guided-revision-gain-v2-live-exec-v4"
...
_PREDECESSOR_DERIVED_FILES = frozenset({"source-lineage.json", "reconciliation.json", "reconciled-verified-receipt.json"})
...
def _derived_predecessor_root(output_root: Path, event_id: str) -> Path:
    return Path(output_root) / "predecessor-reconciliation" / "revision_generation" / event_id


def _validate_derived_output_root(output_root: Path) -> Path:
    candidate = Path(os.path.abspath(output_root))
    source = Path(os.path.abspath(V3_TERMINAL_RUN_ROOT))
    candidate_text, source_text = os.path.normcase(str(candidate)), os.path.normcase(str(source))
    try:
        if os.path.commonpath((candidate_text, source_text)) in {candidate_text, source_text}:
            raise ValueError("Revision-gain live v4 derived output root overlaps the immutable v3 run root")
    except ValueError as error:
        if "overlaps" in str(error):
            raise
        raise ValueError("Revision-gain live v4 derived output root is on a different drive") from error
    ...
    for part in candidate.parts[1:]:
        ...
        try:
            ...
            break
        if stat.S_ISLNK(info.st_mode) or bool(getattr(info, "st_file_attributes", 0) & 0x400):
            raise ValueError("Revision-gain live v4 derived output root has unsafe or reparsed ancestry")
        if not stat.S_ISDIR(info.st_mode):
            raise ValueError("Revision-gain live v4 derived output root ancestry is not a directory")
    return candidate


def _predecessor_reconciliation_identity(output_root: Path, event_id: str) -> dict[str, str]:
    token = canonical({"study_id": STUDY_ID, "output_root": str(Path(output_root).resolve()), "phase": "revision_generation", "source_event_id": event_id})
    return {"reconciliation_event_id": f"exec-v4-reconcile-{_sha256(token)[:24]}", "logical_sample_id": _sha256(token)}


def _validate_derived_predecessor_inventory(root: Path, *, ingested: bool) -> None:
    root = Path(os.path.abspath(root))
    try:
        info = os.lstat(root)
    except FileNotFoundError as error:
        raise ValueError("Revision-gain live v4 derived predecessor root is missing") from error
    if not stat.S_ISDIR(info.st_mode) or stat.S_ISLNK(info.st_mode) or bool(getattr(info, "st_file_attributes", 0) & 0x400):
        raise ValueError("Revision-gain live v4 derived predecessor root is unsafe")
    expected = _PREDECESSOR_DERIVED_FILES | (frozenset({"revision-record.json"}) if ingested else frozenset())
    children = list(root.iterdir())
    if frozenset(child.name for child in children) != expected:
        raise ValueError("Revision-gain live v4 derived predecessor inventory drifted")
    for child in children:
        info = os.lstat(child)
        if not stat.S_ISREG(info.st_mode) or stat.S_ISLNK(info.st_mode) or bool(getattr(info, "st_file_attributes", 0) & 0x400):
            raise ValueError("Revision-gain live v4 derived predecessor inventory contains a non-plain artifact")
        _stable_bytes(child, label="derived predecessor artifact")
...
def prepare_one(*, run_root: Path, source_root: Path, phase: str, event_id: str, acknowledgement_sha256: str, queue_root: Path = DEFAULT_QUEUE_ROOT, lineage_records: list[Mapping[str, Any]] | None = None, feedback_receipt_path: Path | None = None, completed_feedback_receipt_paths: list[Path] | None = None, target_root: Path | None = None, target_manifest_path: Path | None = None) -> dict[str, Any]:
    ...
    if run_root.resolve() == V3_TERMINAL_RUN_ROOT.resolve():
        raise ValueError("Revision-gain live v4 refuses the immutable v3 terminal run root")
...
def reconcile_predecessor_terminal(*, output_root: Path, event_id: str, reconciliation_acknowledgement_sha256: str) -> dict[str, Any]:
    """Derive one v4 receipt authority from immutable v3 postlaunch evidence."""
    output_root = _validate_derived_output_root(output_root)
    if reconciliation_acknowledgement_sha256 != contract()["authorized_acknowledgement_sha256"]:
        raise ValueError("Revision-gain live v4 predecessor reconciliation acknowledgement is invalid")
    _validate_predecessor_lineage(contract()["predecessor"])
    source_root = _predecessor_root(event_id)
    derived_root = _derived_predecessor_root(output_root, event_id)
    if derived_root.exists():
        raise ValueError("Revision-gain live v4 predecessor reconciliation cannot duplicate or remint receipt authority")
    prepared = json.loads(_stable_bytes(source_root / "prepared-cell.json", label="pinned v3 prepared cell").decode("utf-8"))
    if not isinstance(prepared, Mapping) or prepared.get("provider_model") != "grok-4.6" or prepared.get("phase") != "revision_generation" or prepared.get("event_id") != event_id:
        raise ValueError("Revision-gain live v4 predecessor cell identity drifted")
    pilot = _pilot()
    binding = _commitment(V3_TERMINAL_RUN_ROOT, source_root / "governed-route-proof.json")
    _broker, route = _reauth_route(pilot=pilot, run_root=V3_TERMINAL_RUN_ROOT, phase="revision_generation", event_id=event_id, binding=binding)
    raw = _stable_bytes(source_root / "adapter-stdout.raw", label="pinned v3 raw stdout")
    source_outbound = _stable_bytes(source_root / "outbound-payload.json", label="pinned v3 outbound payload")
    native = _receipt_from_control(pilot=pilot, root=source_root, prepared=prepared, route=route, control_raw=raw, payload_override=source_outbound)
    lineage = _predecessor_inventory(event_id)
    identity = _predecessor_reconciliation_identity(output_root, event_id)
    reconciliation = {
        "format_version": 1,
        "study_id": STUDY_ID,
        "kind": "derived_postlaunch_receipt_reconciliation",
        "source_lineage_sha256": _sha256(canonical(lineage)),
        "identity": identity,
        "acknowledgement_sha256": reconciliation_acknowledgement_sha256,
        "action": "accept_settled_native_receipt_without_resend",
        "provider_calls_made": 0,
        "process_launches": 0,
    }
    _state, result = _control_from_adapter(raw)
    assert result is not None
    runtime = result["runtime"]
    receipt = {
        "format_version": 1,
        "study_id": STUDY_ID,
        "kind": "derived_reconciled_predecessor_grok_native_receipt",
        "source_lineage_sha256": _sha256(canonical(lineage)),
        "reconciliation_sha256": _sha256(canonical(reconciliation)),
        "phase": "revision_generation",
        "event_id": event_id,
        "identity": identity,
        "provider_calls_made": 0,
        "process_launches": 0,
        "provider_model": native["provider_model"],
        "reasoning": native["reasoning"],
        "tools_enabled": False,
        "runtime_command_identity": dict(runtime["command_identity"]),
        "observed_command_identity_hash": runtime["command_identity_hash"],
        "native_receipt": native,
        "response": native["response"],
    }
    derived_root.mkdir(parents=True, exist_ok=False)
    _write_once(derived_root / "source-lineage.json", lineage)
    _write_once(derived_root / "reconciliation.json", reconciliation)
    _write_once(derived_root / "reconciled-verified-receipt.json", receipt)
    _validate_derived_predecessor_inventory(derived_root, ingested=False)
    return {
        "study_id": STUDY_ID,
        "kind": "predecessor_reconciliation_only_receipt_ingest",
        "phase": "revision_generation",
        "event_id": event_id,
        "provider_calls_made": 0,
        "process_launches": 0,
        "derived_root": str(derived_root.resolve()),
        "verified_receipt": _commitment(output_root, derived_root / "reconciled-verified-receipt.json"),
        "receipt_authority": native["provider_request_id"],
    }


def _read_derived_predecessor_receipt(*, output_root: Path, event_id: str) -> tuple[Path, dict[str, Any]]:
    output_root = _validate_derived_output_root(output_root)
    _validate_predecessor_lineage(contract()["predecessor"])
    root = _derived_predecessor_root(output_root, event_id)
    _validate_derived_predecessor_inventory(root, ingested=(root / "revision-record.json").exists())
    lineage_raw = _stable_bytes(root / "source-lineage.json", label="derived predecessor lineage")
    reconciliation_raw = _stable_bytes(root / "reconciliation.json", label="derived predecessor reconciliation")
    receipt_raw = _stable_bytes(root / "reconciled-verified-receipt.json", label="derived predecessor receipt")
    lineage = json.loads(lineage_raw.decode("utf-8"))
    reconciliation = json.loads(reconciliation_raw.decode("utf-8"))
    receipt = json.loads(receipt_raw.decode("utf-8"))
    expected_lineage = _predecessor_inventory(event_id)
    identity = _predecessor_reconciliation_identity(output_root, event_id)
    expected_reconciliation = {"format_version": 1, "study_id": STUDY_ID, "kind": "derived_postlaunch_receipt_reconciliation", "source_lineage_sha256": _sha256(canonical(expected_lineage)), "identity": identity, "acknowledgement_sha256": contract()["authorized_acknowledgement_sha256"], "action": "accept_settled_native_receipt_without_resend", "provider_calls_made": 0, "process_launches": 0}
    if canonical(lineage) + b"\n" != lineage_raw or lineage != expected_lineage or canonical(reconciliation) + b"\n" != reconciliation_raw or reconciliation != expected_reconciliation:
        raise ValueError("Revision-gain live v4 derived predecessor reconciliation drifted")
    source_root = _predecessor_root(event_id)
    prepared = json.loads(_stable_bytes(source_root / "prepared-cell.json", label="pinned v3 prepared cell").decode("utf-8"))
    raw = _stable_bytes(source_root / "adapter-stdout.raw", label="pinned v3 raw stdout")
    state, result = _control_from_adapter(raw)
    if state != "completed" or result is None:
        raise ValueError("Revision-gain live v4 derived predecessor control is not completed")
    runtime = result["runtime"]
    source_outbound = _stable_bytes(source_root / "outbound-payload.json", label="pinned v3 outbound payload")
    binding = _commitment(V3_TERMINAL_RUN_ROOT, source_root / "governed-route-proof.json")
    _broker, route = _reauth_route(pilot=_pilot(), run_root=V3_TERMINAL_RUN_ROOT, phase="revision_generation", event_id=event_id, binding=binding)
    expected_native = _receipt_from_control(pilot=_pilot(), root=source_root, prepared=prepared, route=route, control_raw=raw, payload_override=source_outbound)
    expected = {"format_version": 1, "study_id": STUDY_ID, "kind": "derived_reconciled_predecessor_grok_native_receipt", "source_lineage_sha256": _sha256(canonical(expected_lineage)), "reconciliation_sha256": _sha256(canonical(expected_reconciliation)), "phase": "revision_generation", "event_id": event_id, "identity": identity, "provider_calls_made": 0, "process_launches": 0, "provider_model": "grok-4.6", "reasoning": "high", "tools_enabled": False, "runtime_command_identity": runtime["command_identity"], "observed_command_identity_hash": runtime["command_identity_hash"], "native_receipt": expected_native, "response": expected_native["response"]}
    if canonical(receipt) + b"\n" != receipt_raw or receipt != expected:
        raise ValueError("Revision-gain live v4 derived predecessor receipt drifted")
    return root, dict(receipt)


def ingest_predecessor_revision(*, output_root: Path, event_id: str) -> dict[str, Any]:
    """Create a v4 revision lineage record only after all four derived authorities validate."""
    output_root = _validate_derived_output_root(output_root)
    receipts = {name: _read_derived_predecessor_receipt(output_root=output_root, event_id=name) for name in PREDECESSOR_TERMINAL_CELLS}
    root, receipt = receipts[event_id]
    if (root / "revision-record.json").exists():
        raise ValueError("Revision-gain live v4 predecessor revision is already ingested")
    pilot = _pilot()
    event = next(row for row in pilot.revision_schedule() if row["event_id"] == event_id)
    descendant = Path(output_root) / "descendants" / f"{event_id}.md"
    descendant.parent.mkdir(parents=True, exist_ok=True)
    _write_bytes_once(descendant, receipt["response"]["story"].encode("utf-8"))
    source = pilot.contract()["sources"]["items"][event["source_item_id"]]
    record = {"format_version": 1, "study_id": STUDY_ID, "kind": "ingested_reconciled_predecessor_revision", "event_id": event_id, "phase": "revision_generation", "source": {"item_id": event["source_item_id"], "source.md": source["source.md"], "prompt.md": source["prompt.md"]}, "descendant": _commitment(Path(output_root), descendant), "generator": {"model": "grok-4.6", "reasoning": "high", "tools_enabled": False}, "generator_receipt": _commitment(Path(output_root), root / "reconciled-verified-receipt.json"), "source_lineage_sha256": receipt["source_lineage_sha256"]}
    _write_once(root / "revision-record.json", record)
    _validate_derived_predecessor_inventory(root, ingested=True)
    return record
