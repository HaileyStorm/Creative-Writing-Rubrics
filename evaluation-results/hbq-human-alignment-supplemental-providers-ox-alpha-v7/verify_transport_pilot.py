#!/usr/bin/env python3
"""Verify Ox Alpha v7 transport evidence without score or label analysis."""
...
CHECKPOINT_NAME = re.compile(r"batch-[0-9]{4}\.json\Z")


def checkpoint_paths(response_root: Path) -> list[Path]:
    """Return only canonical checkpoint records, never provider sidecar JSON."""
    try:
        return sorted(path for path in response_root.iterdir() if path.is_file() and CHECKPOINT_NAME.fullmatch(path.name))
    except OSError as exc:
        raise ValueError("Ox v7 response directory is unreadable") from exc
...
def _inside(root: Path, path: Path) -> Path:
    ...
    except ValueError as exc: raise ValueError("Ox v7 raw evidence path escapes its run") from exc
    return path
...
def _judge_leaf(evidence: Path, proof_path: Path) -> tuple[Path, Path]:
    ...
    if proof_path.parent.parent != evidence or not proof_path.is_file():
        raise ValueError("Ox v7 serialization proof is not a direct evidence sibling")
    try:
        children = sorted(path for path in evidence.iterdir() if path.is_dir())
    except OSError as exc:
        raise ValueError("Ox v7 evidence parent is unreadable") from exc
    judge = []
    ...
    for child in children:
        ...
        try:
            ...
            raise ValueError("Ox v7 evidence leaf events are unreadable") from exc
    ...
    if len(children) != 2 or len(judge) != 1 or proof_path.parent == judge[0]:
        raise ValueError("Ox v7 evidence parent must contain one Judge leaf and one ProveLock sibling")
    ...
    if receipt.get("status") != "success" or not any(
        ...
        raise ValueError("Ox v7 ProveLock sibling is not sealed")
    ...
    if prove_attempts or len(judge_attempts) != 1:
        raise ValueError("Ox v7 evidence parent must bind exactly one Judge HTTP attempt")
...
def _assert_proof_binding(events: list[Any], proof_path: Path) -> None:
    ...
    if not isinstance(metadata, Mapping) or metadata.get("serialization_proof_sha256") != hashlib.sha256(proof_path.read_bytes()).hexdigest() or Path(str(metadata.get("serialization_proof_path", ""))).resolve() != proof_path.resolve():
        raise ValueError("Ox v7 Judge leaf does not bind its ProveLock sibling")
...
def _expected_prompt(frozen: Mapping[str, Any], folder: Path, cell: Mapping[str, Any]) -> bytes:
    ...
    if assets != judge_assets() or not isinstance(assets, Mapping) or assets.get("strict_ai") is not False or assets.get("judge_prefix", {}).get("included") is not False:
        raise ValueError("Ox v7 judge-prefix policy or assets drifted")
    ...
    if [item["question"]["id"] for item in selected] != requested: raise ValueError("Ox v7 frozen question sequence cannot render its prompt")
    names = [item["name"] for item in assets["active_prompts"]]
    if names != ["BINARY_EVALUATION_PROMPT.md"]: raise ValueError("Ox v7 active judge-instruction policy drifted")
    binary = "\n\n".join((prompts_dir() / "judge" / name).read_bytes().decode("utf-8").strip() for name in names)
    return _render_prompt(binary_prompt=binary, artifact={"name": "source.md", "text": (folder / "source.md").read_bytes().decode("utf-8")}, contexts=[{"name": "prompt.md", "text": (folder / "prompt.md").read_bytes().decode("utf-8")}], bundle_id="prose.short_story", artifact_id=str(cell["item_id"]), questions=selected, provider="nous", model=CONTRACT["provider"]["model"]).encode("utf-8")
...
def _raw_transport(run: Path, checkpoint: Mapping[str, Any], prompt: bytes, frozen: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(provider, Mapping) or any(provider.get(key) != value for key, value in expected.items()): raise ValueError("Ox v7 requires one unrecovered physical HTTP attempt")
    artifacts = provider.get("provider_artifacts")
    if not isinstance(artifacts, Mapping) or set(artifacts) != {"judge_request", "judge_result", "serialization_proof", "evidence_tree"}: raise ValueError("Ox v7 provider artifact receipt is incomplete")
    evidence_ref = artifacts["evidence_tree"].get("path") if isinstance(artifacts["evidence_tree"], Mapping) else None
    if not isinstance(evidence_ref, str): raise ValueError("Ox v7 evidence root is unbound")
    evidence = _inside(run, run / evidence_ref)
    try: _validate_provider_artifacts(run, checkpoint)
    except Exception as exc: raise ValueError("Ox v7 raw provider artifacts are invalid") from exc
    proof_ref = artifacts["serialization_proof"].get("path") if isinstance(artifacts["serialization_proof"], Mapping) else None
    if not isinstance(proof_ref, str): raise ValueError("Ox v7 serialization proof binding is missing")
    proof_path = _inside(run, run / proof_ref)
    if artifacts["serialization_proof"] != _artifact(run, proof_path): raise ValueError("Ox v7 serialization proof artifact drifted")
    judge_leaf, _ = _judge_leaf(evidence, proof_path)
    events_path, receipt_path = judge_leaf / "events.jsonl", judge_leaf / "receipt.json"
    if not events_path.is_file() or not receipt_path.is_file(): raise ValueError("Ox v7 Judge leaf lacks event or provider receipt")
    ...
    if len(attempts) != 1 or not isinstance(attempts[0], Mapping): raise ValueError("Ox v7 raw evidence has a second physical HTTP attempt")
    ...
    if any(isinstance(value, bool) or not isinstance(value, int) for value in (started, finished, status)) or not _within_raw_http_sla(finished - started) or not 200 <= status < 300:
        raise ValueError("Ox v7 HTTP attempt is non-2xx or not below 150 seconds")
    logical_id, payload_sha = attempt.get("logical_request_id"), attempt.get("request_payload_sha256")
    if not isinstance(logical_id, str) or not logical_id or not isinstance(payload_sha, str) or len(payload_sha) != 64: raise ValueError("Ox v7 raw HTTP attempt lacks logical request or payload binding")
    bridge = read_json(receipt_path); run_id = bridge.get("run_id")
    if bridge.get("status") != "success" or not isinstance(run_id, str) or not run_id: raise ValueError("Ox v7 bridge provider receipt is not successful")
    ...
    if not isinstance(request_ref, str) or not isinstance(result_ref, str): raise ValueError("Ox v7 raw request/result bindings are missing")
    request, result = _inside(run, run / request_ref), _inside(run, run / result_ref)
    if not request.is_file() or not result.is_file() or artifacts["judge_request"] != _artifact(run, request) or artifacts["judge_result"] != _artifact(run, result): raise ValueError("Ox v7 raw request/result artifact drifted")
    ...
    if frozen.get("judge_assets") != judge_assets() or raw_request != expected_request: raise ValueError("Ox v7 raw request does not bind the frozen judge prompt or response schema")
...
def _invocation(work: Path) -> dict[str, Any]:
    ...
    if any(record.get(key) != value for key, value in expected.items()) or not isinstance(record.get("zero_cost_fresh_at_invocation"), str) or set(record) != {*expected, "zero_cost_fresh_at_invocation"}: raise ValueError("Ox v7 invocation is unbound")
...
def _claim(work: Path) -> dict[str, Any]:
    ...
    if any(record.get(key) != value for key, value in expected.items()) or isinstance(record.get("pid"), bool) or not isinstance(record.get("pid"), int) or set(record) != {*expected, "pid"}: raise ValueError("Ox v7 exclusive claim is unbound")
    return record
...
def verify_cell(work: Path, frozen: Mapping[str, Any], cell: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(configuration, Mapping) or manifest.get("format_version") != 3 or manifest.get("config_sha256") != hashlib.sha256(_json_bytes(configuration)).hexdigest(): raise ValueError("Ox v7 run manifest is malformed")
    ...
    assets = frozen.get("judge_assets")
    if any(configuration.get(key) != value for key, value in expected.items()) or _compact(configuration.get("artifact")) != cell["inputs"]["source.md"] or [_compact(item) for item in configuration.get("contexts", [])] != [cell["inputs"]["prompt.md"]] or _compact(configuration.get("task_contract")) != cell["inputs"]["task-contract.json"] or [_compact(item) for item in configuration.get("prompts", [])] != assets.get("active_prompts") or _compact(configuration.get("response_schema")) != assets.get("response_schema"): raise ValueError("Ox v7 run settings or inputs drifted")
    checkpoints = checkpoint_paths(run / "responses")
    if [path.name for path in checkpoints] != ["batch-0001.json"] or list((run / "responses" / "rejected").rglob("*.json")): raise ValueError("Ox v7 requires one clean batch")
    checkpoint = read_json(checkpoints[0]); prompt_bytes = gzip.decompress(checkpoints[0].with_suffix(".prompt.txt.gz").read_bytes())
    if checkpoint.get("format_version") != 4 or checkpoint.get("batch") != 1 or checkpoint.get("question_ids") != cell["question_ids"] or checkpoint.get("retry_policy") != {"batch_attempts": 1} or checkpoint.get("accepted_attempt") != 1 or checkpoint.get("recovered_from_rejected") is not None or checkpoint.get("rejected_chain") != {"count": 0, "head_sha256": None} or prompt_bytes != _expected_prompt(frozen, artifact.parent, cell) or checkpoint.get("base_prompt_sha256") != hashlib.sha256(prompt_bytes).hexdigest() or checkpoint.get("prompt_sha256") != hashlib.sha256(prompt_bytes).hexdigest(): raise ValueError("Ox v7 checkpoint is unbound or recovered")
    try: _load_checkpoints(run, artifact_text=artifact.read_bytes().decode("utf-8"), context_texts=[prompt.read_bytes().decode("utf-8")], batch_attempts=1, normalization_policy=EVIDENCE_NORMALIZATION_POLICY)
    except Exception as exc: raise ValueError("Ox v7 completion is not schema-valid") from exc
...
def verify_pilot(work: Path) -> dict[str, Any]:
    ...
    if len(records) != 3 or [record.get("sequence") for record in records] != [1, 2, 3] or any(record.get("status") != "completed" for record in records): raise ValueError("Ox v7 did not produce three completed transport cells")
    proofs = [verify_cell(work, frozen, cell) for cell in frozen["cells"]]
    if any(len({proof[key] for proof in proofs}) != 3 for key in ("session_id", "receipt_id", "logical_request_id")): raise ValueError("Ox v7 reuses a provider session, receipt, or logical request")
    predecessor = frozen.get("uncertain_v6")
    ...
    if not isinstance(ancestor_ids, Mapping) or any(not isinstance(ancestor_ids.get(key), str) or not ancestor_ids[key] for key in ("session_id", "receipt_id", "logical_request_id")):
        raise ValueError("Ox v7 lacks bound accepted-v6 provider identities")
    if any(any(proof[key] == ancestor_ids[key] for proof in proofs) for key in ("session_id", "receipt_id", "logical_request_id")):
        raise ValueError("Ox v7 reuses an accepted-v6 provider session, receipt, or logical request")
    receipt_paths = [work / "pilot-receipts" / f"{cell['cell_id']}.json" for cell in frozen["cells"]]
    if not (work / "pilot-receipts").is_dir() or sorted(path.name for path in (work / "pilot-receipts").iterdir()) != sorted(path.name for path in receipt_paths): raise ValueError("Ox v7 receipt set is malformed")
    for cell, proof, path in zip(frozen["cells"], proofs, receipt_paths):
        if read_json(path) != _expected_receipt(cell, proof): raise ValueError("Ox v7 immutable receipt semantic body drifted")
    ...
    if records != expected or [path.name for path in paths] != [f"{number:04d}-{cell['cell_id']}.json" for number, cell in enumerate(frozen["cells"], 1)]: raise ValueError("Ox v7 journal does not bind immutable receipts")
