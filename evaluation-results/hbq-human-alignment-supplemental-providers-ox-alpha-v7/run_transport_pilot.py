#!/usr/bin/env python3
"""Execute the frozen Ox Alpha v7 probe once, serially, with a cap of one."""
...
def _records(work: Path) -> list[Path]:
    ...
    if len(records) != len(list(root.iterdir())): raise ValueError("Ox v7 journal is malformed")
    return records
...
def _append(work: Path, record: Mapping[str, Any]) -> None:
    ...
    if not isinstance(cell_id, str) or not cell_id: raise ValueError("Ox v7 journal cell is malformed")
    ...
    except FileExistsError as exc: raise ValueError("Ox v7 journal record already exists") from exc
...
def _claim(work: Path) -> None:
    ...
    try: descriptor = os.open(path, os.O_CREAT | os.O_EXCL | os.O_WRONLY)
    except FileExistsError as exc: raise ValueError("Ox v7 root already has an execution claim; retry or resume is forbidden") from exc
    with os.fdopen(descriptor, "w", encoding="utf-8", newline="\n") as output:
        output.write(rendered); output.flush(); os.fsync(output.fileno())


def _tree(root: Path) -> dict[str, Any]:
    if not root.is_dir(): raise ValueError("Ox v7 bridge evidence root is unavailable")
...
def _terminal_judge_boundary(records: list[Any]) -> None:
    ...
    if len(boundaries) != 1 or not isinstance(boundaries[0], Mapping) or boundaries[0].get("request_schema") != CONTRACT["transport_pilot"]["required_request_schema"] or boundaries[0].get("model_policy") != expected_model or boundaries[0].get("transport_policy") != expected_transport or boundaries[0].get("zero_tools") is not True:
        raise ValueError("Ox v7 terminal Judge boundary lacks exact request-v2 cap-1 transport/model policy")
...
def _terminal_failure_seal(work: Path, cell: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if len(children) != 2:
        raise ValueError("Ox v7 terminal evidence must contain exactly ProveLock and Judge leaves")
    ...
    for child in children:
        ...
        try:
            ...
            raise ValueError("Ox v7 terminal evidence leaf is unreadable") from exc
    ...
    if len(judge) != 1:
        raise ValueError("Ox v7 terminal evidence must contain exactly one Judge leaf")
    ...
    if prove_attempts or len(judge_attempts) != 1 or not isinstance(judge_attempts[0], Mapping):
        raise ValueError("Ox v7 terminal evidence does not bind exactly one Judge HTTP attempt")
    status = judge_attempts[0].get("status")
    if isinstance(status, bool) or not isinstance(status, int) or 200 <= status < 300:
        raise ValueError("Ox v7 terminal failure does not bind one failed physical attempt")
    ...
    if prove_receipt.get("status") != "success" or not any(isinstance(record, Mapping) and record.get("event_type") == "serialization_proof" for record in prove_records):
        raise ValueError("Ox v7 terminal evidence lacks a sealed ProveLock sibling")
    ...
    if not isinstance(receipt, Mapping) or receipt.get("status") != "failure" or any(not isinstance(receipt.get(key), str) or not receipt[key] for key in ("sealed_at", "terminal_chain_sha256", "events_sha256")):
        raise ValueError("Ox v7 bridge has not sealed a terminal failure")
    time.sleep(TERMINAL_STABILITY_SECONDS)
    if _tree(evidence) != initial or not failure.is_file() or read_json(failure) != receipt: raise ValueError("Ox v7 bridge terminal evidence is not stable after launcher return")
...
def execute(work: Path) -> None:
    ...
    if _records(work) or any((work / name).exists() for name in ("runs", "pilot-execution-claim.json", "pilot-receipts", "pilot-uncertain.json")):
        raise ValueError("Ox v7 has immutable evidence; retry, fallback, and resume are forbidden")
    ...
    for cell in frozen["cells"]:
        try:
            ...
            try: terminal = _terminal_failure_seal(work, cell)
                ...
                raise ValueError("Ox v7 root is uncertain and blocked; terminal bridge quiescence was not proved") from exc
    ...
    try:
        ...
        raise ValueError("Ox v7 completed root is uncertain and blocked; global verification failed") from exc
