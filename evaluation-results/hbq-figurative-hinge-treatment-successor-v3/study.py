...
def slots()->list[dict[str,str]]: return [{"slot_id":f"hingev3-{c['case_id']}-r{r}","case_id":c["case_id"],"repeat":str(r),"leaf_id":TARGET} for c in corpus() for r in(1,2)]
...
def bundle()->list[dict[str,Any]]:
 ...
 return [{"standard":{"id":"HBQ-RS","version":"1.2.0"},"bundle_id":STUDY_ID,"version":1,"title":"Figurative hinge v3 singleton diagnostic","module_ids":[leaf["module_id"]],"task_contract_domain_id":"figurative-hinge-v3","domains":[{"domain_id":"figurative-hinge-v3","title":TARGET,"points":1.0,"components":[{"module_id":leaf["module_id"],"weight":1.0,"include_question_ids":[TARGET]}],"score_mode":"weighted_binary_mean"}],"penalty_modules":[],"hard_gate_policy":{"no_is_invalid":True,"cannot_assess_is_unresolved":True,"not_applicable_requires_condition_or_reason":True,"hard_gates_are_reported_separately":True},"coverage_policy":{"minimum_weighted_coverage":0.0,"below_threshold_status":"PROVISIONAL","score_interval_required_when_unassessed":True,"whole_work_claims_require_whole_work_evidence":True}}]
def task(s:dict[str,str])->dict[str,Any]: return {"contract_version":1,"contract_id":f"{STUDY_ID}-{s['slot_id']}","artifact_id":s["slot_id"],"context":{"artifact_kind":"prose.short_story","declared_scope":"complete supplied passage","completion_status":"complete","background":["Public synthetic figurative hinge v3 diagnostic."],"constraints":["Use only the supplied artifact.",QUOTE,contract()["treatment"]["exact_text"]],"audience":["development-only rubric validation"]},"preferences":[],"priorities":[],"weighted_goals":[],"binding_requirements":[]}
def override(s:dict[str,str],t:dict[str,Any])->dict[str,Any]: return {"format_version":1,"artifact_id":s["slot_id"],"bundle_id":STUDY_ID,"task_contract_sha256":hashlib.sha256(canon(t)).hexdigest(),"contract_id":t["contract_id"],"artifact_kind":t["context"]["artifact_kind"],"declared_scope":t["context"]["declared_scope"],"compatibility_mode":"reviewed_override","decision_id":"figurative-hinge-v3-scope","reviewer":"hbqrs-reviewed-v1","reason":"Reviewed compatibility for a public synthetic figurative hinge v3 diagnostic."}
