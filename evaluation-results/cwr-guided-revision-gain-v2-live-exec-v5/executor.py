...
STUDY_ID = "cwr-guided-revision-gain-v2-live-exec-v5"
...
V4_EXECUTOR_PATH = ROOT / "evaluation-results" / "cwr-guided-revision-gain-v2-live-exec-v4" / "executor.py"
V3_EXECUTOR_PATH = ROOT / "evaluation-results" / "cwr-guided-revision-gain-v2-live-exec-v3" / "executor.py"
QUEUE_TOOLS_ROOT = Path(r"C:\Users\Haile\.codex\tools")
DEFAULT_QUEUE_ROOT = Path(r"C:\Users\Haile\.codex\state\model-work-queue")
V3_TERMINAL_RUN_ROOT = Path(r"C:\Users\Haile\Documents\cwr-revision-gain-v3-live-8d6cce1-20260830a")
V4_DERIVED_RUN_ROOT = Path(r"C:\Users\Haile\Documents\cwr-revision-gain-v4-reconcile-1f57ad1-20260830a")
V4_EXECUTOR_SHA256 = "1f962cd5cb731968e6baef37932c6aebb7c5667c99ea57a59cc7dc52f9f88250"
V4_DERIVED_INVENTORY_SHA256 = "d184f015ef98a59df777eaafbf62ad6f38ad51911654480d7077dd9af45a9773"
V3_SOL_FEEDBACK_RECEIPTS = {
    "feedback-v2-revision-v2-c1-hanna-1035-grok-4.6-cwr_guided": "48d856fe32e5bf8ec39c6dfcb484ecb2d1d780c0ab69a3fc7c0fd4a7f5e8f162",
    "feedback-v2-revision-v2-c1-hanna-178-grok-4.6-cwr_guided": "71f6c342124baeea517d006a468b0a4bd8cca03b8c0c669ce69ea9a2293705fb",
}
EXPECTED_ROUTES = {
...
def _inventory_candidates(*, state: str, phase: str | None, provider_model: str | None = None) -> set[frozenset[str]]:
    ...
    if state == "reconcile_pending":
        ...
        if provider_model == "gpt-5.6-sol":
            return {base}
        raise ValueError("Revision-gain live v4 cell inventory provider is unsupported")
    if state == "reconciling":
        base = _PREPARED_ROOT_FILES | _RECONCILE_PENDING_FILES | frozenset({"reconciliation.json"})
        native = frozenset({"adapter-native-binding.json"})
        if provider_model is None:
            return {base, base | native}
        if provider_model == "grok-4.6":
    ...
    if state not in {"settled", "reconciled"} or phase not in _INGEST_FILES:
        raise ValueError("Revision-gain live v4 cell inventory state is unsupported")
...
def _validate_cell_inventory(*, root: Path, state: str, phase: str | None = None, provider_model: str | None = None) -> None:
    ...
    try:
        ...
        raise ValueError("Revision-gain live v4 cell root is missing") from error
    if not stat.S_ISDIR(info.st_mode) or stat.S_ISLNK(info.st_mode) or bool(getattr(info, "st_file_attributes", 0) & 0x400):
        raise ValueError("Revision-gain live v4 cell root is not a plain directory")
    ...
    if names not in _inventory_candidates(state=state, phase=phase, provider_model=provider_model):
        raise ValueError("Revision-gain live v4 cell inventory is not valid for its state")
    for child in children:
        ...
        if not stat.S_ISREG(child_info.st_mode) or stat.S_ISLNK(child_info.st_mode) or bool(getattr(child_info, "st_file_attributes", 0) & 0x400):
            raise ValueError("Revision-gain live v4 cell inventory contains a non-plain artifact")
...
PREDECESSOR_TERMINAL_CELLS = {
    "revision-v2-c1-hanna-1035-grok-4.6-cwr_guided": "a43f64ce998a82e41ba8dd76a76bb047d3c96a75576267cbc42455a9f1905b77",
    "revision-v2-c1-hanna-1035-grok-4.6-generic_no_feedback": "6ee728eff1735c95c37997ceb9a5d06cf34e0ffe2c59360ebf5787ad5dbf3adf",
    "revision-v2-c1-hanna-178-grok-4.6-cwr_guided": "6f90dafddfe3ba1cd2041107dcce2eb4ae42b7ae8311b31c5e3a50cc5c9169b4",
    "revision-v2-c1-hanna-178-grok-4.6-generic_no_feedback": "004840636611ae47e0f490dbb1044b76c12a9cab12505369780e9582ffec02db",
}
PREDECESSOR_PRESENT_ARTIFACTS = frozenset({"adapter-control.json", "adapter-schema-binding.json", "adapter-stdout-binding.json", "adapter-stdout.raw", "governed-route-proof.json", "launch-intent.json", "live-admission.json", "outbound-payload.json", "payload.json", "prepared-cell.json", "terminal-outcome.json"})
PREDECESSOR_ABSENT_ARTIFACTS = frozenset({"adapter-native-binding.json", "adapter-stderr.raw", "adapter-stderr-binding.json", "verified-receipt.json", "reconciled-verified-receipt.json", "execution-result.json", "reconciliation.json", "feedback-ingest.json", "revision-record.json", "endpoint-ingest.json"})
def _predecessor_contract() -> dict[str, Any]:
    return {"study_id": "cwr-guided-revision-gain-v2-live-exec-v3", "executor_sha256": "6e1204478a0b614800b53aa557a31dac431bf8666431c8c9bbafbe0fc8e97037", "terminal_run_root": str(V3_TERMINAL_RUN_ROOT), "terminal_inventory_sha256": dict(PREDECESSOR_TERMINAL_CELLS), "present_artifacts": sorted(PREDECESSOR_PRESENT_ARTIFACTS), "absent_artifacts": sorted(PREDECESSOR_ABSENT_ARTIFACTS), "state": "terminal_postlaunch_reconcile_required_completed_control", "reconcile_or_resend": False}


def _cycle_one_import_contract() -> dict[str, Any]:
    return {
        "source_run_root": str(V3_TERMINAL_RUN_ROOT),
        "sol_feedback_receipt_sha256": dict(V3_SOL_FEEDBACK_RECEIPTS),
        "native_endpoint_contact_cardinality": "unproven",
    }


def _v4_import_contract() -> dict[str, Any]:
    return {
        "source_run_root": str(V4_DERIVED_RUN_ROOT),
        "executor_sha256": V4_EXECUTOR_SHA256,
        "inventory_sha256": V4_DERIVED_INVENTORY_SHA256,
    }


def _validate_predecessor_lineage(predecessor: Mapping[str, Any]) -> None:
    if predecessor != _predecessor_contract():
        raise ValueError("Revision-gain live v4 predecessor contract drifted")
    for event_id, expected_hash in PREDECESSOR_TERMINAL_CELLS.items():
        root = V3_TERMINAL_RUN_ROOT / "live-cells" / "revision_generation" / event_id
        if not root.is_dir() or stat.S_ISLNK(os.lstat(root).st_mode) or bool(getattr(os.lstat(root), "st_file_attributes", 0) & 0x400):
            raise ValueError("Revision-gain live v4 predecessor terminal root is unsafe")
        if {path.name for path in root.iterdir()} != PREDECESSOR_PRESENT_ARTIFACTS or any((root / name).exists() for name in PREDECESSOR_ABSENT_ARTIFACTS):
            raise ValueError("Revision-gain live v4 predecessor terminal inventory drifted")
        inventory = {name: {"bytes": len(raw := _stable_bytes(root / name, label="pinned v3 terminal artifact")), "sha256": _sha256(raw)} for name in sorted(PREDECESSOR_PRESENT_ARTIFACTS)}
        if _sha256(canonical(inventory)) != expected_hash:
            raise ValueError("Revision-gain live v4 predecessor terminal artifact drifted")
        if _control_from_adapter(_stable_bytes(root / "adapter-stdout.raw", label="pinned v3 raw stdout"))[0] != "completed":
            raise ValueError("Revision-gain live v4 predecessor control is not completed")


def _predecessor_root(event_id: str) -> Path:
    if event_id not in PREDECESSOR_TERMINAL_CELLS:
        raise ValueError("Revision-gain live v4 predecessor event is not one of the four terminal cells")
    return V3_TERMINAL_RUN_ROOT / "live-cells" / "revision_generation" / event_id


def _predecessor_inventory(event_id: str) -> dict[str, Any]:
    root = _predecessor_root(event_id)
    artifacts = {
        name: _commitment(V3_TERMINAL_RUN_ROOT, root / name)
        for name in sorted(PREDECESSOR_PRESENT_ARTIFACTS)
    }
    value = {
        "format_version": 1,
        "study_id": STUDY_ID,
        "kind": "immutable_v3_terminal_cell_lineage",
        "source_study_id": "cwr-guided-revision-gain-v2-live-exec-v3",
        "source_executor_sha256": contract()["predecessor"]["executor_sha256"],
        "source_run_root": str(V3_TERMINAL_RUN_ROOT),
        "phase": "revision_generation",
        "event_id": event_id,
        "source_artifacts": artifacts,
        "source_inventory_sha256": PREDECESSOR_TERMINAL_CELLS[event_id],
        "source_terminal_state": "terminal_postlaunch_reconcile_required",
        "no_resend": True,
    }
    if _sha256(canonical({name: {"bytes": item["bytes"], "sha256": item["sha256"]} for name, item in artifacts.items()})) != value["source_inventory_sha256"]:
        raise ValueError("Revision-gain live v4 predecessor inventory commitment drifted")
    return value
...
def _validate_v4_import_root() -> None:
    if _sha256(_stable_bytes(V4_EXECUTOR_PATH, label="pinned v4 executor")) != V4_EXECUTOR_SHA256:
        raise ValueError("Revision-gain live v5 pinned v4 executor drifted")
    expected = {"frozen-inputs.json", "frozen-cwr-question-payload.json"}
    for event_id in PREDECESSOR_TERMINAL_CELLS:
        expected |= {f"descendants/{event_id}.md"}
        base = f"predecessor-reconciliation/revision_generation/{event_id}"
        expected |= {f"{base}/{name}" for name in ("source-lineage.json", "reconciliation.json", "reconciled-verified-receipt.json", "revision-record.json")}
    expected_directories = {"descendants", "predecessor-reconciliation", "predecessor-reconciliation/revision_generation"}
    expected_directories |= {f"predecessor-reconciliation/revision_generation/{event_id}" for event_id in PREDECESSOR_TERMINAL_CELLS}
    found: set[str] = set()
    found_directories: set[str] = set()
    for path in V4_DERIVED_RUN_ROOT.rglob("*"):
        info = os.lstat(path)
        if stat.S_ISLNK(info.st_mode) or bool(getattr(info, "st_file_attributes", 0) & 0x400):
            raise ValueError("Revision-gain live v5 immutable v4 import inventory contains a reparse artifact")
        relative = path.relative_to(V4_DERIVED_RUN_ROOT).as_posix()
        if stat.S_ISDIR(info.st_mode):
            found_directories.add(relative)
        elif stat.S_ISREG(info.st_mode):
            _stable_bytes(path, label="immutable v4 import artifact")
            found.add(relative)
        else:
            raise ValueError("Revision-gain live v5 immutable v4 import inventory contains a non-plain artifact")
    if found != expected or found_directories != expected_directories:
        raise ValueError("Revision-gain live v5 immutable v4 import inventory drifted")
    inventory = {name: {"bytes": len(raw := _stable_bytes(V4_DERIVED_RUN_ROOT / name, label="immutable v4 import artifact")), "sha256": _sha256(raw)} for name in sorted(expected)}
    if _sha256(canonical(inventory)) != V4_DERIVED_INVENTORY_SHA256:
        raise ValueError("Revision-gain live v5 immutable v4 import artifact drifted")


def _validate_v5_continuation_root(root: Path, *, label: str) -> Path:
    candidate = Path(os.path.abspath(root))
    candidate_text = os.path.normcase(str(candidate))
    for immutable in (V3_TERMINAL_RUN_ROOT, V4_DERIVED_RUN_ROOT):
        source_text = os.path.normcase(str(Path(os.path.abspath(immutable))))
        try:
            if os.path.commonpath((candidate_text, source_text)) in {candidate_text, source_text}:
                raise ValueError(f"Revision-gain live v5 {label} overlaps immutable predecessor evidence")
        except ValueError as error:
            if "overlaps" in str(error):
                raise
            raise ValueError(f"Revision-gain live v5 {label} is on a different drive") from error
    ...
    for part in candidate.parts[1:]:
        ...
        try:
            ...
            break
        if stat.S_ISLNK(info.st_mode) or bool(getattr(info, "st_file_attributes", 0) & 0x400):
            raise ValueError(f"Revision-gain live v5 {label} has unsafe or reparsed ancestry")
        if not stat.S_ISDIR(info.st_mode):
            raise ValueError(f"Revision-gain live v5 {label} ancestry is not a directory")
    return candidate


def _pinned_v3_sol_feedback(feedback_event: str) -> tuple[Path, bytes, dict[str, Any]]:
    expected_hash = V3_SOL_FEEDBACK_RECEIPTS.get(feedback_event)
    if expected_hash is None:
        raise ValueError("Revision-gain live v5 imported Sol feedback event is not pinned")
    root = V3_TERMINAL_RUN_ROOT / "live-cells" / "cwr_feedback" / feedback_event
    raw = _stable_bytes(root / "verified-receipt.json", label="immutable v3 Sol feedback receipt")
    if _sha256(raw) != expected_hash:
        raise ValueError("Revision-gain live v5 immutable v3 Sol feedback receipt drifted")
    value = json.loads(raw.decode("utf-8"))
    if (canonical(value) + b"\n" != raw or value.get("kind") != "verified_local_codex_lifecycle_receipt"
            or value.get("event_id") != feedback_event or value.get("phase") != "cwr_feedback"
            or value.get("provider_model") != "gpt-5.6-sol" or value.get("reasoning") != "high"
            or value.get("tools_enabled") is not False
            or value.get("evidence_class") != "sol_local_codex_lifecycle_native_endpoint_cardinality_unproven_v1"):
        raise ValueError("Revision-gain live v5 immutable v3 Sol feedback receipt is invalid")
    return root, raw, value


def _imported_sol_reader(pilot: ModuleType):
    native_reader = pilot._native_read_verified_receipt
    def reader(path: Path, *, expected_event_id: str, expected_phase: str) -> dict[str, Any]:
        raw = _stable_bytes(Path(path), label="v5 imported receipt")
        value = json.loads(raw.decode("utf-8"))
        if value.get("kind") == "verified_local_codex_lifecycle_receipt":
            _, verified = _read_receipt_authority(pilot, root=Path(path).parent, event_id=expected_event_id, phase=expected_phase)
            if verified.get("provider_model") != "gpt-5.6-sol":
                raise ValueError("Revision-gain live v5 local lifecycle authority is not Sol feedback")
            return verified
        if value.get("kind") != "v5_imported_sol_lifecycle_authority":
            return native_reader(Path(path), expected_event_id=expected_event_id, expected_phase=expected_phase)
        if (canonical(value) + b"\n" != raw or value.get("event_id") != expected_event_id or value.get("phase") != expected_phase
                or value.get("provider_model") != "gpt-5.6-sol" or value.get("reasoning") != "high" or value.get("tools_enabled") is not False
                or value.get("native_endpoint_contact_cardinality") != "unproven"):
            raise ValueError("Revision-gain live v5 imported Sol authority drifted")
        source_root, source_raw, source_value = _pinned_v3_sol_feedback(expected_event_id)
        expected_source = {"root": str(source_root), "bytes": len(source_raw), "sha256": _sha256(source_raw)}
        if value.get("source_receipt") != expected_source:
            raise ValueError("Revision-gain live v5 imported Sol source receipt drifted")
        if source_value.get("response") != value.get("response"):
            raise ValueError("Revision-gain live v5 imported Sol lifecycle replay failed")
        return dict(value)
    pilot._read_verified_receipt = reader
    return pilot


def _rederive_imported_v4_grok_receipt(*, pilot: ModuleType, output_root: Path, event_id: str) -> tuple[dict[str, Any], dict[str, Any]]:
    source_root = V4_DERIVED_RUN_ROOT / "predecessor-reconciliation" / "revision_generation" / event_id
    source_path = source_root / "reconciled-verified-receipt.json"
    source_raw = _stable_bytes(source_path, label="immutable v4 derived receipt")
    source = json.loads(source_raw.decode("utf-8"))
    if canonical(source) + b"\n" != source_raw or not isinstance(source.get("native_receipt"), Mapping):
        raise ValueError("Revision-gain live v5 immutable v4 derived receipt is invalid")
    import_root = Path(output_root) / "imports" / "revision_generation" / event_id
    prepared_raw = _stable_bytes(import_root / "prepared-cell.json", label="imported prepared cell")
    prepared = json.loads(prepared_raw.decode("utf-8"))
    if canonical(prepared) + b"\n" != prepared_raw or not isinstance(prepared.get("payload"), Mapping):
        raise ValueError("Revision-gain live v5 imported prepared cell is invalid")
    native = dict(source["native_receipt"])
    native["transmitted_payload_sha256"] = prepared["payload"].get("sha256")
    verified = pilot.validate_receipt(prepared_root=import_root, receipt=native, output_path=None)
    binding = {
        "format_version": 1,
        "study_id": STUDY_ID,
        "kind": "v5_imported_v4_grok_receipt_binding",
        "event_id": event_id,
        "source_reconciled_receipt": _commitment(V4_DERIVED_RUN_ROOT, source_path),
        "source_native_receipt_sha256": _sha256(canonical(source["native_receipt"])),
        "normalized_native_receipt_sha256": _sha256(canonical(native)),
        "imported_verified_receipt": {
            "path": f"imports/revision_generation/{event_id}/verified-receipt.json",
            "bytes": len(canonical(verified)) + 1,
            "sha256": _sha256(canonical(verified) + b"\n"),
        },
    }
    return verified, binding


def import_cycle_one(*, output_root: Path) -> tuple[list[dict[str, Any]], dict[str, Path]]:
    """Materialize exact pilot-shaped cycle-one records from immutable v4/v3 evidence."""
    output_root = _validate_v5_continuation_root(Path(output_root), label="cycle-one import root")
    _validate_v4_import_root()
    if (output_root / "imports").exists():
        return _load_cycle_one_import(output_root=output_root)
    if output_root.exists():
        raise ValueError("Revision-gain live v5 cycle-one import root must be fresh")
    pilot = _imported_sol_reader(_pilot())
    output_root.mkdir(parents=True, exist_ok=False)
    for name in ("frozen-inputs.json", "frozen-cwr-question-payload.json"):
        target = output_root / name
        raw = _stable_bytes(V4_DERIVED_RUN_ROOT / name, label="immutable v4 frozen import artifact")
        if target.exists():
            if _stable_bytes(target, label="existing v5 frozen import artifact") != raw:
                raise ValueError("Revision-gain live v5 frozen import artifact drifted")
        else:
            _write_bytes_once(target, raw)
    schedule = {row["event_id"]: row for row in pilot.revision_schedule()}
    records: list[dict[str, Any]] = []
    feedback_paths: dict[str, Path] = {}
    for event_id in PREDECESSOR_TERMINAL_CELLS:
        event = schedule[event_id]
        source_base = V4_DERIVED_RUN_ROOT / "predecessor-reconciliation" / "revision_generation" / event_id
        source_record_raw = _stable_bytes(source_base / "revision-record.json", label="immutable v4 revision record")
        source_record = json.loads(source_record_raw.decode("utf-8"))
        receipt_raw = _stable_bytes(source_base / "reconciled-verified-receipt.json", label="immutable v4 derived receipt")
        receipt = json.loads(receipt_raw.decode("utf-8"))
        descendant_raw = _stable_bytes(V4_DERIVED_RUN_ROOT / f"descendants/{event_id}.md", label="immutable v4 descendant")
        if (canonical(source_record) + b"\n" != source_record_raw or canonical(receipt) + b"\n" != receipt_raw
                or source_record.get("event_id") != event_id or source_record.get("source") != {"item_id": event["source_item_id"], "source.md": pilot.contract()["sources"]["items"][event["source_item_id"]]["source.md"], "prompt.md": pilot.contract()["sources"]["items"][event["source_item_id"]]["prompt.md"]}
                or receipt.get("response", {}).get("story", "").encode("utf-8") != descendant_raw):
            raise ValueError("Revision-gain live v5 immutable v4 revision import drifted")
        import_root = output_root / "imports" / "revision_generation" / event_id
        import_root.mkdir(parents=True, exist_ok=False)
        v3_root = V3_TERMINAL_RUN_ROOT / "live-cells" / "revision_generation" / event_id
        for name in ("prepared-cell.json", "payload.json", "launch-intent.json"):
            _write_bytes_once(import_root / name, _stable_bytes(v3_root / name, label="immutable v3 revision import artifact"))
        verified, binding = _rederive_imported_v4_grok_receipt(pilot=pilot, output_root=output_root, event_id=event_id)
        _write_once(import_root / "verified-receipt.json", verified)
        _write_once(import_root / "v4-reconciliation-binding.json", binding)
        descendant = output_root / "imports" / "descendants" / f"{event_id}.md"
        _write_bytes_once(descendant, descendant_raw)
        feedback = None
        if event["guidance_arm"] == "cwr_guided":
            feedback_event = event["cwr_feedback_event_id"]
            source_feedback_root, original, _source_value = _pinned_v3_sol_feedback(feedback_event)
            imported_feedback = {"format_version": 1, "study_id": STUDY_ID, "kind": "v5_imported_sol_lifecycle_authority", "event_id": feedback_event, "phase": "cwr_feedback", "provider_model": "gpt-5.6-sol", "reasoning": "high", "tools_enabled": False, "native_endpoint_contact_cardinality": "unproven", "source_receipt": {"root": str(source_feedback_root), "bytes": len(original), "sha256": _sha256(original)}, "response": json.loads(original.decode("utf-8"))["response"]}
            feedback_path = output_root / "imports" / "cwr_feedback" / f"{feedback_event}.json"
            _write_once(feedback_path, imported_feedback)
            feedback_paths[feedback_event] = feedback_path
            feedback = {"event_id": feedback_event, "verified_receipt": _commitment(output_root, feedback_path)}
        records.append({"event_id": event_id, "source": source_record["source"], "parent": None, "descendant": _commitment(output_root, descendant), "generator": {"model": "grok-4.6", "reasoning": "high", "tools_enabled": False}, "generator_receipt": _commitment(output_root, import_root / "verified-receipt.json"), "cwr_feedback": feedback})
    if len(records) != 4:
        raise ValueError("Revision-gain live v5 requires all four imported cycle-one records")
    return records, feedback_paths


def _load_cycle_one_import(*, output_root: Path) -> tuple[list[dict[str, Any]], dict[str, Path]]:
    """Re-open the complete write-once import so later cycle-two cells share it."""
    output_root = _validate_v5_continuation_root(Path(output_root), label="cycle-one import root")
    _validate_v4_import_root()
    pilot = _imported_sol_reader(_pilot())
    imports = output_root / "imports"
    expected_imports: set[str] = set()
    expected_directories = {"revision_generation", "descendants", "cwr_feedback"}
    for event_id in PREDECESSOR_TERMINAL_CELLS:
        expected_imports |= {
            f"revision_generation/{event_id}/{name}"
            for name in ("prepared-cell.json", "payload.json", "launch-intent.json", "verified-receipt.json", "v4-reconciliation-binding.json")
        }
        expected_imports.add(f"descendants/{event_id}.md")
        expected_directories.add(f"revision_generation/{event_id}")
    expected_imports |= {f"cwr_feedback/{event_id}.json" for event_id in V3_SOL_FEEDBACK_RECEIPTS}
    found: set[str] = set()
    found_directories: set[str] = set()
    for path in imports.rglob("*"):
        info = os.lstat(path)
        if stat.S_ISLNK(info.st_mode) or bool(getattr(info, "st_file_attributes", 0) & 0x400):
            raise ValueError("Revision-gain live v5 cycle-one import contains a reparse artifact")
        if stat.S_ISDIR(info.st_mode):
            found_directories.add(path.relative_to(imports).as_posix())
            continue
        if not stat.S_ISREG(info.st_mode):
            raise ValueError("Revision-gain live v5 cycle-one import contains a non-plain artifact")
        _stable_bytes(path, label="v5 cycle-one import artifact")
        found.add(path.relative_to(imports).as_posix())
    if found != expected_imports or found_directories != expected_directories:
        raise ValueError("Revision-gain live v5 cycle-one import is incomplete or has extra artifacts")
    for name in ("frozen-inputs.json", "frozen-cwr-question-payload.json"):
        if _stable_bytes(output_root / name, label="v5 frozen import artifact") != _stable_bytes(V4_DERIVED_RUN_ROOT / name, label="immutable v4 frozen import artifact"):
            raise ValueError("Revision-gain live v5 frozen import artifact drifted")
    schedule = {row["event_id"]: row for row in pilot.revision_schedule()}
    records: list[dict[str, Any]] = []
    feedback_paths: dict[str, Path] = {}
    for event_id in PREDECESSOR_TERMINAL_CELLS:
        event = schedule[event_id]
        import_root = imports / "revision_generation" / event_id
        v3_root = V3_TERMINAL_RUN_ROOT / "live-cells" / "revision_generation" / event_id
        for name in ("prepared-cell.json", "payload.json", "launch-intent.json"):
            if _stable_bytes(import_root / name, label="v5 imported Grok artifact") != _stable_bytes(v3_root / name, label="immutable v3 revision import artifact"):
                raise ValueError("Revision-gain live v5 imported Grok artifact drifted")
        verified, binding = _rederive_imported_v4_grok_receipt(pilot=pilot, output_root=output_root, event_id=event_id)
        if (_stable_bytes(import_root / "verified-receipt.json", label="v5 imported Grok receipt") != canonical(verified) + b"\n"
                or _stable_bytes(import_root / "v4-reconciliation-binding.json", label="v5 imported V4 receipt binding") != canonical(binding) + b"\n"):
            raise ValueError("Revision-gain live v5 imported Grok receipt binding drifted")
        pilot._read_verified_receipt(import_root / "verified-receipt.json", expected_event_id=event_id, expected_phase="revision_generation")
        descendant = imports / "descendants" / f"{event_id}.md"
        if _stable_bytes(descendant, label="v5 imported descendant") != _stable_bytes(V4_DERIVED_RUN_ROOT / f"descendants/{event_id}.md", label="immutable v4 descendant"):
            raise ValueError("Revision-gain live v5 imported descendant drifted")
        source = pilot.contract()["sources"]["items"][event["source_item_id"]]
        feedback = None
        if event["guidance_arm"] == "cwr_guided":
            feedback_event = event["cwr_feedback_event_id"]
            source_root, raw, source_value = _pinned_v3_sol_feedback(feedback_event)
            feedback_path = imports / "cwr_feedback" / f"{feedback_event}.json"
            expected_feedback = {"format_version": 1, "study_id": STUDY_ID, "kind": "v5_imported_sol_lifecycle_authority", "event_id": feedback_event, "phase": "cwr_feedback", "provider_model": "gpt-5.6-sol", "reasoning": "high", "tools_enabled": False, "native_endpoint_contact_cardinality": "unproven", "source_receipt": {"root": str(source_root), "bytes": len(raw), "sha256": _sha256(raw)}, "response": source_value["response"]}
            if _stable_bytes(feedback_path, label="v5 imported Sol feedback") != canonical(expected_feedback) + b"\n":
                raise ValueError("Revision-gain live v5 imported Sol feedback drifted")
            pilot._read_verified_receipt(feedback_path, expected_event_id=feedback_event, expected_phase="cwr_feedback")
            feedback_paths[feedback_event] = feedback_path
            feedback = {"event_id": feedback_event, "verified_receipt": _commitment(output_root, feedback_path)}
        records.append({"event_id": event_id, "source": {"item_id": event["source_item_id"], "source.md": source["source.md"], "prompt.md": source["prompt.md"]}, "parent": None, "descendant": _commitment(output_root, descendant), "generator": {"model": "grok-4.6", "reasoning": "high", "tools_enabled": False}, "generator_receipt": _commitment(output_root, import_root / "verified-receipt.json"), "cwr_feedback": feedback})
    if len(records) != len(PREDECESSOR_TERMINAL_CELLS):
        raise ValueError("Revision-gain live v5 requires all four imported cycle-one records")
    return records, feedback_paths
...
def _successor_event_id(run_root: Path, phase: str, event_id: str) -> str:
    ...
    return f"exec-v4-{_sha256(token)[:24]}"
...
def _read_outbound_payload(root: Path, *, run_root: Path, phase: str, event_id: str) -> tuple[dict[str, Any], bytes]:
    ...
    if canonical(value) + b"\n" != raw or value != expected:
        raise ValueError("Revision-gain live v4 outbound successor identity or payload drifted")
    return identity, raw
...
def _underlying_response_schema(*, root: Path, prepared: Mapping[str, Any]) -> dict[str, Any]:
    ...
    if not isinstance(schema, Mapping) or "$schema_version" in schema:
        raise ValueError("Revision-gain live v4 underlying pilot response schema is invalid")
    return dict(schema)


def _decorate_adapter_schema(underlying: Mapping[str, Any]) -> dict[str, Any]:
    if "$schema_version" in underlying:
        raise ValueError("Revision-gain live v4 underlying schema is already adapter-decorated")
    ...
    if decorated.get("$schema_version") != 1 or set(decorated) != {"$schema_version", *underlying} or {key: value for key, value in decorated.items() if key != "$schema_version"} != dict(underlying):
        raise ValueError("Revision-gain live v4 schema decoration changed pilot semantics")
    return decorated
...
def _read_adapter_schema(*, run_root: Path, root: Path, prepared: Mapping[str, Any]) -> dict[str, Any]:
    ...
    expected = {"format_version": 1, "study_id": STUDY_ID, "kind": "adapter_schema_decoration", "phase": prepared["phase"], "underlying_pilot_response_schema": underlying, "underlying_pilot_response_schema_sha256": _sha256(canonical(underlying)), "adapter_output_schema": adapter_schema, "adapter_output_schema_sha256": _sha256(canonical(adapter_schema))}
    if canonical(value) + b"\n" != raw or value != expected:
        raise ValueError("Revision-gain live v4 adapter schema binding drifted")
...
def prepare_one(*, run_root: Path, source_root: Path, phase: str, event_id: str, acknowledgement_sha256: str, queue_root: Path = DEFAULT_QUEUE_ROOT, lineage_records: list[Mapping[str, Any]] | None = None, feedback_receipt_path: Path | None = None, completed_feedback_receipt_paths: list[Path] | None = None, target_root: Path | None = None, target_manifest_path: Path | None = None) -> dict[str, Any]:
    run_root = _validate_v5_continuation_root(Path(run_root), label="prepared work root")
    pilot = _imported_sol_reader(_pilot())
    ...
    scheduled_revision = next((row for row in pilot.revision_schedule() if row["event_id"] == event_id or row.get("cwr_feedback_event_id") == event_id), None)
    if scheduled_revision and scheduled_revision["cycle"] == 2:
        if lineage_records is None:
            lineage_records, imported_feedback = import_cycle_one(output_root=run_root)
            feedback_receipt_path = imported_feedback.get(event_id, feedback_receipt_path)
        if phase == "revision_generation" and scheduled_revision["guidance_arm"] == "cwr_guided" and feedback_receipt_path is None:
            raise ValueError("Revision-gain live v5 guided cycle-two revision requires imported Sol feedback")
...
def _receipt_from_control(*, pilot: ModuleType, root: Path, prepared: Mapping[str, Any], route: Mapping[str, Any], control_raw: bytes, payload_override: bytes | None = None) -> dict[str, Any]:
    ...
    payload = payload_override if payload_override is not None else _read_outbound_payload(root, run_root=Path(prepared["work_root"]), phase=prepared["phase"], event_id=prepared["event_id"])[1]
    ...
    if prepared["provider_model"] == "grok-4.6":
        ...
        if set(runtime) != required or runtime["adapter_version"] != 1 or runtime["requested_model"] != prepared["provider_model"] or runtime["reported_model"] != GROK_REPORTED_MODEL or runtime["requested_reasoning_effort"] != prepared["reasoning"] or runtime["reasoning_attested"] is not False or runtime["identity_evidence"] != "requested_only" or runtime["observed_turns"] != 1 or runtime["nonvisual_max_turns"] != 1 or any(not isinstance(runtime[key], str) or not re.fullmatch(r"[0-9a-f]{64}", runtime[key]) for key in ("session_id_hash", "request_id_hash", "envelope_hash", "command_identity_hash", "subscription_receipt_hash")) or not isinstance(runtime["command_identity"], Mapping) or dict(runtime["command_identity"]) != route.get("grok_command_identity") or runtime["request_id_hash"] == runtime["session_id_hash"] or runtime["subscription_receipt_hash"] != route_proof.get("route_receipt_sha256"):
...
def _write_grok_native_binding(*, root: Path, run_root: Path, native: Mapping[str, Any], route: Mapping[str, Any], control_raw: bytes) -> None:
    outbound = _commitment(run_root, root / "outbound-payload.json")
    _state, result = _control_from_adapter(control_raw)
    runtime_identity = result["runtime"]["command_identity"] if result is not None else None
    runtime_identity_hash = result["runtime"].get("command_identity_hash") if result is not None else None
    if (not isinstance(runtime_identity, Mapping) or dict(runtime_identity) != route.get("grok_command_identity")
            or not isinstance(runtime_identity_hash, str) or not re.fullmatch(r"[0-9a-f]{64}", runtime_identity_hash)):
        raise ValueError("Revision-gain live v4 Grok runtime command identity is not the governed route identity")
    value = {"format_version": 1, "study_id": STUDY_ID, "kind": "raw_rederived_grok_native_receipt", "outbound_payload": outbound, "runtime_command_identity": dict(runtime_identity), "observed_command_identity_hash": runtime_identity_hash, "native_receipt": dict(native)}
    path = root / "adapter-native-binding.json"
    if path.exists():
        raw = _stable_bytes(path, label="existing raw-rederived Grok receipt binding")
        if canonical(value) + b"\n" != raw:
            raise ValueError("Revision-gain live v4 existing Grok raw receipt binding drifted")
        return
...
def _verify_grok_native_binding(*, pilot: ModuleType, root: Path, prepared: Mapping[str, Any], verified: Mapping[str, Any]) -> None:
    ...
    value = json.loads(raw.decode("utf-8"))
    run_root = Path(prepared["work_root"])
    state = "reconciled" if (root / "reconciled-verified-receipt.json").exists() else "settled"
    admission, _ = _read_admission(root=root, phase=prepared["phase"], event_id=prepared["event_id"], inventory_state=state)
    _broker, route = _reauth_route(pilot=pilot, run_root=run_root, phase=prepared["phase"], event_id=prepared["event_id"], binding=admission["route_evidence"])
    control_raw = _stored_adapter_stdout(root)
    actual = _receipt_from_control(pilot=pilot, root=root, prepared=prepared, route=route, control_raw=control_raw)
    _state, result = _control_from_adapter(control_raw)
    expected = {"format_version": 1, "study_id": STUDY_ID, "kind": "raw_rederived_grok_native_receipt", "outbound_payload": _commitment(run_root, root / "outbound-payload.json"), "runtime_command_identity": result["runtime"]["command_identity"] if result is not None else None, "observed_command_identity_hash": result["runtime"]["command_identity_hash"] if result is not None else None, "native_receipt": actual}
    ...
    if canonical(value) + b"\n" != raw or value != expected or verified.get("native_receipt") != normalized:
        raise ValueError("Revision-gain live v4 Grok receipt authority is not bound to raw stdout and successor payload")
...
def _validate_receipt_authority_state(*, root: Path, authority_path: Path, phase: str) -> None:
    ...
    expected_reconciliation = {"format_version": 1, "study_id": PILOT_STUDY_ID,
                               ...
                               "acknowledgement_sha256": contract()["authorized_acknowledgement_sha256"],
    ...
    if (canonical(terminal) + b"\n" != terminal_raw or terminal != expected_terminal
            ...
            or acknowledgement != contract()["authorized_acknowledgement_sha256"]):
...
def execute_one(*, run_root: Path, phase: str, event_id: str, allow_remote: bool) -> dict[str, Any]:
    ...
    try:
        ...
        if prepared["provider_model"] == "grok-4.6":
            native = _receipt_from_control(pilot=pilot, root=root, prepared=prepared, route=route, control_raw=completed.stdout)
            _write_grok_native_binding(root=root, run_root=run_root, native=native, route=route, control_raw=completed.stdout)
...
def reconcile_existing_receipt(*, run_root: Path, phase: str, event_id: str, reconciliation_acknowledgement_sha256: str) -> dict[str, Any]:
    ...
    if reconciliation_acknowledgement_sha256 != contract()["authorized_acknowledgement_sha256"]:
        raise ValueError("Revision-gain live v4 reconciliation acknowledgement is invalid")
    pilot = _pilot()
    root = _cell_root(Path(run_root), phase, event_id)
    try:
        admission, prepared = _read_admission(root=root, phase=phase, event_id=event_id, inventory_state="reconcile_pending")
        write_reconciliation = True
    except ValueError:
        admission, prepared = _read_admission(root=root, phase=phase, event_id=event_id, inventory_state="reconciling")
        write_reconciliation = False
    prepared = json.loads(_stable_bytes(root / "prepared-cell.json", label="prepared cell").decode("utf-8"))
    if not isinstance(prepared, Mapping):
        raise ValueError("Revision-gain live v4 reconciliation prepared cell is invalid")
    _validate_cell_inventory(root=root, state="reconcile_pending" if write_reconciliation else "reconciling", phase=phase, provider_model=prepared.get("provider_model"))
    control_raw = _stored_adapter_stdout(root)
    ...
    _broker, route = _reauth_route(pilot=pilot, run_root=Path(run_root), phase=phase, event_id=event_id, binding=admission["route_evidence"])
    native_receipt = None
    if prepared["provider_model"] == "grok-4.6":
        native_receipt = _receipt_from_control(pilot=pilot, root=root, prepared=prepared, route=route, control_raw=control_raw)
    else:
    ...
    _validate_cell_inventory(root=root, state="reconciling", phase=phase, provider_model=prepared["provider_model"])
    if prepared["provider_model"] == "grok-4.6":
        assert native_receipt is not None
        _write_grok_native_binding(root=root, run_root=Path(run_root), native=native_receipt, route=route, control_raw=control_raw)
...
def freeze_targets(*, run_root: Path, source_root: Path, lineage_records: list[Mapping[str, Any]], target_root: Path) -> dict[str, Any]:
    run_root = _validate_v5_continuation_root(Path(run_root), label="target work root")
    target_root = _validate_v5_continuation_root(Path(target_root), label="target output root")
    return _imported_sol_reader(_pilot()).prepare_targets(work_root=run_root, target_root=target_root, source_root=Path(source_root), revision_records=lineage_records)
...
def _projection_boundary(*, root: Path, prepared: Mapping[str, Any], event_id: str) -> dict[str, Any]:
    ...
    if root.resolve() != _cell_root(run_root, "blind_endpoint_judgment", event_id).resolve():
        raise ValueError("Revision-gain live v4 endpoint receipt is outside its prepared run root")
    ...
    if not isinstance(frozen_sha256, str) or _sha256(_stable_bytes(run_root / "frozen-inputs.json", label="projection frozen inputs")) != frozen_sha256:
        raise ValueError("Revision-gain live v4 endpoint frozen-input boundary drifted")
    ...
    if not isinstance(endpoint_target, Mapping) or set(endpoint_target) != {"target_root", "target_manifest", "blind_target_id", "target"}:
        raise ValueError("Revision-gain live v4 endpoint target boundary is invalid")
    ...
    if not isinstance(manifest, Mapping) or set(manifest) != {"path", "bytes", "sha256"}:
        raise ValueError("Revision-gain live v4 endpoint target manifest commitment is invalid")
    ...
    if not isinstance(relative, str) or not relative or Path(relative).is_absolute() or any(part in {"", ".", ".."} for part in Path(relative).parts):
        raise ValueError("Revision-gain live v4 endpoint target manifest path is unsafe")
    actual_manifest = _commitment(target_root, target_root / relative)
    if actual_manifest != dict(manifest):
        raise ValueError("Revision-gain live v4 endpoint target manifest commitment drifted")
    ...
    if manifest_value.get("work_root") != str(run_root) or manifest_value.get("frozen_manifest_sha256") != frozen_sha256:
        raise ValueError("Revision-gain live v4 endpoint target manifest crosses a run or frozen-input boundary")
...
def _require_single_projection_boundary(boundaries: list[Mapping[str, Any]]) -> dict[str, Any]:
    if not boundaries:
        raise ValueError("Revision-gain live v4 endpoint projection has no boundary")
    ...
    if set(first) != required or any(dict(boundary) != first for boundary in boundaries[1:]):
        raise ValueError("Revision-gain live v4 endpoint projection splices multiple runs or target freezes")
    return first
