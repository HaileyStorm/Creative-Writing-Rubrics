...
def test_frozen_package_binds_source_lifecycle_parent_runtime_leaves_and_pairs():
    value = study()
    assert value.validate_package() == {"study_id": value.STUDY_ID, "source_commit": value.SOURCE_COMMIT, "slots": 15, "provider_calls": 0, "image_slots": 0}
    assert value.contract()["parent_result"] == value.PARENT_RESULT
    assert value.contract()["template_executor"] == value.TEMPLATE_EXECUTOR
    assert value.contract()["prompt_hashes"] == value.PROMPT_HASHES
    assert value.contract()["prompt_aggregate_sha256"] == value.PROMPT_AGGREGATE_SHA256
    assert value.contract()["compiled_leaf_hash"] == value.COMPILED_LEAF_HASH


def test_exact_text_only_schedule_is_blind_and_lifecycle_is_cached():
    value = study()
    slots = value.build_schedule()
    assert len(slots) == len({slot["slot_id"] for slot in slots}) == 15
    assert len({slot["logical_sample_id"] for slot in slots}) == len({slot["run_id"] for slot in slots}) == 15
    assert [slot["slot_id"] for slot in slots] == [f"l2materialholdoutexec-v1-{number:03d}" for number in range(1, 16)]
    assert {slot["case_id"] for slot in slots} == {"h01", "h02", "h03", "h04", "h05"}
    assert {slot["prompt_sha256"] for slot in slots} == set(value.PROMPT_HASHES.values())
...
def test_schedule_never_opens_expected_ledger(monkeypatch):
    ...
    assert len(value.build_schedule()) == 15
...
def test_provider_free_dry_run_has_schema_manifest_auth_disclosure_and_no_image(private_root):
    ...
    assert report["provider_calls"] == 0 and report["planned_slots"] == 15 and report["image_slots"] == 0
    ...
    assert len(disclosure["slots"]) == 15 and all(item["attachment"] is None for item in disclosure["slots"])
    assert not list((private_root / "inputs").glob("*.png"))
    assert len(value._validated_schedule(private_root)) == 15
...
def test_claim_contention_and_failed_contact_terminalize_without_retry(private_root):
    ...
    try:
        ...
        assert sum(item["state"] == "blocked_before_dispatch" for item in terminals[1:]) == 14
    finally:
        shutil.rmtree(root, ignore_errors=True)
...
def test_full_histogram_external_boolean_settlement_and_aggregate_only_publication(private_root):
    ...
    assert settlement["decision"] == "PROMOTION_REVIEW_ELIGIBLE"
    assert settlement["aggregate_cells"] == {"zero_of_three": 0, "one_of_three": 0, "two_of_three": 0, "three_of_three": 5, "total": 5}
    assert settlement["normalization_events"] == 1 and "verdict_counts" not in public
    h05 = next(slot for slot in slots if slot["case_id"] == "h05")
    miss, _ = value._aggregate_test_only(schedule=slots, records=records, scorer=lambda slot, _record: slot["slot_id"] != h05["slot_id"])
    assert miss["decision"] == "NO_GO"
    assert miss["aggregate_cells"] == {"zero_of_three": 0, "one_of_three": 0, "two_of_three": 1, "three_of_three": 4, "total": 5}
...
def test_complete_execution_settles_claim_bound_aggregate_only_and_mutation_rejects(private_root):
    ...
    value.dry_run(private_root, auth_call=fake_auth)
    contacts = []
    assert value.execute(private_root, allow_remote=True, acknowledged_zero_incremental_charge=True, runner_call=accepted(contacts), auth_call=fake_auth)["completed_slots"] == 15
    ...
    assert len(contacts) == 15 and settlement["decision"] == "PROMOTION_REVIEW_ELIGIBLE"
