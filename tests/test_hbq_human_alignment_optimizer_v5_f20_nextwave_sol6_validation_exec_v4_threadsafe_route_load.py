...
PACKAGE = ROOT / "evaluation-results" / "hbq-human-alignment-optimizer-v5-f20-nextwave-sol6-validation-exec-v4-threadsafe-route-load"
ACK = "a" * 64
...
def test_concurrent_configure_installs_one_route_wrapper_without_self_deadlock():
    value = module()
    class RacingBase(ModuleType):
        def __init__(self):
            super().__init__("_race_base")
            self._configured = False
            self._marker_lock = threading.Lock()
            self.first_unconfigured_read = threading.Event()
            self.second_unconfigured_read = threading.Event()
            self.release_first_read = threading.Event()

        @property
        def _v4_configured(self):
            with self._marker_lock:
                configured = self._configured
                if not configured:
                    if not self.first_unconfigured_read.is_set():
                        self.first_unconfigured_read.set()
                        wait_for_release = True
                    else:
                        self.second_unconfigured_read.set()
                        self.release_first_read.set()
                        wait_for_release = False
                else:
                    wait_for_release = False
            if wait_for_release:
                self.release_first_read.wait(1)
            return configured

        @_v4_configured.setter
        def _v4_configured(self, configured):
    ...
    def prior_racy_route(*_args, **_kwargs):
        with state_lock:
            if state["active"]:
                raise RuntimeError("shared sys.modules import race")
            state["active"] += 1
        ...
        with state_lock:
            state["active"] -= 1
        return {}, {}, None
    base._route = prior_racy_route
    base._prepared = lambda *_args: {"prepared.json": b'{"source":{}}'}
    base.canonical = value.canonical
    schedule = {"schedule_sha256": "s"}
    bindings = {"collector_sha256": "c", "result_sha256": "r", "result_internal_sha256": "i"}
    configure_barrier = threading.Barrier(3)
    configure_failures = []

    def configure():
        configure_barrier.wait()
        try:
            value._configure(base, (), schedule, bindings)
        except BaseException as error:
            configure_failures.append(error)

    installers = [threading.Thread(target=configure, daemon=True) for _ in range(2)]
    for installer in installers:
        installer.start()
    configure_barrier.wait()
    assert base.first_unconfigured_read.wait(1)
    try:
        assert not base.second_unconfigured_read.wait(0.2)
    finally:
        base.release_first_read.set()
    for installer in installers:
        installer.join(2)
    assert not any(installer.is_alive() for installer in installers)
    assert not configure_failures and base._v4_configured
    ...
    assert not failures and state == {"active": 0, "maximum": 1, "calls": 2}
