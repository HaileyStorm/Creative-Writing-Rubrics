...
PACKAGE = ROOT / "evaluation-results" / "hbq-human-alignment-optimizer-v7-desc16-referent-evidence-development-optimizer-v1"


def module():
    spec = importlib.util.spec_from_file_location("_desc16_referent_evidence_optimizer", PACKAGE / "analyzer.py")
...
def fixture(value, levels=None):
    ...
    schedule = {"format_version": 1, "study_id": value.EXECUTOR_ID, "kind": "frozen_desc16_referent_evidence_grok_development_execution_schedule", "geometry": {"candidates": 4, "development_groups": 7, "development_items": 13, "grok_cells": 52, "sol_cells": 0, "confirmation_cells": 0}, "frozen_schedule_sha256": value.FREEZE_SCHEDULE_SHA256, "schedule_sha256": "s" * 64, "cells": cells}
    collector = {"format_version": 1, "study_id": value.EXECUTOR_ID, "kind": "complete_52_desc16_referent_evidence_grok_receipts_cardinality_unproven", "schedule_sha256": schedule["schedule_sha256"], "authorization_acknowledgement_sha256": "a" * 64, "route": {"route": "grok"}, "route_evidence": {"fixture": True}, "cells": receipts, "native_endpoint_contact_cardinality": "unproven", "provider_calls_made": None, "process_launches": 52}
...
def projection(value, metrics):
    return {"format_version": 1, "study_id": value.STUDY_ID, "kind": "desc16_referent_evidence_grok_development_equal_group_projection", "authority": value.ENGINE.AUTHORITY, "metrics": metrics, "source_execution": {"collector_sha256": "c" * 64, "executor_sha256": value.ENGINE.EXECUTOR_HASHES["executor.py"], "frozen_schedule_sha256": value.FREEZE_SCHEDULE_SHA256, "schedule_sha256": "s" * 64}}
...
def test_package_binds_desc16_freeze_and_defers_development_libraries():
    value = module()
    contract = value.validate_package()
    assert contract["pinned_freeze"]["commit"] == "2fb8b1e1dd9acc0d0869c3ebf51c384653ac3ee5"
    assert contract["pinned_freeze"]["schedule_sha256"] == value.FREEZE_SCHEDULE_SHA256
...
def test_projection_independently_recomputes_52_native_cells_with_equal_group_weighting():
    value = module()
...
def test_real_optuna_and_dspy_freeze_only_child20_relative_qualifiers():
    value = module()
...
def test_external_collector_is_accepted_only_via_receipt_replay(tmp_path, monkeypatch):
    value = module()
    ...
    fake_executor = SimpleNamespace(
        frozen_schedule=lambda _root: schedule,
        replay_collector=lambda **kwargs: {"cells": 52, "collector_sha256": value.sha256(roots["collector_path"].read_bytes()), "equal_group_projection_ready": True, "native_endpoint_contact_cardinality": "unproven", "provider_calls_made": None},
    )
...
def test_qualification_rejects_fabricated_objective_and_empty_qualifiers_make_zero_sol_calls():
    value = module()
    groups = {f"group-{number}": 1.0 for number in range(7)}
