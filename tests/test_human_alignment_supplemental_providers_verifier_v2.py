...
def test_verify_run_preserves_mixed_newline_inputs_and_rejects_prompt_tampering(monkeypatch, tmp_path):
    ...
    source, context, binary = (path.read_bytes().decode("utf-8-sig") for path in (source_path, context_path, binary_path))
    ...
    frozen = {"input_commitments": {"development": {"dev-0": {"source.md": bound(source_path), "prompt.md": bound(context_path), "task-contract.json": bound(contract_path)}}, "confirmatory": {}}, "primary_runtime_files": {"prompts/judge/BINARY_EVALUATION_PROMPT.md": bound(binary_path), "schema/hbq_judge_response.schema.json": schema}}
    ...
    monkeypatch.setattr(verifier, "prompts_dir", lambda: prompt_root)
