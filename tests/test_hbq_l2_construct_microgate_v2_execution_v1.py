...
ROOT = book_root() / "evaluation-results" / "hbq-l2-construct-microgate-v2-execution-v1"
def study():
...
def accepted(contacts, verdict="YES"):
    def call(command, **kwargs):
        ...
        output.write_text(json.dumps({"verdicts":[{"question_id":question_id,"verdict":verdict,"confidence":0.8,"evidence":[{"kind":"summary","reference":"synthetic","exact_quote":None,"summary":"Grounded."}],"note":"test"}]}), encoding="utf-8")
...
def test_geometry_bindings_and_nonvoting_failed_lineage():
    ...
    failed=s.contract()["failed_execution"]; assert failed["non_voting"] is True and failed["accepted_slots"] == 6 and failed["later_slots_blocked_before_dispatch"] == 17
...
def test_prompts_are_production_shaped_and_ledger_blind():
    ...
    for slot in slots:
        ...
        expected=production_runner._render_prompt(binary_prompt=s._frozen_binary_prompt(),artifact={"name":artifact["artifact_name"],"text":artifact["text"]},contexts=[],bundle_id=artifact["bundle_id"],artifact_id="public-synthetic-artifact",questions=[question],task_contract_context=s._task_context(artifact))
    ...
    assert all(x["slot_id"].startswith("l2microexec-v2-") for x in slots)
    assert s.sha256_bytes(s.canonical_json({x["slot_id"]:x["prompt_sha256"] for x in slots})) == "bc182dc2a6c2d66cc1d642eb399fe9be28657ae87f1995d03b140e8bfffd81d9"
...
def test_atomic_claim_and_no_old_root_reuse(private_root):
    ...
    assert not (private_root/"runs"/"l2microexec-v2-001").exists()
