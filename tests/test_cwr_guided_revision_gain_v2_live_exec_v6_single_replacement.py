...
def test_forged_admission_or_extra_run_artifact_rejects_before_launch(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    run = tmp_path / "admission"; mod.prepare_replacement(run_root=run, source_root=source, acknowledgement_sha256=mod.ACK, queue_root=tmp_path / "queue")
    root = mod._replacement_root(run); forged = json.loads((root / "replacement-admission.json").read_text(encoding="utf-8")); forged["provider_calls_made"] = 999
    (root / "replacement-admission.json").write_bytes(mod.canonical(forged) + b"\n")
    ...
    assert launches == [] and not (root / "launch-intent.json").exists()
