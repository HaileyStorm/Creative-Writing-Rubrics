...
SOURCE = DIRECTORY / "sol_import_serialized_successor.py"
FREEZE_ROOT = Path(r"C:\Users\Haile\Documents\cwr-wpb-pilot-source-freeze-20260904-r3")
RECONCILIATION = Path(r"C:\Users\Haile\Documents\cwr-wpb-sol-import-serialized-reconciliation-20260912-r1\reconciliation.json")
...
def load(path: Path = SOURCE, name: str = "wpb_import_serialized_successor") -> Any:
...
def _support() -> tuple[Any, Any, Any]:
    pending_tests = _load_module(PENDING_TEST, "wpb_import_serialized_pending_tests")
    support = _load_module(SUPPORT, "wpb_import_serialized_support")
    return pending_tests, support, support.load(support.V12_TEST, "wpb_import_serialized_route")
...
def test_four_completed_cells_are_excluded_and_reconciliation_is_hash_bound() -> None:
    ...
    assert len(ids) == 125 and not set(ids) & set(value.PREFIX)
    ...
    assert reconciliation["remaining_logical_cells"] == 125 and reconciliation["automatic_resend_authorized"] is False


def test_wrong_import_helper_pin_is_rejected(monkeypatch: pytest.MonkeyPatch) -> None:
    value = load(); monkeypatch.setattr(value, "IMPORT_SERIALIZER_SHA256", "0" * 64)
    with pytest.raises(ValueError, match="helper source drifted"):
        value._import_serializer()
...
def test_full_synthetic_13_batch_joined_129_report(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    verifier, verifier_sha, review, review_sha, context = pending_tests.fixture(value._previous(), tmp_path)
    root, queue = tmp_path / "import-serialized-success", tmp_path / "queue"; queue.mkdir()
    reconciliation_sha = hashlib.sha256(RECONCILIATION.read_bytes()).hexdigest()
    created = value.create_campaign(campaign_root=root, reconciliation_path=RECONCILIATION, expected_reconciliation_sha256=reconciliation_sha,
    ...
    assert created["logical_cells"] == 125
    ...
    def run(command: list[str], **arguments: Any) -> subprocess.CompletedProcess[bytes]:
        ...
        answer = support.answer(value._previous()._load_legacy()._load_pending()._load_strict())
        final = json.dumps(answer, sort_keys=True, separators=(",", ":"))
        message = Path(command[command.index("--output-last-message") + 1]); message.parent.mkdir(parents=True, exist_ok=True)
        message.write_text(final, encoding="utf-8")
        events = b"".join(json.dumps(event, separators=(",", ":")).encode() + b"\n" for event in (
            {"type": "thread.started", "thread_id": f"import-serialized-{output.name}"}, {"type": "turn.started"},
    ...
    def prepare(number: int, cells: list[str]) -> list[str]:
        ...
        batch = root / "batches" / f"{number:04d}"
        assert (batch / "strict-native-schema.json").is_file()
        return result["prepared_cells"]

    def dispatch_and_settle(number: int, cells: list[str]) -> None:
        material = value.review_material(campaign_root=root, batch_number=number); review_path = tmp_path / f"import-serialized-review-{number}.json"
    ...
    for number in range(1, 14):
        current = cells[(number - 1) * 10:number * 10]
        dispatch_and_settle(number, prepare(number, current))
    ...
    assert report["new_measurement_count"] == 125 and report["preserved_completed_cells"] == list(value.PREFIX)
