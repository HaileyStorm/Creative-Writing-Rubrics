...
PACKAGE = ROOT / "evaluation-results" / "hbq-human-alignment-optimizer-v6-desc13-lower-step-sol-nonwinner-extension-exec-v1"
ACK = "a" * 64
...
def fake_schedule(value):
    ...
    candidates = [
        ...
        *({"candidate_id": child} for child in value.EXTENSION_CHILDREN),
        {"candidate_id": "broader-nextwave-18-construct_framing-referent-resolution"},
    ]
    cells = []
    for group in groups:
...
def test_committed_result_replay_preserves_observed_grok_payload_bytes_for_each_extension_child(monkeypatch, tmp_path: Path):
    ...
    rows = value._rows(schedule=rebuilt, targets=targets, grok_execution_root=output)
    assert len(rows) == 21
    assert {row["candidate_id"] for row in rows} == set(value.EXTENSION_CHILDREN)
    assert all(row["source_cell_id"] is not None for row in rows)
...
def test_prepared_inventory_rejects_extra_or_non_directory_cell_entries(tmp_path: Path):
    value = module()
    rows = ({"cell_id": "one"}, {"cell_id": "two"})
    ...
    for row in rows:
        (root / row["cell_id"]).mkdir()
    value._prepared_inventory(Base(), root, rows)
    (root / "extra").mkdir()
    with pytest.raises(ValueError, match="inventory drifted"):
        value._prepared_inventory(Base(), root, rows)


def test_rows_reject_each_child_reusing_one_group_item_pair(tmp_path: Path):
    value = module(); schedule = fake_schedule(value)
    output = tmp_path / "grok"; output.mkdir()
    for row in schedule["cells"]:
        root = output / row["cell_id"]; root.mkdir()
        (root / "outbound-payload.json").write_bytes(base64.b64decode(row["payload_base64"]))
        if row["candidate_id"] in value.EXTENSION_CHILDREN:
            row["prompt_group_id"] = "group-0"; row["item_id"] = "item-0"
    targets = {f"item-{index}": {dimension: 2.0 for dimension in DIMS} for index in range(7)}
    with pytest.raises(ValueError, match="geometry drifted"):
        value._rows(schedule=schedule, targets=targets, grok_execution_root=output)
...
def test_prepare_is_zero_contact_and_fake_wave_stays_at_two_lanes(monkeypatch, tmp_path: Path):
    ...
    class Base:
        ...
        @staticmethod
        def _inventory(root):
            assert {path.name for path in root.iterdir()} == {"outbound-payload.json", "response-schema.json", "prepared.json"}
...
def test_wave_rejects_later_cell_residue_before_any_fake_call(monkeypatch, tmp_path: Path):
    value = module()
    rows = tuple({"cell_id": f"sol-{index}", "candidate_id": "candidate", "payload_base64": base64.b64encode(value.canonical({"response_schema": {"type": "object"}})).decode("ascii"), "payload_sha256": "x", "payload_parity": "fixture", "target": {dimension: 2.0 for dimension in DIMS}} for index in range(21))
    resolution = {"rows": rows, "schedule": {"schedule_sha256": "schedule"}, "bindings": {}}
    monkeypatch.setattr(value, "_resolve", lambda **_kwargs: resolution)
    calls = {"count": 0}
    ...
    class Base:
        ...
        @staticmethod
        def _prepared(_row, payload, schema, _target, _route, _evidence, _ack):
            return {"outbound-payload.json": payload, "response-schema.json": schema, "prepared.json": b"{}\n"}
        ...
        @staticmethod
        def _inventory(root):
            if {path.name for path in root.iterdir()} != {"outbound-payload.json", "response-schema.json", "prepared.json"}:
                raise ValueError("fixture inventory drifted")

        @staticmethod
        def execute_one(**_kwargs): calls["count"] += 1

    monkeypatch.setattr(value, "_configured_base", lambda _resolution: Base())
    source_roots = {name: tmp_path / name for name in ("candidate", "development", "normalized", "materialization", "successor", "csv", "grok", "collector", "result")}
    for path in source_roots.values(): path.mkdir()
    queue = tmp_path / "queue"; queue.mkdir()
    common = {"output_root": tmp_path / "sol", "queue_root": queue, "authorization_acknowledgement_sha256": ACK, "candidate_freeze_root": source_roots["candidate"], "development_freeze_root": source_roots["development"], "normalized_root": source_roots["normalized"], "materialization_root": source_roots["materialization"], "frozen_successor_path": source_roots["successor"], "hanna_csv_path": source_roots["csv"], "grok_execution_root": source_roots["grok"], "grok_collector_path": source_roots["collector"], "grok_result_path": source_roots["result"]}
    value.prepare_all(**common)
    (common["output_root"] / "sol-20" / "extra").write_text("residue", encoding="utf-8")
    with pytest.raises(ValueError, match="fixture inventory drifted"):
        value.execute_wave(**common, allow_remote=True)
    assert calls["count"] == 0


def test_prepare_rejects_an_output_root_inside_the_public_checkout(monkeypatch, tmp_path: Path):
    value = module()
    monkeypatch.setattr(value, "_resolve", lambda **_kwargs: pytest.fail("unsafe output must reject before resolution"))
    source_roots = {name: tmp_path / name for name in ("candidate", "development", "normalized", "materialization", "successor", "csv", "grok", "collector", "result")}
    for path in source_roots.values():
        path.mkdir()
    queue = tmp_path / "queue"; queue.mkdir()
    with pytest.raises(ValueError, match="disjoint"):
        value.prepare_all(
            output_root=PACKAGE / "unsafe-output", queue_root=queue, authorization_acknowledgement_sha256=ACK,
            candidate_freeze_root=source_roots["candidate"], development_freeze_root=source_roots["development"], normalized_root=source_roots["normalized"],
            materialization_root=source_roots["materialization"], frozen_successor_path=source_roots["successor"], hanna_csv_path=source_roots["csv"],
            grok_execution_root=source_roots["grok"], grok_collector_path=source_roots["collector"], grok_result_path=source_roots["result"],
        )


def test_terminal_evidence_never_resumes_or_adopts_a_prior_cell(tmp_path: Path):
    value = module(); row = {"cell_id": "cell"}; root = tmp_path / "cell"; root.mkdir()
    (root / "launch-intent.json").write_bytes(value.canonical({}))
    with pytest.raises(ValueError, match="fresh root or manual reconciliation"):
        value._terminal_state(object(), root, row)
    (root / "result.json").write_bytes(value.canonical({"forged": True}))
    with pytest.raises(ValueError, match="fresh root or manual reconciliation"):
        value._terminal_state(object(), root, row)
...
def test_result_replay_mismatch_is_rejected_and_package_has_no_runtime_optimizer_imports(monkeypatch, tmp_path: Path):
    ...
    class Analyzer:
        @staticmethod
        def replay(**_kwargs): return {**result, "selection": {"candidate_id": "forged-child"}}
