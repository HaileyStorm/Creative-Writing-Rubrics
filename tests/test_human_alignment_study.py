...
def test_confirmation_gate_binds_both_analysis_hashes():
    ...
    assert "development_analysis_manifest_sha256" in text and "development_analysis_summary_sha256" in text and 'checkpoint.get("provider",{}).get("reported",{}).get("session_id")' in analyzer
...
def test_executed_repeat_session_rejection_and_eleven_item_aggregation(tmp_path,monkeypatch):
    ...
    with pytest.raises(ValueError,match="distinct nonblank"): analysis.verify_phase_runs(tmp_path,frozen,"repeatability")
