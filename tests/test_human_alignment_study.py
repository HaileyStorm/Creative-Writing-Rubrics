...
def synthetic_external_run(tmp_path, *, session_id="session-1"):
    ...
    configuration={"artifact":{**source,"path":str(inputs/"source.md")},"contexts":[{**prompt,"path":str(inputs/"prompt.md")}],"task_contract":{**task,"path":str(inputs/"task-contract.json"),"contract_id":"hanna"},"weight_profile":_,"bundle_id":"prose.short_story","bundle_version":bundle["version"],"question_ids":ids,"provider":"codex","model":"gpt-5.6-sol","endpoint":None,"api_key_env":None,"temperature":None,"allow_model_mismatch":None,"reasoning":"high","codex_bin":"codex","batch_size":32,"retry_policy":{"batch_attempts":3},"artifact_id":item.item_id,"judge_id":"codex:gpt-5.6-sol","strict_ai":False,"prompts":[{**package["BINARY_EVALUATION_PROMPT.md"],"path":"binary"}],"response_schema":{**package["hbq_judge_response.schema.json"],"path":"schema"},"questions_sha256":hashlib.sha256(_json_bytes(_question_payload(records))).hexdigest(),"compiled_bundle_sha256":hashlib.sha256(_json_bytes(compiled)).hexdigest()}
    folder=tmp_path/"runs"/"development"/item.item_id/"run-01"; (folder/"responses").mkdir(parents=True); run_id="run-synthetic"; study.write_json(folder/"run.json",{"format_version":2,"run_id":run_id,"created_at":"x","config_sha256":hashlib.sha256(_json_bytes(configuration)).hexdigest(),"remote":True,"configuration":configuration}); (folder/"verdicts.jsonl").write_text("".join(json.dumps(row,sort_keys=True)+"\n" for row in verdicts)); previous=None; accumulated=[]
    for batch,offset in enumerate(range(0,len(verdicts),32),1):
        chunk=verdicts[offset:offset+32]; accumulated.extend(chunk); prompt_bytes=f"prompt-{batch}".encode(); message_bytes=f'{{"content":"batch-{batch}"}}'.encode(); message_path=folder/"responses"/f"batch-{batch:04d}.message.json"; message_path.write_bytes(message_bytes); response={"format_version":3,"batch":batch,"question_ids":[row["question_id"] for row in chunk],"prompt_sha256":hashlib.sha256(prompt_bytes).hexdigest(),"response_sha256":hashlib.sha256(message_bytes).hexdigest(),"response_artifact":{"path":message_path.relative_to(folder).as_posix(),"bytes":len(message_bytes),"sha256":hashlib.sha256(message_bytes).hexdigest()},"retry_policy":{"batch_attempts":3},"accepted_attempt":1,"rejected_chain":{"count":0,"head_sha256":None},"previous_checkpoint_sha256":previous,"verdicts_sha256":hashlib.sha256(analysis.verdict_bytes(accumulated)).hexdigest(),"provider":{"reported":{"provider":"openai","model":"gpt-5.6-sol","reasoning_effort":"high","session_id":f"{session_id}-{batch}"}},"normalized_verdicts":chunk}; path=folder/"responses"/f"batch-{batch:04d}.json"; (folder/"responses"/f"batch-{batch:04d}.prompt.txt.gz").write_bytes(__import__("gzip").compress(prompt_bytes,mtime=0)); study.write_json(path,response); previous=hashlib.sha256(path.read_bytes()).hexdigest()
...
def test_executed_synthetic_run_verification_and_score_tamper_rejection(tmp_path):
    ...
    with pytest.raises(ValueError,match="accepted response artifact"): analysis.verify_run(tmp_path,frozen,"development",row,1)
...
def test_legacy_manifest_and_v2_checkpoint_message_fallback(tmp_path):
    frozen,row,folder=synthetic_external_run(tmp_path)
    manifest=json.loads((folder/"run.json").read_text()); manifest["format_version"]=1; manifest["configuration"].pop("retry_policy"); manifest["config_sha256"]=hashlib.sha256(__import__("hbqrs.runner",fromlist=["_json_bytes"])._json_bytes(manifest["configuration"])).hexdigest(); study.write_json(folder/"run.json",manifest)
    previous=None
    for checkpoint in sorted((folder/"responses").glob("batch-[0-9][0-9][0-9][0-9].json")):
        record=json.loads(checkpoint.read_text()); record["format_version"]=2
        for key in ("retry_policy","accepted_attempt","response_artifact","rejected_chain"): record.pop(key)
        record["previous_checkpoint_sha256"]=previous; study.write_json(checkpoint,record); previous=hashlib.sha256(checkpoint.read_bytes()).hexdigest()
    rows,score=analysis.verify_run(tmp_path,frozen,"development",row,1)
    assert len(rows)==179 and score["retry_provenance"]["accepted_attempts"]==[]
def test_rejected_retry_provenance_is_bound_and_not_a_repetition(tmp_path):
    frozen,row,folder=synthetic_external_run(tmp_path)
    checkpoint=sorted((folder/"responses").glob("batch-[0-9][0-9][0-9][0-9].json"))[-1]; record=json.loads(checkpoint.read_text()); rejected=folder/"responses"/"rejected"/f"batch-{record['batch']:04d}"; rejected.mkdir(parents=True); raw=b'{"content":"invalid"}'; raw_path=rejected/"attempt-0001.message.txt"; raw_path.write_bytes(raw)
    attempt={"format_version":2,"batch":record["batch"],"attempt":1,"sequence":1,"previous_rejected_sha256":None,"stage":"grounding","retry_policy":{"batch_attempts":3},"prompt_sha256":record["prompt_sha256"],"raw_content":{"path":raw_path.relative_to(folder).as_posix(),"bytes":len(raw),"sha256":hashlib.sha256(raw).hexdigest()},"provider":None,"error":{"class":"HBQError","message":"fixture"}}
    attempt_path=rejected/"attempt-0001.json"; study.write_json(attempt_path,attempt); record["accepted_attempt"]=2; record["rejected_chain"]={"count":1,"head_sha256":hashlib.sha256(attempt_path.read_bytes()).hexdigest()}; study.write_json(checkpoint,record)
    _,score=analysis.verify_run(tmp_path,frozen,"development",row,1)
    assert score["retry_provenance"]["rejected_attempt_count"]==1 and score["retry_provenance"]["accepted_checkpoint_count"]==6
    raw_path.unlink(); attempt["format_version"]=3; attempt["raw_content"]={"encoding":"utf-8","text":raw.decode(),"bytes":len(raw),"sha256":hashlib.sha256(raw).hexdigest()}; study.write_json(attempt_path,attempt); record["rejected_chain"]={"count":1,"head_sha256":hashlib.sha256(attempt_path.read_bytes()).hexdigest()}; study.write_json(checkpoint,record)
    _,score=analysis.verify_run(tmp_path,frozen,"development",row,1)
    assert score["retry_provenance"]["rejected_attempt_count"]==1
    (rejected/"attempt-0002.message.txt").write_bytes(b"orphan")
    with pytest.raises(ValueError,match="unmatched"): analysis.verify_run(tmp_path,frozen,"development",row,1)
