"""Regression coverage for the Ox Alpha v5 transport successor."""
...
ROOT = book_root() / "evaluation-results" / "hbq-human-alignment-supplemental-providers-ox-alpha-v5"
UNCERTAIN_ROOT = Path(os.environ.get("CWR_OX_V4_UNCERTAIN_ROOT", r"C:\Users\Haile\Documents\cwr-ox-alpha-v4-cap1-pilot-20260821-82d9b09"))
...
study = load("ox_alpha_v5_study", "study.py")
pilot = load("ox_alpha_v5_pilot", "run_transport_pilot.py", {"study": study})
verify = load("ox_alpha_v5_verify", "verify_transport_pilot.py", {"study": study})


def frozen() -> dict:
    return {"cells": [{"cell_id": f"ox-alpha-v5-{number:02d}", "item_id": f"item-{number}", "question_ids": [str(question) for question in range(4)]} for number in range(1, 4)]}


def test_contract_is_four_leaf_serial_cap1_and_score_blind():
    policy = study.CONTRACT["transport_pilot"]
    assert {key: policy[key] for key in ("cells", "batch_size", "question_count", "batch_attempts", "workers", "timeout_seconds", "maximum_http_seconds_exclusive")} == {"cells": 3, "batch_size": 4, "question_count": 4, "batch_attempts": 1, "workers": 1, "timeout_seconds": 240, "maximum_http_seconds_exclusive": 100}
...
def test_parent_package_is_exact_and_hash_drift_fails_closed(monkeypatch):
    parent = study._parent_v4()
    assert parent.CONTRACT["study_id"] == "hbq-human-alignment-supplemental-providers-ox-alpha-v4"
    changed = dict(study.V4_FILES); changed["study.py"] = "0" * 64
    monkeypatch.setattr(study, "V4_FILES", changed)
    with pytest.raises(ValueError, match="parent file drifted"):
        study._parent_v4()


def test_uncertain_v4_exact_root_proves_one_accepted_cap1_slow_http():
    if not UNCERTAIN_ROOT.is_dir(): pytest.skip("set CWR_OX_V4_UNCERTAIN_ROOT for immutable predecessor verification")
    observed = study.uncertain_v4_commitments(UNCERTAIN_ROOT)
    assert observed["historical_http_attempts"] == [{"status": 200, "duration_ns": 111_868_373_300}]
    assert observed["status"] == "permanently_blocked_uncertain"
    assert observed["journal_present"] is False


def test_uncertain_v4_missing_or_extra_evidence_fails_closed(monkeypatch, tmp_path):
    with pytest.raises(ValueError, match="unavailable|extra, missing, or drifted"):
        study.uncertain_v4_commitments(tmp_path)
    if not UNCERTAIN_ROOT.is_dir(): pytest.skip("set CWR_OX_V4_UNCERTAIN_ROOT for immutable predecessor verification")
    ...
    with pytest.raises(ValueError, match="extra, missing, or drifted"):
        study.uncertain_v4_commitments(UNCERTAIN_ROOT)


def test_cells_are_first_four_v4_leaves_without_reselection():
    if not UNCERTAIN_ROOT.is_dir(): pytest.skip("set CWR_OX_V4_UNCERTAIN_ROOT for immutable predecessor verification")
...
def test_real_v4_evidence_parent_selects_judge_leaf_and_provelock_sibling():
    if not UNCERTAIN_ROOT.is_dir(): pytest.skip("set CWR_OX_V4_UNCERTAIN_ROOT for immutable predecessor verification")
    evidence = UNCERTAIN_ROOT / "runs" / "pilot" / "ox-alpha-v4-01" / "responses" / "batch-0001.attempt-0001.nous.evidence"
    proof = next(evidence.rglob("serialization-proof.json"))
    judge, prove = verify._judge_leaf(evidence, proof)
    assert judge.name == "20260822T021149.704782Z-3af36a8349c6481589411fee4d280f58"
    assert prove.name == "20260822T021141.069205Z-cbe327ef954944a68261f929f85705d1"
...
def test_execute_records_global_duplicate_as_uncertain(monkeypatch, tmp_path, duplicate):
    ...
    def fail_global(work):
        calls.append(str(work)); raise ValueError(f"Ox v5 reuses a provider {duplicate}")
...
def test_terminal_failure_requires_stable_sealed_tree(monkeypatch, tmp_path):
    evidence = tmp_path / "runs" / "pilot" / "ox-alpha-v5-01" / "responses" / "batch-0001.attempt-0001.nous.evidence"; prove = evidence / "prove"; judge = evidence / "judge"; prove.mkdir(parents=True); judge.mkdir()
    (prove / "receipt.json").write_text(json.dumps({"status": "success"}), encoding="utf-8")
    (prove / "events.jsonl").write_text(json.dumps({"event_type": "serialization_proof"}) + "\n", encoding="utf-8")
