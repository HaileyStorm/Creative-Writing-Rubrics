# Long-form map construction

Build a whole-work map from the supplied deterministic units. Return one JSON object that validates against `schema/hbq_long_form_map.schema.json`; return no surrounding prose.

## Required analysis

1. Orient an unfamiliar reader with a compact premise, the evaluated scope, and short cast roles.
2. Map every supplied unit exactly once and in the supplied order.
3. Track chronology, point of view, characters, locations, promises, motifs, and each unit's ending state.
