...
2. Treat declared completion status literally. A work in progress or excerpt is not a failed complete work.
...
9. Sample local units by declared strata, while reserving global claims for the whole-work map. Do not plan to average chapter scores.
10. If `local_sample_limit` is supplied, select no more than that many local units. Cover the most useful distinct strata within the limit; the complete source is still judged globally.
...
14. If `required_sample_ordinals` is supplied, use those one-based unit positions for local diagnostics. The caller will freeze the corresponding unit IDs after validation so matched drafts use the same positions.
