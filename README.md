...
- The bounded [Desc18 Fresh96 development checkpoint](evaluation-results/hbq-human-alignment-optimizer-v9-desc18-broad-replication-sol-veto-result-v1/) tested the unchanged `broader-nextwave-20-missing_evidence_not_no-referent-evidence` descendant across 32 public/open items and 16 prompt groups. Grok MAE fell from `0.7951389` to `0.7795139` (`1.965%`) and the separate frozen Sol veto fell from `1.1751736` to `1.0699653` (`8.9526%`), with `384/384` Sol coverage flags true. This is open development evidence only: endpoints are not pooled, native contact cardinality is unproven, and it grants no new confirmation, generalization, promotion, or runtime authority.
...
- [HANNA development and confirmation](docs/RESULTS.md#hanna-prompt-development-and-held-out-confirmation)
  ...
  bounded Desc18 checkpoint summarized above. DSPy and Optuna remain
  development-only and have no production runtime authority.
- [CWR-guided revision gain](evaluation-results/cwr-guided-revision-gain-v2-live-exec-v9-historical-input-replay-result-v1/)
  has a historical development result that is independently recomputable only
  with the exact completed external V7 evidence root plus the pinned historical
  V6 executor; a fresh held-back confirmation remains open.
